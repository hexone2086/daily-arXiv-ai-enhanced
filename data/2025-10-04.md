...
Motivation: 现有基于图像潜在扩散模型（LDMs）的方法在视频修复中存在时间不一致性问题，需要开发能够同时恢复空间细节和捕捉时间依赖性的视频修复方法。

Method: 利用视频一致性模型（VCMs）作为先验，提出条件机制绕过自动微分，仅需少量神经网络评估即可实现高质量视频重建。

Result: 在多种视频逆问题上的实验表明，该方法在重建保真度和计算效率方面均优于当前最先进的逐帧图像LDM方法。

Conclusion: LVTINO为高清晰度视频修复建立了新的基准，在保持时间一致性的同时实现了卓越的重建质量和计算效率。
...
Conclusion: 该方法能够最大化预训练生成模型的生成能力，通过风格提取和投影技术实现细粒度风格控制，解决了风格描述和控制的难题。
...
<details>
  <summary>Details</summary>
Motivation: 现有操作数据集未关注挣扎随时间的演变，而理解技能学习过程中挣扎类型和频率的变化对于优化人类学习和开发辅助系统至关重要。

Method: 收集了76名参与者执行18个任务的视频数据，任务分为四类活动（打结、折纸、七巧板、洗牌），每个任务重复五次以捕捉技能演变。将挣扎检测定义为时间动作定位任务。

Result: 时间动作定位模型能够成功检测挣扎线索，在未见任务上平均mAP为34.56%，在未见活动上为19.24%，表明挣扎是一个跨任务的可迁移概念。

Conclusion: 挣扎检测在跨任务和跨活动场景中具有可行性，但仍需进一步改进检测性能。数据集已公开提供。
...
Result: 在6个未见下游PDE任务上达到最先进泛化性能，参数数量显著减少，仅需少量微调数据。

Conclusion: SPUS证明了残差U-Net作为PDE求解基础模型的潜力，为多样化PDE系统提供了高度参数高效的基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到图像模型在现实主义方面表现出色，但在处理多人提示时会出现问题：重复面孔、合并身份和错误计数个体。

Method: DisCo通过组相对策略优化(GRPO)微调流匹配模型，使用组合奖励：(i)惩罚图像内面部相似性，(ii)阻止跨样本身份重复，(iii)强制执行准确的人员计数，(iv)通过人类偏好分数保持视觉保真度。采用单阶段课程来稳定训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉地理定位方法在学习地理表示方面仍有改进空间，需要更有效地将视觉内容与地理位置关联起来。

Method: 将地理定位建模为视觉表示与地理表示的对齐，使用分层地理嵌入模型表示世界，并融合图像外观特征和语义分割图形成鲁棒视觉表示。

Result: 在5个基准数据集的25个指标中，22个指标达到了新的最佳性能，超越了现有最先进方法和大型视觉语言模型。

Conclusion: 通过结合地理和视觉表示的方法，显著提升了视觉地理定位的性能，证明了这种组合策略的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法在大型视觉语言模型（LVLM）的指令调优中表现不佳，甚至无法超越随机选择。需要开发专门针对LVLM的高效数据选择方法。

Method: XMAS方法基于理论证明：在指令调优过程中具有相似跨模态注意力矩阵的样本具有相似的梯度。通过聚类样本注意力矩阵的顶部奇异值轨迹，从各聚类中平衡采样构建训练子集。

Result: XMAS能丢弃LLaVA-665k数据集的50%和Vision-Flan数据集的85%，同时完全保持LLaVA-1.5-7B在10个下游基准测试中的性能，并将训练速度提升1.2倍。

Conclusion: XMAS是首个针对LVLM指令调优的高效数据选择方法，能显著减少训练数据冗余，在保持性能的同时加速训练过程。
...
<details>
  <summary>Details</summary>
Motivation: 为了在向量量化图像生成中同时获得连续方法的几何感知能力和离散方法的明确监督，解决传统方法在效率和效果上的权衡问题。

Method: 通过将变分流匹配适配到向量量化潜在空间，学习码书索引的类别后验分布，同时在连续嵌入空间中计算速度场，结合了连续传输动态和离散监督。

Result: 在ImageNet-1k 256x256生成任务上，训练收敛速度比连续流匹配和离散流匹配基线更快，同时获得了与最先进模型竞争的FID分数。
...
<details>
  <summary>Details</summary>
Motivation: 传统对比增强CT需要碘造影剂，存在肾毒性、过敏反应和环境危害等风险。现有深度学习方法采用多阶段流程，导致误差累积且无法利用共享的语义和解剖结构。

Method: 集成条件扩散模型与多任务学习，实现端到端的图像合成和解剖分割联合优化。共享编码器和解码器参数，采用半监督训练策略处理临床数据中常见的缺失分割标签问题。

Result: 在264名患者队列中表现优于现有方法：图像合成PSNR达25.61 dB，腔分割Dice分数提升至0.89，血栓分割Dice分数提升至0.53，临床测量误差显著降低。

Conclusion: 该统一框架在减少造影剂使用的同时，通过联合优化提高了图像合成和分割的准确性，为腹部主动脉瘤的无创评估提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态内容分析复杂、计算成本高且需要大量工程投入，现有预训练模型与视频等复杂数据的融合具有挑战性

Method: 设计候选流水线配方，结合预训练模型将视频转换为时序半结构化数据格式，进一步翻译为帧级索引知识图谱

Result: 创建了可查询的知识图谱表示，支持持续学习，能够通过交互方式动态整合新的领域特定知识

Conclusion: 该框架有效简化了多模态内容分析的原型开发，实现了从视频到结构化知识表示的转换
...
<details>
  <summary>Details</summary>
Motivation: 当前Web代理方法依赖逐步UI交互和大量LLM推理，在动态布局和长任务中容易失败。相比之下，人类通过网站提供的高级功能（如搜索、过滤、排序）更高效地完成任务。

Method: WALT框架逆向工程网站功能，将其转化为可调用的工具，涵盖发现（搜索、过滤、排序）、通信（发布、评论、点赞）和内容管理（创建、编辑、删除）等操作。

Result: 在VisualWebArena和WebArena测试中，WALT实现了更高的成功率、更少的步骤和更少的LLM依赖推理。

Conclusion: WALT建立了一个鲁棒且可泛化的浏览器自动化范式，将计算负担从脆弱的逐步推理转移到可靠的工具调用。
...
<details>
  <summary>Details</summary>
Motivation: 在半监督分割中，从未标记数据中捕捉有意义的语义结构至关重要，这在密集分布的病理图像中尤其具有挑战性。

Method: 利用随机dropout和时间训练快照获得多个扰动预测，通过拓扑一致性机制强制这些不同输出之间保持一致，并引入结合空间重叠和全局结构对齐的新型匹配策略。
...
Method: 提出Diffusion-LPO框架，将用户反馈聚合成图像排名列表，在Plackett-Luce模型下推导列表式DPO目标，强制整个排名的一致性。

Result: 在文本到图像生成、图像编辑和个性化偏好对齐等任务中，Diffusion-LPO在视觉质量和偏好对齐方面始终优于成对DPO基线。

Conclusion: Diffusion-LPO是一个简单有效的列表式偏好优化框架，能更好地利用人类反馈中的排序信息来对齐扩散模型与人类偏好。
...
Method: 采用混合Transformer架构，提出语义到像素的离散表示，将紧凑的语义token与细粒度像素token相结合，在仅增加7.9%序列长度的情况下实现强语言对齐和精确视觉细节描述。

Result: 在多样化多模态基准测试中，Bridge在理解和生成基准上均取得竞争性或更优的结果，同时相比先前的统一MLLM需要更少的训练数据和训练时间。
...
Result: 在训练数据分布相似的测试集上达到94%准确率，与传统CNN相当；在真实世界图像数据上，尽管存在差异，仍达到88%准确率，显著优于传统CNN的72.94%。模型对正确分类样本置信度高，对错误分类样本置信度低。

Conclusion: 贝叶斯推理在数据稀缺环境下能有效提高模型可靠性和泛化能力，有助于改善口腔癌早期诊断。
...
<details>
  <summary>Details</summary>
Motivation: 无源域自适应(SFDA)面临无法直接获取源域数据的挑战，现有方法主要关注评估目标域中与源域相似的不变特征，但这些方法容易受到硬样本和域偏差的影响。

Method: 开发辅助域模块获取多样化表示，通过多个一致性策略获得不变特征表示来区分简单和硬样本，并构建条件多核最大均值差异(CMK-MMD)策略来对齐硬样本。

Result: 在Office-31、Office-Home、VISDA-C和DomainNet-126等多个基准数据集上进行了广泛实验，证明了所提方法带来的显著性能提升。

Conclusion: CADTrans通过构建域一致性的不变特征表示，有效解决了SFDA中的硬样本和域偏差问题，在多个数据集上实现了显著的性能改进。
...
Method: 基于VizWiz-LF数据集中的历史BLV用户问题，系统识别相似的视觉上下文，并利用相关问题指导MLLM生成更相关的描述。

Result: 评估显示，上下文感知描述在76.1%的情况下能够预测并回答用户问题，在54.4%的比较中被优先选择。

Conclusion: 利用BLV用户的历史视觉问题指导MLLM可以显著提高描述的相关性和实用性。
...
Method: 使用两个最先进的VLM（GLM-4.1V-9B-Thinking和Kimi-VL-A3B-Thinking-2506）生成合成数据集，每张图像配有两对思维-答案序列，捕捉逐步推理过程。

Result: 创建了包含结构化思维标记和对应答案的多模态推理数据集，支持训练和评估多模态推理模型。

Conclusion: 该数据集将公开可用，旨在推动推理/思维多模态VLM的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在解决成像逆问题时通常忽略感知矩阵零空间的特定结构，而NPN方法旨在利用零空间的结构信息来设计更有效的先验。

Method: 使用神经网络在感知矩阵的零空间中进行非线性投影，构建低维投影空间，并在此空间中寻找解，该方法可与现有重建框架兼容。

Result: 在压缩感知、去模糊、超分辨率、计算机断层扫描和磁共振成像等多种成像逆问题中，NPN先验均能显著提升重建保真度。

Conclusion: NPN方法通过关注零空间结构，提供了一种可解释且灵活的正则化策略，能够有效提升各种成像逆问题的重建性能。
...
<details>
  <summary>Details</summary>
Motivation: 为了在基因组医学中建立可靠的自动决策基础，需要将原始DNA序列转化为可解释且可操作的决策，以支持医疗自动化和机器人系统的集成。

Method: 结合混沌游戏表示(CGR)和概念瓶颈模型(CBM)，通过GC含量、CpG密度和k-mer基序等生物学概念进行预测。采用概念保真度监督、先验一致性对齐、KL分布匹配和不确定性校准来增强可靠性。

Result: 在HIV亚型分类任务中达到最先进的性能，在内部和LANL数据集上均表现出色。系统提供可解释的证据，能够直接验证生物学先验，并通过成本感知推荐层实现更好的成本效益权衡。

Conclusion: 该工作通过弥合可解释基因组建模与自动决策之间的差距，为基因组医学中的机器人和临床自动化建立了可靠的基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型缺乏显式的逐步推理，忽视可承受性约束和几何关系，且后训练流程很少强化推理质量。

Method: 集成RLVR与GRPO，设计基于可验证奖励的后训练策略，创建VLA-CoT-13K数据集提供链式思维监督。

Result: 在领域内、领域外、仿真和真实机器人平台上均表现出优越的泛化能力和实际性能。

Conclusion: VLA-R1通过系统优化推理和执行，显著提升了VLA模型的推理鲁棒性和执行准确性。
...
<details>
  <summary>Details</summary>
Motivation: 微距摄影中的离焦模糊是一个长期存在的问题，严重阻碍了被拍摄物体的清晰成像和高质量3D重建。传统去模糊方法需要大量图像和标注，且目前没有针对微距摄影的多视角3D重建方法。
...
Conclusion: 该方法成功解决了微距摄影中的离焦模糊问题，实现了联合去模糊和3D重建，为小尺寸细节物体的高质量3D建模提供了有效解决方案。
...
Result: 在全参考指标上取得优越性能，超越先前的扩散方法，与其他最先进模型性能相当。
...
<details>
  <summary>Details</summary>
Motivation: 青铜器铭文作为早期中国文字的重要阶段，对考古和历史研究至关重要，但由于严重的视觉退化、跨领域变异（照片、拓片、摹本）和极长的字符分布，自动识别仍然困难。

Method: 构建包含22454张全页图像和198598个标注字符的大规模数据集，开发两阶段检测识别流程，使用LadderMoE增强预训练CLIP编码器，实现动态专家专业化和更强的鲁棒性。

Result: 在单字符和全页识别任务上的综合实验表明，该方法显著优于最先进的场景文本识别基线，在头部、中部和尾部类别以及所有采集模式上都实现了更高的准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的UDA方法需要为每个新的源-目标对微调整个骨干网络，导致参数数量和存储需求线性增长，且无法重用训练好的骨干网络参数。

Method: VirDA在骨干网络前添加领域特定的视觉重编程层，生成视觉提示作为纹理偏置来调整输入图像的风格，通过优化域内和域间分布差异来训练这些层，而不修改骨干网络参数。

Result: 在Office-31数据集上达到92.8%的平均准确率，仅需1.5M可训练参数，优于PDA方法1.6%准确率且参数减少54%，相比全骨干微调方法仅需1.7%-2.8%的参数。

Conclusion: VirDA提供了一种参数高效的领域自适应方法，能够在保持高性能的同时大幅减少训练参数和存储需求，实现骨干网络在不同领域的重用。
...
Result: 在压力检测、人格预测和抑郁检测三个心理学任务中，基于学习标记的简单词袋模型持续优于FACS流程以及强图像和视频表示学习模型。

Conclusion: DFE能够比FACS捕获更精确的面部行为，覆盖更广泛的面部显示，有望成为心理学和情感计算应用中FACS的可扩展有效替代方案。
...
Conclusion: Con-NRSfM方法消除了现有方法的约束，能够准确计算局部保形尺度，实现更精确的深度估计，为可变形SLAM提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖密集观测来优化模型参数，难以处理不一致的多视角图像。需要一种能处理多样化图像不一致性的通用方法。

Method: 将鲁棒重建分解为修复和重建两个子任务：先将不一致图像转换为初始视频，使用专门设计的视频扩散模型修复为一致图像，最后从修复后的图像重建3D场景。

Result: 在合成和真实数据集上的大量实验表明，该方法在鲁棒重建方面具有强大的泛化能力和优越性能，并能控制重建3D场景的风格。

Conclusion: UniVerse框架通过解耦修复和重建任务，利用扩散模型学习的大规模场景先验，有效解决了多视角图像不一致的3D重建问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统模板匹配方法在复合变换下效率低下，而深度学习方法仅估计相似度分数而不显式建模几何姿态，无法满足实际部署需求。

Method: 使用模板感知动态卷积模块动态注入模板特征，结合深度可分离卷积和像素洗牌实现高效匹配，采用旋转-剪切增强策略进行无几何标注训练，并通过轻量级细化模块提升精度。

Result: 3.07M参数模型在复合变换下实现高精度和14ms推理速度，在小模板和多目标场景中表现出强鲁棒性。

Conclusion: 该方法适合实时工业应用部署，能够有效解决模板匹配中的姿态估计问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM在处理需要精确理解细粒度视觉元素的任务时表现不佳，主要由于图像编码过程中的信息丢失或对关键区域关注不足。虽然引入像素级信息有帮助，但往往被过度使用导致效率低下和无关视觉细节干扰

Method: 首先进行操作感知的监督微调建立文本推理和视觉操作的基础能力，然后设计基于模型自身响应反馈的rollout引导强化学习框架，让VLM根据查询难度动态决定何时调用像素操作

Result: 在广泛的多模态推理基准测试中，模型在HR-Bench 4K上达到73.4%准确率，同时工具使用率仅为20.1%，相比之前方法在提高准确率的同时减少了66.5%的工具使用

Conclusion: 提出的自适应像素推理框架能有效平衡性能与效率，在保持高准确率的同时显著减少不必要的视觉操作，为VLM的细粒度视觉理解提供了更高效的解决方案
...
Method: 使用临床合理的旋转（±15°/±30°）并利用RAD-DINO编码器测量嵌入变化，通过敏感度评分将样本分为稳定性四分位数。

Result: 高敏感度病例的召回率显著降低（-0.2到-0.3），尽管AUC和置信度很高，ASRS能够有效识别错误倾向的病例。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频风格化方法存在时间一致性差、风格丰富度低的问题，而训练专用模型需要配对视频数据且计算成本高。

Method: 使用预训练I2V模型整合多个风格化参考，采用高频补偿约束内容布局和运动，结合基于光流的运动线索保留低显著性区域的风格纹理。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像质量评估方法受限于标量评分指标，无法反映专家评估中的人类推理过程，需要建立更符合人类评估方式的基准。

Method: 构建MedQ-Bench基准，包含MedQ-Perception（低层感知能力）和MedQ-Reasoning（无参考和比较推理任务），采用多维评判协议评估模型输出。

Result: 评估14个先进MLLM显示模型具有初步但不稳定的感知和推理能力，临床应用的准确性不足。

Conclusion: MLLM在医学图像质量评估中需要针对性优化，MedQ-Bench将推动该领域的进一步探索和发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要昂贵的输入格式（类别标签、二值分割掩码）和推理成本（二次方量的前向传播），限制了实际应用。

Method: InstaFormer基于对象查询和潜在掩码描述符之间的交互，这些描述符语义上表示相同对象但携带互补信息。

Result: 通过全面基准测试和消融实验验证了该方法的有效性。

Conclusion: 提出的InstaFormer能够以更低的成本实现实例级几何理解，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有的CNN和Transformer模型在处理复杂风格和高分辨率输入时效率低下，需要一种能够同时捕捉局部细节和全局上下文的高效方法。

Method: 提出PyramidStyler框架，采用金字塔位置编码(PPE)实现多尺度特征捕捉，并结合强化学习动态优化风格化过程。
...
<details>
  <summary>Details</summary>
Motivation: 现有分治方法在大规模无边界场景中存在两个瓶颈：(i) 分区负载严重不平衡，均匀或启发式分割无法反映实际计算需求；(ii) 粗到细流水线未能有效利用粗阶段，经常重新加载整个模型并产生高开销。

Method: 引入深度感知分区方法将预处理从数小时减少到数分钟；采用基于优化的策略平衡可见高斯分布（计算负载的强代理）；使用可见性裁剪和选择性致密化两种轻量级技术进一步降低训练成本。

Result: 在大规模城市和户外数据集上的评估显示，LoBE-GS始终保持比最先进基线快2倍的端到端训练时间，同时保持重建质量，并能够扩展到传统3DGS无法处理的场景。
...
Result: MemoryPack设计能随视频长度优雅扩展，保持计算效率和线性复杂度；Direct Forcing显著提升上下文一致性和长视频生成的可靠性。

Conclusion: MemoryPack和Direct Forcing共同大幅增强了长视频生成的上下文一致性和可靠性，推进了自回归视频模型的实际可用性。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶系统中，精确的目标检测和不确定性估计对于自感知和安全运行至关重要。需要关注完整预测置信度分布在所有类别上的校准，而不仅仅是主导类别的预测。

Method: 提出了两种辅助正则化损失项：一种校准主导预测，另一种校准完整预测向量。评估了多种后处理和训练时方法，包括等渗回归，在CenterPoint、PillarNet和DSVT-Pillar等3D检测器上进行测试。

Result: 结合完整类别预测校准损失项和等渗回归的方法在CenterPoint和PillarNet上取得了最佳校准效果，同时改善了主导和次要类别预测的校准。但DSVT-Pillar无法使用相同方法同时校准主导和次要预测。

Conclusion: 完整预测分布校准对于3D目标检测器的置信度校准至关重要，提出的方法在多个检测器上有效，但不同检测器可能需要特定的校准策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有人员搜索方法主要使用ImageNet预训练骨干网络，可能无法有效捕捉复杂空间上下文和细粒度身份线索，且共享骨干网络导致检测和重识别任务优化目标冲突
...
Conclusion: DiffPS通过利用扩散先验知识有效解决了人员搜索中检测和重识别任务的优化冲突问题，取得了优异性能
...
<details>
  <summary>Details</summary>
Motivation: 高光谱成像成本高且重建困难，压缩感知系统如CASSI虽提高效率，但重建仍面临严重退化和光谱细节丢失的挑战。

Method: 提出Flow-Matching-guided Unfolding网络(FMU)，将流匹配嵌入深度展开框架，引入平均速度损失增强流一致性，结合优化方法的可解释性和流匹配的生成能力。

Result: 在模拟和真实数据集上的广泛实验表明，FMU在重建质量上显著优于现有方法。

Conclusion: FMU成功将流匹配集成到HSI重建中，通过混合设计实现了更鲁棒和准确的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统工业组件缺陷检测耗时耗力，给质检人员带来巨大负担且难以管理产品质量，需要自动化解决方案。

Method: 使用数字相机光学和深度学习模型，采用ConSinGAN生成合适尺寸的训练测试数据集，比较四种YOLO模型(v3,v4,v7,v9)单独及结合ConSinGAN的效果。

Result: YOLOv7结合ConSinGAN在精度(95.50%)和检测时间(285ms)上优于其他YOLO版本，远胜于基于阈值的方法。

Conclusion: 所提出的自动缺陷检测系统易于建立，可处理多种缺陷类型或缺陷数据不足的情况，并开发了SCADA系统和相关传感器架构。
...
<details>
  <summary>Details</summary>
Motivation: 少样本异常检测在工业安全检查中很重要，但样本有限导致正常和异常特征难以区分，特别是在类别无关条件下

Method: 利用基础视觉编码器学习正常图像的通用分布，通过非线性投影算子将图像映射到自然图像流形，识别分布外区域

Result: 实验表明该方法支持多类别检测，性能具有竞争力且参数数量显著少于先前方法
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers 在真实世界机器人系统中应用受限，因为其二次注意力复杂度。现有 token 合并方法适合分类但不适合密集预测任务。

Method: 在 ViT 骨干网络中加入可训练的聚类模块，根据分割掩码的伪聚类指导合并相似 token，然后使用再生模块恢复细节用于下游任务。

Result: 在三个不同数据集上实现了 2.18 倍更少的 GFLOPs 和 1.64 倍更快的推理速度，同时保持可比较的分割精度。

Conclusion: ClustViT 有效解决了 ViT 在密集预测任务中的计算效率问题，为实际机器人应用提供了可行的解决方案。
...
Method: 引入视觉参考标记(VRTs)，从查询图像的视觉补丁嵌入中提取，与LLM输出文本标记交错；使用轻量级解码器将LLM输出转换为检测、分割和定位预测；采用动态扩展嵌入表和独立处理VRTs的方法。

Result: 在四个视觉感知和理解任务上的实证研究表明，PaDT始终达到最先进的性能，即使与显著更大的MLLM模型相比也是如此。

Conclusion: PaDT提供了一个统一的范式，使MLLM能够直接生成多样化的视觉输出，在多个视觉任务上表现出色，代码已开源。
...
TL;DR: 4DGS-Craft是一个一致且交互式的4D高斯溅射编辑框架，通过4D感知的InstructPix2Pix模型、多视图网格模块和基于LLM的用户意图理解模块，解决了视图、时间一致性以及复杂文本指令处理的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯溅射编辑方法在视图、时间一致性以及非编辑区域保持方面存在挑战，且难以处理复杂的文本指令。

Method: 提出4D感知InstructPix2Pix模型，集成4D VGGT几何特征；使用多视图网格模块迭代优化多视图输入图像；设计高斯选择机制保护非编辑区域；采用基于LLM的用户意图理解模块分解复杂指令。

Result: 相比相关工作，该方法实现了更一致和可控的4D场景编辑。

Conclusion: 4DGS-Craft框架能够有效处理4D高斯溅射编辑中的一致性和复杂指令问题，提供更好的编辑性能。
...
Method: Pure-Pass机制利用固定颜色中心点对像素进行分类，识别纯像素并免除其复杂计算，实现像素级细粒度掩码，保持自适应灵活性。

Result: PP-ATD-light在保持相似计算量的情况下，在重建质量和参数效率方面均优于CAMixer-ATD-light，实现了更优的超分辨率性能。
...
TL;DR: 使用GPT-4o多模态能力自动生成颌骨囊肿的影像学发现，通过自校正循环框架(SLSO)提高准确性，相比传统CoT方法在多个评估项目上有显著改进。


<details>
  <summary>Details</summary>
Motivation: 提高牙科全景片中颌骨囊肿影像学发现的自动生成准确性，减少幻觉现象，改善结构化输出的一致性。

Method: 构建SLSO框架，包含10步处理流程：图像输入分析、结构化数据生成、牙号提取与一致性检查、检测到不一致时的迭代再生、发现生成及后续重构与验证。

Result: SLSO框架在牙号、牙齿移动和牙根吸收方面的准确率分别提高了66.9%、33.3%和28.6%。成功案例中经过最多5次再生后获得一致的结构化输出。

Conclusion: SLSO框架能有效强制执行阴性发现描述、抑制幻觉并提高牙号识别准确性，但识别跨多颗牙齿的大范围病变仍有局限，需要进一步改进以实现实用的发现生成系统。
...
TL;DR: 提出了一种名为LiLa-Net的3D自编码器架构，仅使用LiDAR点云从真实交通环境中提取高效特征，通过简化编码器层和跳跃连接实现了准确的点云重建。


<details>
  <summary>Details</summary>
Motivation: 开发一个仅依赖LiDAR点云就能从真实交通环境中提取高效特征的3D自编码器，旨在减少资源消耗同时保持性能。

Method: 采用跳跃连接概念，减少编码器层数并简化跳跃连接，构建高效且具有代表性的潜在空间，实现原始点云的准确重建。

Result: 模型在跳跃连接携带的信息与潜在编码之间实现了有效平衡，提高了重建质量而不影响性能，并展现出强大的泛化能力，能成功重建与原始交通环境无关的物体。

Conclusion: LiLa-Net在资源消耗较少的情况下实现了高质量的点云重建，具有良好的泛化性能，适用于真实交通环境中的3D特征提取。
...
Result: 相比地面方法，无人机观察将可见性损失减少15%，捕获更多行为转换且精度和连续性更高。验证了969个行为序列，发现不同物种行为模式差异。
...
TL;DR: GaussianMorphing是一个从多视角图像进行语义感知3D形状和纹理变形的新框架，通过网格引导的3D高斯泼溅实现高保真几何和外观建模，在TexMorph基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖点云或需要预定义同胚映射来处理无纹理数据，存在局限性。需要开发能够同时处理形状和纹理变形的高质量方法。

Method: 使用网格引导的3D高斯泼溅(3DGS)进行高保真建模，核心是统一变形策略，将3D高斯锚定到重建的网格面片上，通过拓扑感知约束保持纹理保真度，并利用网格拓扑作为几何先验建立无监督语义对应。

Result: 在TexMorph基准上，GaussianMorphing显著优于先前的2D/3D方法，颜色一致性误差(ΔE)降低22.2%，EI降低26.2%。

Conclusion: 该方法能够在不需要标注数据的情况下，在整个变形过程中保持局部细节和全局语义一致性，为3D形状和纹理变形提供了有效的解决方案。
...
TL;DR: 提出InPose方法，使用预训练扩散模型仅基于旋转测量进行姿态估计，通过似然项引导生成最符合稀疏传感器测量的姿态序列，实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有基于条件扩散模型的方法泛化性能差，主要因为位置测量高度受用户体型影响，需要解决跨用户泛化问题。

Method: 将姿态估计建模为逆问题，使用预训练扩散模型仅以旋转测量为条件，通过从测量位置导出的似然项引导模型先验。

Result: InPose方法能够为零样本泛化生成高度可能的姿态序列，准确解释稀疏体上测量数据。

Conclusion: 提出的逆问题公式和算法设计有效解决了跨用户泛化问题，在有限传感器设置下实现鲁棒姿态估计。
...
TL;DR: 提出VGDM框架，结合视觉transformer和扩散模型进行脑肿瘤检测与分割，相比传统U-Net在边界精度和体积准确性方面有显著提升
...
Conclusion: transformer引导的扩散模型有潜力推动肿瘤分割技术发展，超越传统U-Net基准，为神经肿瘤学提供改进的鲁棒性和可扩展性路径
...
Result: 处理了941个高分辨率图块，覆盖整个法国本土，最终马赛克总体准确率达到73%，有效捕捉了多样化的城市模式。

Conclusion: 成功创建了首个开放获取的全国尺度城市足迹数据集，支持长期城市化动态的后续研究。
...
TL;DR: 系统分析腹腔镜胆囊切除术视频中点跟踪的失败模式，发现点跟踪对于手术工具表现良好，但对于解剖目标（如胆囊）表现不佳，主要由于组织相似性和边界模糊导致失败。
...
Result: 点跟踪在手术工具上表现有竞争力，但在解剖目标上持续表现不佳，主要由于组织相似性和模糊边界导致失败。

Conclusion: 通过定性分析揭示了影响跟踪结果的关键因素，并提供了选择和放置跟踪点的实用建议，以改善手术视频分析性能。
...
TL;DR: 提出FFREEDG新任务：在服务器预训练后仅使用客户端未标注数据进行联邦学习，不重新访问源数据。FRIEREN框架利用视觉基础模型，结合视觉语言模态和弱到强一致性学习来解决该任务。
...
Method: FRIEREN框架：1）利用CLIP文本嵌入引导的视觉语言解码器改善语义消歧；2）采用弱到强一致性学习策略在伪标签上进行鲁棒的本地训练。

Result: 在合成到真实和清晰到恶劣天气基准测试中，该方法有效解决了新任务，性能与现有域泛化和适应方法相当，为未来研究建立了强基线。

Conclusion: 提出的FFREEDG任务具有挑战性，FRIEREN框架通过整合视觉语言模态和一致性学习策略，为该任务提供了有效解决方案，展现了在联邦学习场景下处理未标注域偏移数据的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示方法过于抽象，忽略了监控视频中定义复杂异常的细粒度人-物交互或动作语义。
...
TL;DR: 提出一种基于猪耳静脉模式的非侵入性生物识别方法，使用智能手机采集图像，通过计算机视觉和机器学习实现98.12%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 传统猪只识别方法（耳标、芯片）不可靠、成本高且主要针对纯种猪，不适用于小规模养殖户。

Method: 收集20头混种猪的800张耳部图像，开发多阶段计算机视觉流程增强静脉可见性，提取结构和空间特征，使用支持向量机进行分类。

Result: 支持向量机在混种猪群中达到98.12%的识别精度，从图像处理到分类平均耗时8.3秒，适合实时部署。

Conclusion: 该系统通过永久性生物标记替代易损物理标识，为农民提供经济高效、无压力的动物识别方法，证实耳静脉生物识别在数字化畜牧管理中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 解决密集和遮挡场景中传统检测计数方法失效的问题，需要开发能够处理多类别对象计数的密度图估计方法。

Method: 使用Twins金字塔视觉transformer作为骨干网络，结合多类别计数头和基于分割的类别聚焦模块，采用两任务设计抑制训练时的类别间干扰。

Result: 在VisDrone和iSAID基准测试中表现优于现有多类别人群计数方法，MAE分别降低了33%、43%和64%，并在生物多样性监测数据集中展示了应用潜力。

Conclusion: 该方法证明了在密集场景中人群计数方法的必要性，其区域损失设计为多类别人群计数开辟了新领域，有助于保护工作和生态研究。
...
Result: TempoControl能够在保持高视频质量和多样性的同时，实现对时间安排的精确控制，适用于多种视频生成应用。

Conclusion: 该方法为生成视频模型提供了有效的时间控制能力，扩展了视频生成的应用范围。
...
TL;DR: 提出了RewardMap框架，通过密集奖励和多阶段强化学习解决多模态大语言模型在细粒度视觉推理任务中的稀疏奖励和不稳定优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在结构化信息丰富的场景（如交通地图）中的空间推理能力不足，而标准强化学习因稀疏奖励和优化不稳定而受阻。
...
TL;DR: DragFlow是首个利用FLUX强大先验进行拖拽式图像编辑的框架，通过区域级编辑范式、个性化适配器和多模态大语言模型，显著优于现有基线方法。
...
Conclusion: DragFlow成功利用FLUX的丰富先验进行拖拽式编辑，通过区域级监督和个性化增强，显著提升了编辑质量和一致性，为拖拽式图像编辑设定了新的基准。
...
TL;DR: 本文提出F2C方法，通过选择关键片段而非单帧来保持视频的时间连贯性，同时采用自适应分辨率策略控制token数量，在多个长视频基准测试中显著优于均匀采样方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型面临"大海捞针"问题：原始视频帧产生的大量视觉token耗尽模型上下文窗口。现有解决方案通过选择稀疏帧集来减少token数量，但这种逐帧选择丢弃了关键的时间动态信息，导致对运动和事件连续性的推理效果不佳。

Method: 提出F2C方法：1）将选择范围从孤立关键帧扩展到关键片段（短时间连贯片段）；2）采用自适应分辨率策略，动态平衡空间分辨率和片段长度，确保每个视频的token数量恒定。

Result: 在三个长视频基准测试上的实验表明，这种无需训练的方法相比均匀采样在Video-MME、LongVideoBench和MLVU基准上分别提升了8.1%、5.6%和10.3%。

Conclusion: 研究结果强调了在帧选择中保持时间连贯性的重要性，为将视频大语言模型扩展到实际视频理解应用提供了实用途径。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习和可穿戴传感器的发展，需要在真实世界条件下准确评估人体运动，这对远程医疗、运动科学和康复至关重要。

Method: 使用VIDIMU数据集，包含13种临床相关日常活动的视频和IMU数据，评估四种深度学习框架(MotionAGFormer、MotionBERT、MMPose 2D-to-3D姿态提升和NVIDIA BodyTrack)与IMU数据的性能对比。

Result: MotionAGFormer表现最优，总体RMSE为9.27±4.80度，MAE为7.86±4.18度，Pearson相关性0.86±0.15，决定系数R²为0.67±0.28。两种技术都可行但各有权衡。

Conclusion: 研究阐明了现成视频模型在健康成人中提供临床可行运动学的位置，以及它们落后于IMU的位置，为开发远程医疗解决方案提供了指导。
...
TL;DR: NeuroSwift通过整合AutoKL和CLIP适配器的扩散模型，实现了从fMRI数据中跨被试重建视觉刺激，仅需微调17%参数就能在轻量级GPU上快速训练。


<details>
  <summary>Details</summary>
Motivation: 解决从脑活动中重建视觉信息时面临的跨被试差异大、计算成本高以及大脑对复杂视觉输入的抽象语义编码等挑战。

Method: 提出NeuroSwift框架，集成AutoKL（处理低级特征）和CLIP（处理语义）适配器，通过扩散模型进行训练。CLIP适配器使用Stable Diffusion生成图像与COCO标题配对训练，模拟高级视觉皮层编码。跨被试时预训练一个被试，然后仅微调17%参数（全连接层）。

Result: 在轻量级GPU（3个RTX 4090）上每个被试仅需1小时训练，就能达到最先进的性能，超越现有方法。

Conclusion: NeuroSwift通过创新的适配器设计和参数高效微调策略，成功解决了跨被试视觉重建的挑战，实现了高效准确的视觉刺激重建。
...
TL;DR: microCLIP是一个自训练框架，通过Saliency-Oriented Attention Pooling和TokenFusion模块联合优化CLIP的视觉和文本表示，在细粒度图像分类任务上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在细粒度图像分类任务中表现受限，因为它依赖粗粒度的全局特征，而忽略了微观局部线索。现有方法通过对齐大语言模型描述与CLIP的[CLS]token来注入细粒度知识，但缺乏空间精度。

Method: 提出Saliency-Oriented Attention Pooling(SOAP)构建显著性引导的[FG]token，与全局[CLS]token融合实现粗-细粒度对齐；采用双头LLM衍生分类器，包括冻结分类器提供稳定文本先验和可学习分类器进行微调；开发动态知识聚合机制迭代优化伪标签。

Result: 在13个细粒度基准测试上平均准确率提升2.90%，仅需轻量级适配。

Conclusion: microCLIP成功挖掘了CLIP中的潜在细粒度信号，为细粒度图像分类提供了一种有效的无监督适配方法。
...
TL;DR: VidGuard-R1是首个基于多模态大语言模型(MLLM)的视频真实性检测器，通过群体相对策略优化(GRPO)进行微调，提供高精度判断和可解释推理。
...
Method: 使用群体相对策略优化(GRPO)微调多模态大语言模型(Qwen-VL)，配备两个专门针对时间伪影和生成复杂度的奖励模型，在包含14万真实和AI生成视频的挑战性数据集上进行训练。

Result: VidGuard-R1在现有基准测试中实现了最先进的零样本性能，额外训练后准确率超过95%，案例研究显示其预测具有精确且可解释的推理过程。

Conclusion: VidGuard-R1通过结合高精度检测和可解释推理，为解决AI生成视频检测的挑战提供了有效解决方案，代码已公开。
...
TL;DR: 提出一种简单有效的方法来缓解长视频生成中的质量退化问题，无需长视频教师监督或长视频数据集重新训练，通过利用教师模型的知识为自生成长视频中的采样片段提供指导。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成方面取得了革命性进展，但基于transformer架构的计算成本过高，特别是在生成长视频时。现有自回归方法在超越训练范围时会出现明显的质量退化问题。

Method: 利用教师模型的丰富知识，通过从自生成长视频中抽取采样片段为学生模型提供指导，保持时间一致性，避免过曝光和误差累积问题。

Result: 能够将视频长度扩展到教师模型能力的20倍以上，最长可生成4分15秒的视频，相当于基础模型位置嵌入支持的最大跨度的99.9%，比基线模型长50倍以上。

Conclusion: 在标准基准测试和改进基准测试上的实验表明，该方法在保真度和一致性方面显著优于基线方法。
...
TL;DR: KineMask是一种物理引导的视频生成方法，通过两阶段训练策略和物理基础控制机制，显著提升了物体交互的真实性和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在物体交互的物理合理性方面存在不足，缺乏物理基础的控制机制，限制了其在机器人学和具身决策等领域的应用潜力。

Method: 提出两阶段训练策略，通过物体掩码逐步移除未来运动监督，在合成场景上训练视频扩散模型，并结合低层运动控制与高层文本条件。

Result: 在真实场景中显著改善了物体交互效果，相比同类规模模型有显著提升，能够有效支持复杂动态现象的合成。

Conclusion: KineMask通过物理引导的视频生成方法，成功实现了更真实的刚体控制和物体交互，低层和高层条件在视频扩散模型中具有互补作用。
...
TL;DR: 提出了一种结合本体感觉、运动感觉、力触觉和肌肉激活等多模态感官的细粒度动作模拟方法，通过特征对齐和因果正则化提升家庭机器人精细控制的模拟精度


<details>
  <summary>Details</summary>
Motivation: 当前视频模型缺乏细粒度控制能力，无法作为世界模型。通用家庭机器人需要实时精细运动控制来处理精细任务和紧急情况

Method: 引入多模态感官捕捉精确控制，开发特征学习范式对齐不同模态同时保留各自独特信息，提出正则化方案增强动作轨迹特征的因果性

Result: 实验表明多模态感官的加入提高了模拟精度并减少了时间漂移，广泛的消融研究和下游应用证明了方法的有效性和实用性

Conclusion: 多模态感官方法能够有效模拟精细交互，为家庭机器人的精细控制提供了可行的解决方案
...
TL;DR: VideoNSA通过原生稀疏注意力机制增强视频语言模型的长视频理解能力，在Qwen2.5-VL基础上实现128K令牌扩展，在长视频理解、时序推理和空间基准测试中表现优异。
...
Result: 相比令牌压缩和无训练稀疏基线，VideoNSA在长视频理解、时序推理和空间基准测试中表现更好，能可靠扩展到128K令牌，并发现最优全局-局部注意力分配模式。
...
TL;DR: NoiseShift是一种无需训练的方法，通过重新校准去噪器的噪声水平来改善扩散模型在低分辨率图像生成中的性能，解决了噪声调度器在不同分辨率下感知效果不均等的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的高分辨率文本到图像生成器无法为不需要高分辨率图像的用户提供开箱即用的经济高效替代方案，因为扩散模型在固定分辨率集上训练后往往无法泛化到低分辨率生成。
...
Conclusion: NoiseShift有效缓解了分辨率相关的伪影，显著提升了低分辨率图像生成的质量，证明了其在改善扩散模型分辨率泛化能力方面的有效性。
...
TL;DR: 该论文研究从视频中预测动态物理属性的任务，包括弹性、粘度和动态摩擦，通过构建数据集并比较多种视频分析方法。


<details>
  <summary>Details</summary>
Motivation: 研究如何从视频中推断需要时间信息才能判断的物理属性，如弹性、粘度和动态摩擦，以扩展计算机视觉在物理属性预测方面的应用。

Method: 收集了三个物理属性的视频数据集；探索了三种推断方法：基于传统计算机视觉的oracle方法、使用视觉提示和可训练提示向量的预训练视频模型方法、以及多模态大语言模型的提示策略。

Result: 生成式或自监督预训练的视频基础模型性能相似，但落后于oracle方法；多模态大语言模型目前表现较差，但通过合适的提示可以改善性能。
...
TL;DR: 提出了声音对象检测任务，通过多模态对象感知框架从第一人称视角视频中学习，将声音与直接参与的物体关联起来。


<details>
  <summary>Details</summary>
Motivation: 日常物体交互产生独特的声音，但现有模型难以区分这些声音对应的具体物体，如勺子敲击硬木地板与地毯的声音差异。

Method: 开发自动管道计算参与物体的分割掩码，引导模型关注交互中最具信息量的区域；使用槽注意力视觉编码器强化对象先验。

Result: 在新任务和现有多模态动作理解任务上实现了最先进的性能。

Conclusion: 提出的对象感知框架能有效链接声音与相关物体，在声音对象检测任务中表现出色。
...
Method: 使用核密度估计(KDE)识别低密度区域，策略性地注入高斯点嵌入视角依赖的虚假物体；引入自适应噪声策略破坏多视角一致性；提出基于KDE的攻击难度评估协议。

Result: 大量实验表明，该方法相比现有技术具有更优越的性能，能够从投毒视角清晰显示虚假物体，同时对无辜视角影响最小。

Conclusion: 本文提出的密度引导投毒方法有效揭示了3DGS的脆弱性，为未来研究提供了客观的基准测试框架。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在单实体提示上表现良好，但在多主体描述中经常出现属性泄漏、身份纠缠和主体遗漏等问题，需要专门的方法来提升多主体保真度。

Method: 通过随机最优控制视角看待流匹配，将主体解缠定义为对训练好的流匹配采样器的控制。提出两种架构无关算法：无需训练的单次更新测试时控制器，以及轻量级微调规则Adjoint Matching。

Result: 在Stable Diffusion 3.5、FLUX和Stable Diffusion XL上，两种算法一致提升了多主体对齐度，同时保持了基础模型的风格。测试时控制可在消费级GPU上高效运行，微调控制器在有限提示上训练后能泛化到未见提示。
...
TL;DR: 提出了一种用于GNSS拒止环境下无人机长距离飞行的全机载系统，通过激光雷达高度图匹配和粒子滤波来校正漂移，在CPU硬件上实现实时运行。


<details>
  <summary>Details</summary>
Motivation: 解决无人机在GNSS拒止环境中长距离飞行时的漂移问题，特别是在无先验密集地图和嵌入式平台计算能力有限的情况下。

Method: 集成感知、建图、规划和控制系统，使用轻量级漂移校正方法，通过梯度模板匹配将激光雷达局部高度图与先验地理数据高度图匹配，并在聚类粒子滤波中将证据与里程计融合。

Result: 在比赛中执行了千米级飞行，跨越城市、森林和开阔地地形，相对于原始里程计显著减少了漂移，在仅使用CPU的硬件上实现实时运行。

Conclusion: 该系统为GNSS拒止环境下的无人机自主飞行提供了实用设计见解，证明了在有限计算资源下实现可靠长距离导航的可行性。
...
TL;DR: 提出了一种结合模型预测控制(MPC)和控制屏障函数(CBFs)的安全运动规划策略，用于自主船舶在狭窄内河水道的导航。通过时变膨胀椭圆障碍物表示和自适应膨胀机制，减少控制器的保守性。


<details>
  <summary>Details</summary>
Motivation: 传统运动规划方法在狭窄内河水道等挑战性环境中计算密集或过于保守，需要一种既能保证安全又能实时导航的解决方案。

Method: 使用MPC提供近似运动规划，结合高阶CBFs确保安全。采用时变膨胀椭圆障碍物表示，根据船舶与障碍物的相对位置和姿态自适应调整膨胀半径。

Result: 仿真和真实世界实验表明，该策略使全驱动自主机器人船舶能够在实时条件下安全通过狭窄空间，并解决潜在的僵局问题。

Conclusion: 提出的MPC-CBF组合策略有效平衡了安全性和实时性，为自主船舶在狭窄水道中的安全导航提供了实用解决方案。
...
Method: 采用基于连续时间运动学的因子图优化方法，使用简单机器人模型配合高速率传感，通过高斯过程处理噪声。

Result: 能够连续估计机器人的位姿、速度和应变的均值和协方差，具有线性求解复杂度，并在实际系统中展示了其多功能性。

Conclusion: 该方法提供了一种高效、适应性强的连续体机器人状态估计解决方案，能够处理未建模干扰和数据丢失问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决VLMs在机器人导航任务中由于动作空间差异和预训练目标不同导致的迁移困难问题，让机器人能够在非结构化开放世界中适应多样化的人类指令并安全操作。

Method: 使用图像扩散模型生成路径掩码作为视觉规划，通过自监督跟踪模型获取路径掩码监督信号，结合VLM增强的标注进行训练，避免手动像素级标注。

Result: 在真实世界评估中，VENTURA在物体到达、障碍物避让和地形偏好任务上优于最先进的基础模型基线，成功率提高33%，碰撞减少54%，并能泛化到未见过的任务组合。

Conclusion: VENTURA展示了通过视觉路径规划方法可以有效解决VLMs在机器人导航中的迁移问题，并展现出组合泛化能力，为开放世界机器人导航提供了有前景的解决方案。
...
TL;DR: INSIGHT框架通过分析VLA模型的token级不确定性信号来预测何时需要请求人类帮助，使用transformer分类器处理不确定性序列，比较了强弱监督两种训练方式的效果。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏内省机制来预测失败并请求人类监督，需要开发能够主动识别不确定性并寻求帮助的框架。

Method: 基于π₀-FAST模型提取token级不确定性信号（熵、对数概率、偶然和认知不确定性），训练紧凑transformer分类器将这些序列映射到帮助触发信号，探索强弱监督两种训练方式。

Result: 强监督能捕捉细粒度不确定性动态实现可靠帮助检测，弱监督在训练和评估一致时仍具竞争力，且建模token级不确定性的时间演化比静态序列级评分具有更强预测能力。
...
TL;DR: 提出动态抛物线控制屏障函数(DPCBF)来解决非完整机器人在密集动态环境中CBF方法保守性和不可行的问题，通过动态调整抛物线边界提高导航成功率和QP可行性。


<details>
  <summary>Details</summary>
Motivation: 现有基于碰撞锥或速度障碍约束的CBF方法过于保守，在密集场景中容易导致二次规划不可行，限制了非完整机器人在复杂动态环境中的安全导航能力。

Method: 设计动态抛物线控制屏障函数，其抛物线顶点和曲率根据障碍物距离和相对速度大小动态调整，形成限制更少的安全约束，并证明该方法适用于受输入约束的自行车运动学模型。

Result: 仿真实验表明，DPCBF控制器相比基线方法显著提高了导航成功率和QP可行性，能在包含100个动态障碍物的密集环境中成功导航，而基于碰撞锥的方法因不可行性而失败。

Conclusion: DPCBF通过动态调整安全边界，有效解决了传统CBF方法在密集动态环境中的保守性和不可行问题，为非完整机器人的安全导航提供了更实用的解决方案。
...
TL;DR: 本文研究扩散策略在机器人模仿学习中学习运动学约束的能力，通过双手抓取任务分析数据集大小、质量和流形曲率对约束学习的影响。


<details>
  <summary>Details</summary>
Motivation: 虽然扩散策略在机器人模仿学习中表现出色，但任务性能并不能可靠反映策略学习训练数据中约束的精确程度，需要研究扩散策略如何发现这些约束流形。

Method: 通过双手抓取任务案例研究，分析数据集大小、数据集质量和约束流形曲率三个因素对训练策略的影响。

Result: 扩散策略学习到约束流形的粗略近似，数据集大小和质量的降低对学习产生负面影响，而约束流形曲率与约束满足和任务成功的关系不明确。硬件评估验证了结果的现实适用性。

Conclusion: 扩散策略能够学习运动学约束，但学习效果受数据集特征影响，需要进一步研究约束流形特性对学习的影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于视觉的机器人学习依赖密集图像或点云输入的计算负担问题，以及现有基于关键点的方法依赖手动启发式或任务耦合选择，限制了可扩展性和语义理解。

Method: 采用三阶段流程：可操作性过滤、类别级关键点构建、基于transformer的策略学习与嵌入门控机制，推理最相关的关键点。

Result: 在多样化真实世界操作任务中，AFFORD2ACT持续提升数据效率，在未见过的物体、新类别、背景和干扰物上达到82%的成功率。

Conclusion: AFFORD2ACT框架能够从文本提示和单张图像中提取最小语义2D关键点集，构建紧凑高效的状态策略，在实时操作中表现良好，无需本体感知或密集表示。
...
Method: 使用可微分物理模拟精确建模颗粒动力学，采用低维技能空间参数化降低优化复杂度，并应用基于课程学习的策略在长时域内逐步提升任务能力。
...
TL;DR: 开发了一种用于乳腺癌保乳手术的协作机器人引导系统，通过触觉反馈帮助外科医生定位肿瘤边界，在模拟实验中证明能改善切除边缘并降低手术难度。


<details>
  <summary>Details</summary>
Motivation: 保乳手术中肿瘤边界定位困难，因为肿瘤通常高度移动、不可触及且形状不规则，需要一种能精确定位肿瘤边界的技术来改善手术效果。

Method: 将小型触觉机器人改装为电灼刀，结合超声和电磁导航识别肿瘤边界，当手术工具碰撞肿瘤边界时施加禁止区域虚拟约束，在模拟实验中比较有无触觉引导的切除效果。

Result: 虚拟约束引导改善了切除边缘，用户在使用触觉反馈时感觉任务更轻松、精神负担更小、挫败感更低，同时也发现了一些对手术流程的意外影响。

Conclusion: 虚拟约束有助于在模拟保乳手术中定位肿瘤边界，未来将进行更广泛的用户研究来进一步验证结果并优化引导系统。
...
TL;DR: VL-KnG是一个视觉场景理解系统，通过构建时空知识图谱来解决视觉语言模型在机器人导航中的局限性，实现了可解释的空间推理和实时应用。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在机器人导航中存在三个主要限制：缺乏持久场景记忆、空间推理能力有限、无法有效扩展视频时长以适应实时应用。

Method: 采用分块处理视频序列的方法，利用现代视觉语言模型构建持久知识图谱，维护对象在时间上的身份一致性，并通过可查询的图结构实现可解释的空间推理。

Result: 在真实世界差分驱动机器人上部署，实现了77.27%的成功率和76.92%的答案准确率，性能与Gemini 2.5 Pro相当，同时提供可解释的推理支持。
...
TL;DR: 本文展示了使用无迹卡尔曼滤波结合高斯过程残差学习的方法，对具有非驱动关节、链接安装推进器和单陀螺仪的仿生多连杆机器人进行位姿估计，证明了该方法的可靠性和跨步态泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究针对具有非驱动关节、最小感知平台的自由浮动仿生机器人，解决在非零均值、非高斯噪声环境下的位姿估计问题。

Method: 采用无迹卡尔曼滤波增强高斯过程残差学习来补偿非零均值、非高斯噪声，通过概念验证硬件实验和离线卡尔曼滤波分析进行验证。

Result: 实验表明机器人位姿可以可靠估计，在多步态数据集上训练的滤波器在仅前进步态测试轨迹上的表现与在更大前进步态数据集上训练的滤波器相当。

Conclusion: 这些结果揭示了步态输入空间的重叠性，可利用此特性减少训练数据需求，同时增强滤波器在多种步态间的泛化能力。
...
TL;DR: 提出了一种分层规划方法，通过稀疏图捕获全局连通性，结合基于神经场的局部规划器来解决复杂室内环境中的机器人导航问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在分辨率控制、可扩展性问题，或需要大量演示数据。ANTFields方法虽无需演示但受限于频谱偏差和灾难性遗忘问题。

Method: 分层结构：高层使用稀疏图表示全局连通性，低层使用基于神经场的规划器通过求解Eikonal PDE来导航局部障碍物。

Result: 在大规模环境中验证了框架的有效性，相比先前方法展现出更好的适应性和精度。

Conclusion: 该方法克服了频谱偏差和神经场拟合困难，实现了平滑精确的成本景观表示，具有在线探索、建图和实际导航的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有在线平面映射方法难以平衡精度和计算效率：直接深度图像分割缺乏时间整合，高度图方法无法表示复杂3D结构（如悬垂），体素基平面分割在实时应用中尚未探索。

Method: 开发了集成顶点基连通分量标记、随机采样一致性平面检测和凸包的新框架，利用GPU并行计算从高分辨率3D体素地图中快速提取平面区域。

Result: 实验结果表明，该方法即使在0.01米分辨率下也能以超过30Hz的更新率实现快速准确的3D多平面分割，使检测到的平面能实时用于运动任务。
...
TL;DR: 提出MiniBEE系统，使用两个低自由度手臂耦合形成紧凑的双手机器人末端执行器，支持穿戴式数据采集和标准机械臂部署，实现全工作空间的双手机动操作。


<details>
  <summary>Details</summary>
Motivation: 传统双手机器人使用两个完整6-7自由度手臂，系统复杂且仅利用部分工作空间进行灵巧交互。需要更紧凑、轻量化的解决方案。

Method: 设计两个低自由度手臂(3+自由度)耦合的紧凑系统，提出运动灵巧度指标指导设计，支持穿戴式运动数据采集和标准机械臂部署两种模式。

Result: 开发出MiniBEE系统，通过运动分析和设计优化最大化灵巧范围，实现穿戴式演示训练模仿学习策略，完成鲁棒的真实世界双手机动操作。

Conclusion: MiniBEE系统在保持完整相对定位能力的同时，显著降低了系统复杂性，扩展了双手机器人的应用范围和工作空间利用率。
...
TL;DR: ActiveUMI是一个数据收集框架，通过VR遥操作和传感器化控制器将人类演示转移到机器人上，实现复杂的双手操作任务。


<details>
  <summary>Details</summary>
Motivation: 开发便携式数据收集系统，将真实世界的人类演示转化为机器人可学习的策略，解决复杂双手操作任务的数据收集难题。

Method: 结合便携式VR遥操作套件和传感器化控制器，通过精确姿态对齐桥接人机运动学，采用沉浸式3D模型渲染、自包含可穿戴计算机和高效校准方法。

Result: 在6个挑战性双手任务上，仅使用ActiveUMI数据训练的策略达到70%的平均成功率，在新物体和新环境中保持56%的成功率，表现出强泛化能力。

Conclusion: 便携式数据收集系统结合学习到的主动感知，为创建可泛化且高能力的真实世界机器人策略提供了有效且可扩展的途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人操作数据集主要提供地面真实轨迹，无法处理执行过程中的故障；少数涉及故障检测的数据集仅提供文本解释，难以直接用于VLA模型。需要解决机器人遇到不可预测和突然故障时的推理和恢复问题。

Method: 开发FailSafe系统，可无缝应用于任何模拟器中的操作任务，规模化生成故障-动作数据；通过微调LLaVa-OneVision-7B构建FailSafe-VLM。

Result: FailSafe-VLM成功帮助机械臂检测和恢复潜在故障，在Maniskill多个任务中平均提升三个最先进VLA模型(pi0-FAST、OpenVLA、OpenVLA-OFT)性能达22.6%；并能泛化到不同空间配置、相机视角和机器人形态。

Conclusion: FailSafe系统有效解决了VLA模型在机器人操作中的故障恢复问题，显著提升了模型性能，具有良好的泛化能力，计划向社区发布代码。
...
TL;DR: 提出了一种在线学习测量可靠性的统计框架，通过多视角几何一致性作为自监督，动态评估视觉惯性里程计中传感器测量的可靠性，相比固定不确定性参数的方法显著提升了跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 传统方法假设所有测量具有静态、均匀的不确定性，这无法捕捉真实世界数据中动态变化的误差特性，限制了鲁棒视觉惯性里程计的性能。

Method: 利用多视角几何一致性作为自监督，从传感器数据和优化结果中在线学习测量可靠性评估，推断地标不确定性并在优化过程中自适应加权视觉测量。

Result: 在EuRoC数据集上的评估显示，相比固定不确定性参数的基线方法，平均平移误差减少约24%，旋转误差减少约42%，框架能够实时运行且具有更好的准确性和鲁棒性。

Conclusion: 提出的统计框架能够有效在线学习测量可靠性，显著提升视觉惯性里程计的性能，同时保持实时运行能力，代码将开源以促进进一步研究。
...
TL;DR: SymSkill是一个统一学习框架，结合模仿学习和任务运动规划的优势，实现组合泛化和实时故障恢复。
...
Method: 从无标签、无分割的演示中联合学习谓词、操作符和技能，使用符号规划器组合和重排序技能，在运动层和符号层进行实时恢复。

Result: 在RoboCasa模拟中执行12个单步任务成功率85%，无需额外数据即可组合成最多6次技能重组的复杂任务；在真实机器人上从5分钟无标签游戏数据学习后能执行多种任务。
...
TL;DR: 提出了一种几何反步控制器，用于可变倾斜全向多旋翼飞行器，该控制器明确考虑了伺服和转子动力学，相比不考虑执行器动态的基线方法，在快速平移跟踪、快速旋转跟踪和扰动恢复实验中表现出更好的跟踪性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有执行器感知控制方法依赖于执行器输入与力矩之间的线性关系，无法捕捉可变倾斜角度引起的非线性效应，这在激进飞行或突然扰动恢复时会影响控制效果和可靠性。

Method: 利用多旋翼刚体动力学与非线性执行器动力学之间的级联结构，设计几何反步控制器，并建立整个系统的指数稳定性。

Result: 在三个实验场景中，相比不考虑执行器动态的基线方法，所提控制器始终获得更好的跟踪性能，且在最快平移轨迹跟踪和恢复实验中，基线方法发散并坠毁，而所提控制器保持稳定并成功完成任务。

Conclusion: 所提出的控制器能够有效处理执行器非线性动态，对参数不确定性具有鲁棒性，在激进飞行和扰动恢复场景中显著优于不考虑执行器动态的传统方法。
...
TL;DR: 提出了PolySim平台，通过在多仿真器中联合训练人形机器人全身控制策略，减少仿真器归纳偏差，实现零样本真实世界部署。


<details>
  <summary>Details</summary>
Motivation: 解决仿真到真实世界的差距问题，该差距源于单个仿真器的归纳偏差（假设和限制），导致跨仿真器和仿真到真实世界的重要差异。

Method: 开发PolySim平台，在单个训练运行中同时启动来自不同引擎的并行环境，实现动态级领域随机化，联合训练策略以捕获超越任何单个仿真器假设的通用动态。

Result: 在仿真到仿真评估中显著减少运动跟踪误差，在MuJoCo上相比IsaacSim基线提高执行成功率52.8%；在真实Unitree G1机器人上实现零样本部署，无需额外微调。
...
TL;DR: 本文提出了一种机器人状态感知对比损失（RS-CL），用于改善视觉-语言-动作（VLA）模型在机器人操作任务中的表示学习，通过将表示与机器人本体感知状态对齐来增强控制相关表示。
...
Method: 引入RS-CL作为表示正则化方法，使用机器人状态之间的相对距离作为软监督，将VLM表示与机器人本体感知状态对齐，同时保持与标准VLA训练流程的兼容性。

Result: 在RoboCasa-Kitchen的拾放任务中，将最先进VLA模型的性能从30.8%提升到41.5%；在真实机器人操作任务中，成功率从45.0%提升到58.3%。

Conclusion: RS-CL是一种简单有效的表示正则化方法，能够显著提升VLA模型在机器人操作任务中的性能，通过更准确的抓取和放置定位实现改进。
...
TL;DR: 提出了一种新型磁性连续体机器人设计，通过在导管壁内径向嵌入永磁体，实现单个外部永磁体独立控制弯曲或扭转运动，并开发了基于扭转剪切的双层阻塞机制实现按需药物释放。


<details>
  <summary>Details</summary>
Motivation: 现有的轴向磁化磁性连续体机器人主要限于弯曲运动，需要扩展变形能力以实现更复杂的医疗干预。

Method: 采用径向嵌入永磁体的简单组装设计，结合基于物理的建模和有限元分析建立驱动原理，开发了利用扭转剪切实现药物释放的双层阻塞机制。

Result: 实验验证了在实用磁场下的解耦模式控制，并在体模干预实验中展示了从管腔跟随、弯曲接近目标到扭转激活释放的端到端操作。

Conclusion: 该紧凑、无缆平台结合了多功能变形与精确有效载荷输送，显示出在下一代定点治疗中的巨大潜力。
...
TL;DR: 提出了TS-ACES，第一个可扩展到包含数百台机器的智能工厂的可扩展SFE解决方案
...
Method: 开发了TS-ACES（基于交通系统的任意时间循环嵌入求解器），这是一种完整的求解器

Result: TS-ACES能够扩展到基于真实工业场景、包含超过100台机器的SFE实例

Conclusion: TS-ACES填补了智能工厂嵌入问题的可扩展性空白，是第一个高度可扩展的SFE解决方案
...
TL;DR: Nav-EE是一个导航引导的早期退出框架，通过预计算任务特定的退出层并基于导航先验动态应用，在自动驾驶中显著降低视觉语言模型的推理延迟


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶中应用广泛，但高推理延迟阻碍实时部署。早期退出方法虽然能减少延迟，但其任务依赖性限制了在不同场景中的泛化能力

Method: 利用导航系统预测即将到来的上下文（如交叉口、交通灯），离线预计算任务特定的退出层，在线基于导航先验动态应用这些退出层

Result: 在CODA、Waymo和BOSCH数据集上，Nav-EE实现了与完整推理相当的准确度，同时将延迟降低高达63.9%。真实车辆集成显示推理延迟从600ms降至300ms

Conclusion: 将导航预见性与早期退出相结合，为在自主系统中高效部署大型模型提供了可行路径
...
TL;DR: 本文对模块化强化学习的ObjectNav系统进行了大规模实证研究，分解为感知、策略和测试时增强三个关键组件，发现感知质量和测试时策略是性能的决定性因素，而策略改进仅带来边际收益。


<details>
  <summary>Details</summary>
Motivation: ObjectNav是移动机器人在日常环境中部署的关键组件，但现有方法缺乏统一分析来确定真正驱动性能的组件。需要系统研究不同设计选择对性能的影响。

Method: 通过大规模受控实验，将模块化RL-based ObjectNav系统分解为感知、策略和测试时增强三个组件，分别隔离每个组件的贡献。

Result: 感知质量和测试时策略是性能的决定性驱动因素，策略改进仅带来边际增益。基于这些洞察构建的增强模块化系统在SPL上超过SotA方法6.6%，成功率提高2.7%。

Conclusion: 研究不仅设定了SotA性能，还为未来ObjectNav开发和评估提供了原则性指导，同时揭示了RL智能体与人类专家导航（98%成功率）之间的差距。
...
TL;DR: 该研究将无人机中成功的时空轨迹规划方法创新性地应用于双足机器人系统，解决了人形机器人足球中踢球动作的轨迹规划问题。


<details>
  <summary>Details</summary>
Motivation: 当前人形机器人足球中，无论是传统位置控制方法还是强化学习方法都存在显著局限性。MPC方法虽然广泛用于腿式机器人，但现有研究往往过度简化腿部摆动过程，仅依赖简单轨迹插值方法，严重限制了脚部环境交互能力，阻碍了踢球等任务。

Method: 创新性地将无人机应用中成功的时空轨迹规划方法适配到双足机器人系统，提出的方法能够自主生成满足目标踢球位置、速度和加速度约束的足部轨迹，同时优化摆动阶段持续时间。

Result: 实验结果表明优化的轨迹能够紧密模仿人类踢球行为，具有后摆动作特征。仿真和硬件实验证实了算法的高效性（轨迹规划时间小于1毫秒）和可靠性（当足球目标在-90度到90度范围内时，任务完成准确率接近100%）。

Conclusion: 该研究成功地将时空轨迹规划方法应用于双足机器人踢球任务，实现了高效可靠的轨迹规划，为人形机器人足球中的精确踢球控制提供了有效解决方案。
...
TL;DR: GreenhouseSplat：一个从廉价RGB图像生成逼真温室资产的框架和数据集，支持ROS仿真，用于农业机器人评估


<details>
  <summary>Details</summary>
Motivation: 现有温室环境模拟方法依赖简单或合成资产，限制了仿真到现实的迁移能力，需要更逼真的温室规模仿真

Method: 基于辐射场方法（如高斯泼溅），从廉价RGB图像直接生成逼真温室资产，集成到支持相机和LiDAR渲染的ROS仿真中

Result: 提供了包含82棵黄瓜植物在多种行配置下的数据集，展示了在机器人定位等任务中的实用性

Conclusion: 这是迈向温室规模辐射场仿真的第一步，为农业机器人研究提供了基础
...
TL;DR: TACOS是一个基于大语言模型的多无人机系统统一框架，通过自然语言接口实现高层任务控制，集成了语言接口、智能协调器和自主代理三大功能模块。


<details>
  <summary>Details</summary>
Motivation: 单飞行员管理多无人机系统需要不同层级的自主性，从直接控制到群体协调再到完全自主的群体行为。需要支持多种共享自主模式的框架来减轻飞行员工作负担。

Method: TACOS框架整合了三个关键能力：一对多自然语言接口、将用户意图转化为结构化任务计划的智能协调器、以及与真实世界交互执行计划的自主代理。LLM与可执行API库交互，连接语义推理与实时多机器人协调。

Result: 在真实多无人机系统中演示了系统性能，并通过消融研究评估了各模块的贡献。

Conclusion: TACOS提供了一个统一的框架，通过大语言模型实现多无人机系统的高层自然语言控制，有效减轻了飞行员工作负担。
...
TL;DR: SPARC是一个紧凑的开源3自由度脊柱模块，集成了旋转和直线运动，具有可编程任务空间阻抗，用于四足机器人。
...
Method: 开发了基于RNEA的计算加速度控制器，结合平滑Stribeck摩擦补偿，实现弹簧-阻尼行为，无需显式惯性整形。

Result: 实验验证了线性力-位移特性，水平刚度范围300-700 N/m，相对误差≤1.5%；动态测试确认了质量-弹簧-阻尼响应。

Conclusion: SPARC为腿部运动中脊柱柔顺性的系统研究提供了可靠平台，将发布完整的硬件和固件资源。
...
TL;DR: 提出一种基于模型预测控制的运动提示算法，通过惩罚感官冲突和比力误差来联合优化驾驶模拟器的保真度和舒适度，显著减少晕动症。


<details>
  <summary>Details</summary>
Motivation: 驾驶模拟器在研究和开发中应用日益广泛，但由于运动缩放和视觉不匹配，经常导致晕动症。需要开发既能保持运动保真度又能减少晕动症的算法。

Method: 使用模型预测控制(MPC)开发运动提示算法，在成本函数中同时惩罚感官冲突和比力误差。进行人机回路实验比较四种运动设置：两种MPC算法变体、自适应清洗算法和无运动情况。

Result: 实验结果显示，折衷方案相比自适应清洗和纯比力跟踪算法，晕动症减少超过50%（平均MISC水平从3降至1.5），且保真度评级没有显著降低。无运动条件晕动症最低但保真度最差。

Conclusion: 提出的方法综合考虑模拟器动力学和晕动症时间演化，在实现晕动症控制和比力重现的最优平衡方面取得显著进展，支持模拟器的更广泛应用。
...
TL;DR: EC3R-SLAM是一种无需相机标定的单目稠密SLAM框架，通过结合跟踪模块和基于前馈3D重建的映射模块，实现了高精度定位与建图、低延迟和低GPU内存消耗，并在资源受限平台上有效运行。


<details>
  <summary>Details</summary>
Motivation: 传统单目稠密SLAM存在高延迟、大GPU内存消耗和依赖相机标定的问题，限制了其实际应用。

Method: 结合跟踪模块（维护稀疏特征点地图）和基于前馈3D重建的映射模块（同时估计相机内参），并融入局部和全局闭环检测以确保多视角一致性。

Result: 在多个基准测试中达到与最先进方法竞争的性能，同时更快、更节省内存，能在笔记本电脑和Jetson Orin NX等资源受限平台上有效运行。

Conclusion: EC3R-SLAM展示了在现实世界机器人应用中实现高效、鲁棒SLAM的潜力，特别是在资源受限环境下。
...
TL;DR: LangGrasp是一个语言交互的机器人抓取框架，通过微调大语言模型处理包含隐含意图的模糊指令，实现从物体级到部件级的精细抓取操作。
...
Method: 集成微调的大语言模型进行常识推理和环境感知，结合基于2D部件分割的点云定位模块，实现从物体级到部件级的精细抓取。

Result: LangGrasp能准确解析模糊指令中的隐含意图，识别未明确表述的关键操作和目标信息，动态选择最优抓取姿态，显著提升抓取精度和任务执行效率。

Conclusion: 该框架显著增强了机器人在非结构化环境中的适应性和任务执行能力，实现了从粗粒度物体级到细粒度部件级的精确抓取操作。
...
TL;DR: 本文介绍了NAO机器人站立动作的改进方法，通过解决关节位置误差问题显著提高了站立成功率，该方法已被多个机器人足球团队采用。


<details>
  <summary>Details</summary>
Motivation: 在类人机器人足球比赛中，机器人无法自主站立会被罚离场一段时间，因此可靠的站立动作至关重要。

Method: 通过执行特殊动作释放卡住的肢体（如手臂），或使用其他关节补偿大误差，来解决关节位置误差问题。

Result: 显著提高了站立动作的整体成功率，该方法在多个团队中应用并达到相似的成功率。

Conclusion: 解决关节位置误差是提高机器人站立成功率的关键，该方法经过多年验证具有可靠性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种多功能的光学感知软体机器人平台，扩展软体机器人的光谱感知能力，探索光学作为软体机器人多功能感知模式的潜力。

Method: 通过材料分析确定最佳光谱感知软基底，评估预接触和手持性能，使用线性判别分析识别关键光谱特征。

Result: 实验显示系统能够对不同类别和尺寸的物体（金属、木材、塑料、有机物、纸张、泡沫）进行可解释的统计分离，近红外波长对区分视觉相似物体至关重要。
...
TL;DR: 本文提出使用数字孪生技术优化电动汽车电池的拆解过程，通过扩展产品-过程-资源资产网络(PAN)为双向PAN(Bi-PAN)，覆盖制造和再制造/回收两个阶段，以提高可持续性。


<details>
  <summary>Details</summary>
Motivation: 在循环经济背景下，产品生命周期结束时应进行再制造或回收，但制造商往往不共享相关数据，阻碍了这些可持续过程的优化。

Method: 采用数字孪生技术，基于扩展的产品-过程-资源资产网络(Bi-PAN)范式，建模产品、生产资源、制造过程和具体操作之间的关系，覆盖制造和再制造/回收两个阶段。

Result: 通过电动汽车电池拆解用例证明，产品数字孪生能够灵活高效地解决不同类型电池的拆解挑战，减少生态影响。
...
TL;DR: DisCo-Layout是一个解耦物理和语义精炼的3D室内布局合成框架，通过独立和协作的精炼工具生成真实、连贯且可泛化的布局。


<details>
  <summary>Details</summary>
Motivation: 传统方法因固定数据集而泛化能力差，现有LLM和VLM方法语义丰富但缺乏灵活精炼，导致布局不理想。

Method: 开发解耦物理和语义精炼的框架：语义精炼工具修正抽象对象关系，物理精炼工具通过网格匹配算法解决空间问题；多智能体框架协调这些工具，包括规划器、设计器和评估器。

Result: 实验证明DisCo-Layout达到最先进性能，生成真实、连贯且可泛化的3D室内布局。

Conclusion: DisCo-Layout通过解耦和协调物理与语义精炼，有效解决了3D室内布局合成的挑战，代码将公开。
...
TL;DR: 提出了FalconGym 2.0仿真框架和性能引导精炼算法，训练出的视觉策略在泛化性和鲁棒性方面优于现有方法，并能实现零样本从仿真到现实的迁移。


<details>
  <summary>Details</summary>
Motivation: 现有视觉策略在航迹几何变化时性能下降，存在过拟合问题，需要提高策略的泛化能力和鲁棒性。

Method: 开发了基于高斯泼溅的FalconGym 2.0仿真框架，提出性能引导精炼算法，在具有挑战性的航迹上集中训练视觉策略。

Result: 在两种无人机案例中，单一视觉策略在三个未见航迹上实现100%成功率，在门姿态扰动下保持更高成功率，并在真实四旋翼上实现98.6%的成功率。

Conclusion: FalconGym 2.0和PGR算法能有效训练出具有强泛化性和鲁棒性的视觉策略，并能成功迁移到真实硬件。
...
TL;DR: 本文提出了一种新的运动重定向方法GMR，解决了人类运动数据重定向到人形机器人时产生的伪影问题，通过系统评估重定向质量对策略性能的影响，在减少奖励工程的情况下显著提升了跟踪性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前人类运动跟踪策略面临人机体现差距的挑战，现有方法在重定向过程中会产生脚滑、自穿透和物理不可行运动等伪影，这些伪影被保留在参考轨迹中，需要强化学习策略来纠正，但需要大量奖励工程和领域随机化。

Method: 提出通用运动重定向(GMR)方法，与PHC、ProtoMotions等开源重定向器以及Unitree的闭源数据集进行比较，使用BeyondMimic进行策略训练以隔离重定向效果而不依赖奖励调优。

Result: 实验表明，大多数运动可以被跟踪，但重定向数据中的伪影显著降低了策略鲁棒性，特别是在动态或长序列中。GMR在跟踪性能和运动保真度方面持续优于现有开源方法，达到了接近闭源基线的感知保真度和策略成功率。

Conclusion: 重定向质量对运动跟踪策略性能有显著影响，GMR方法能够有效减少重定向伪影，提升策略的鲁棒性和性能，为人形机器人运动控制提供了更可靠的解决方案。
...
TL;DR: 该论文研究了通过显式将策略与相机外参条件化来实现视角不变模仿学习，使用Plucker嵌入表示像素光线，在多种行为克隆策略中显著提升了跨视角泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习策略在固定场景中可能通过静态背景线索推断相机姿态，但当工作空间几何或相机位置变化时这种捷径会失效，需要开发对相机外参具有鲁棒性的方法。

Method: 使用Plucker嵌入表示像素光线，将相机外参显式条件化到策略中，在RoboSuite和ManiSkill中构建了6个操作任务，包含固定和随机化场景变体以分离背景线索和相机姿态。

Result: 实验表明，不包含外参的策略在固定场景中依赖背景线索推断相机姿态，但在场景变化时性能崩溃；而条件化外参的方法恢复了性能，实现了无需深度的鲁棒RGB控制。

Conclusion: 显式条件化相机外参能够显著提升模仿学习策略的视角不变性和鲁棒性，为RGB-only控制提供了有效解决方案。
...
TL;DR: ARMADA是一个多机器人部署和适应系统，通过FLOAT自主故障检测方法实现人机共享控制，仅在必要时请求人工干预，显著减少对人监督的依赖，提高部署效率和场景适应速度。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在大规模真实世界数据中表现良好，但预训练策略在缺乏领域数据时性能较差。人工收集演示数据成本高且质量不一，现有的人机协作系统需要全程人工监督，限制了可扩展性。

Method: 开发ARMADA系统，包含FLOAT自主在线故障检测方法，实现并行策略部署，仅在必要时请求人工干预，通过人机共享控制获取领域特定数据。

Result: FLOAT故障检测准确率平均达到95%，比现有方法提高20%以上。ARMADA在多次策略部署和后训练中，成功率提高4倍以上，人工干预率降低2倍以上。

Conclusion: ARMADA系统通过高效的故障检测和人机协作机制，显著减少了人工监督需求，实现了更可扩展的部署和更快的场景适应能力。
...
TL;DR: 将分子动力学模拟重新定义为时间序列预测问题，通过位移预测而非绝对位置来预测原子轨迹，结合物理约束的损失函数确保物理合理性，显著提升模拟效率。


<details>
  <summary>Details</summary>
Motivation: 传统密度泛函理论方法计算成本高昂，限制了长期分子动力学模拟的可行性，需要开发更高效的模拟方法。

Method: 将MD模拟建模为时间序列预测问题，使用基于DFT参数化的Morse势函数的物理约束损失函数，通过位移预测而非绝对位置来预测原子轨迹。

Result: 在多种材料上持续超越标准基线方法的模拟精度，能够在几分钟内稳定模拟数千个MD步骤，为昂贵的DFT模拟提供了可扩展的替代方案。

Conclusion: 结果表明，结合物理知识对于提高原子轨迹预测的可靠性和精度至关重要，该方法为高效分子动力学模拟提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统高温采样方法在需要高精度的任务中会降低推理质量，因为会在敏感解码位置采样到错误的延续。

Method: 训练轻量级分类器预测采样风险，根据风险指标动态选择贪婪采样或高温采样，最小化延迟开销。

Result: 在数学推理任务上的实验表明，选择性采样在高温设置下也能改善质量-多样性权衡。

Conclusion: 选择性采样方法有效解决了高温采样在精度敏感任务中的准确性问题，实现了更好的质量-多样性平衡。
...
TL;DR: 开发了一个基于大语言模型的自动化工作流，从10,000篇科学论文中提取热电和结构性能数据，创建了包含27,822条记录的大型数据集，并提供了可复现的数据提取流程。


<details>
  <summary>Details</summary>
Motivation: 现有材料数据库规模小、手动整理或偏向第一性原理计算，实验文献数据未被充分利用，制约了材料发现速度。

Method: 采用动态令牌分配、零样本多智能体提取和条件表解析的LLM驱动工作流，平衡准确性与计算成本。

Result: GPT-4.1达到最高准确率（热电性能F1=0.91，结构字段F1=0.82），GPT-4.1 Mini以更低成本提供相近性能。构建了包含ZT、塞贝克系数等热电性能和晶体结构属性的数据集。

Conclusion: 提供了迄今最大的LLM整理热电数据集，建立了可扩展的数据驱动材料发现基础，超越了热电领域。
...
Result: 在LLaMA-3 8B的2比特量化中，RSAVQ在困惑度上比VPTQ和QuIP#等基线方法领先0.4，在零样本准确率上领先1.5。
...
Result: 在CIFAR-10上验证了方法的有效性：准确率稳步提升；客户端重叠增加能改善准确率和降低攻击成功率；信号预算减少时准确率稳定但攻击成功率上升。

Conclusion: 深度Q网络在鲁棒性-准确性权衡方面表现最佳，优于随机、线性Q和策略梯度控制器。
...
TL;DR: 提出RSTGCN模型预测铁路车站平均到达延误，包含空间注意力机制创新，并在印度铁路网络上构建了大规模数据集。


<details>
  <summary>Details</summary>
Motivation: 准确的列车延误预测对铁路运营效率至关重要，现有方法多关注单个列车延误预测，需要支持更高层次交通管理的车站级延误预测。

Method: 设计铁路中心化时空图卷积网络(RSTGCN)，整合列车频率感知的空间注意力机制等架构创新和特征集成。

Result: 在印度铁路网络4735个车站上进行广泛实验，相比现有基准方法在标准指标上取得一致改进。
...
<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法基于参数对损失的影响（如大小或梯度）进行剪枝，但缺乏对网络表示多样性和效率的系统性考虑。

Method: BB方法为每个单元分配本地流量预算，通过约束熵分析推导出选择性-受众平衡公式，并使用简单的本地执行器剪枝扇入（降低活动率）或扇出（减少广播）。

Result: 在ASR的Transformer、人脸识别的ResNet和突触预测的3D U-Net上，BB在相同稀疏度下提高了编码熵、去相关性和准确率，有时甚至超过密集基线。在电子显微镜图像上达到了最先进的F1和PR-AUC。
...
TL;DR: 扩展IsaacLab框架以支持在高保真物理模拟中进行对抗性多智能体强化学习训练，引入具有异构目标和能力智能体的对抗性MARL环境套件，并集成竞争性HAPPO算法。


<details>
  <summary>Details</summary>
Motivation: 虽然现有MARL研究主要关注协作场景，但对抗性交互在追逃、安全和竞争性操作等现实应用中同样关键，需要支持异构智能体间的不对称竞争。

Method: 扩展IsaacLab框架，开发对抗性MARL环境套件，集成竞争性异构智能体近端策略优化(HAPPO)算法进行高效训练。

Result: 实验表明该框架能够在保持高吞吐量和仿真真实性的同时，有效建模和训练形态多样的多智能体竞争策略。

Conclusion: 该工作为对抗性多智能体强化学习提供了可扩展的高保真仿真平台，支持异构智能体间的复杂竞争行为训练。
...
TL;DR: RLP是一种在预训练阶段引入强化学习思想的创新方法，通过将思维链视为探索性行动，基于信息增益计算奖励，让模型在预测前学会独立思考。


<details>
  <summary>Details</summary>
Motivation: 传统方法只在最后阶段使用强化学习进行后训练，而RLP旨在将强化学习的探索精神更早地引入预训练阶段，以优化大型推理模型的训练方式。

Method: 将思维链作为探索性行动，奖励信号基于思维链带来的信息增益（下一个token对数似然的提升），在预训练阶段使用密集奖励信号进行训练。

Result: 在Qwen3-1.7B-Base上，RLP将8个数学和科学基准测试的平均性能提升19%；在Nemotron-Nano-12B-v2上，整体平均从42.81%提升到61.32%，科学推理任务提升23%。

Conclusion: RLP成功将强化学习重新定位为预训练目标，弥合了下一个token预测与有用思维链推理之间的差距，在不同架构和模型规模上都显示出良好的可扩展性。
...
TL;DR: 提出了一种结合LQR和强化学习的混合控制框架，用于结构振动控制，解决RL控制器训练期间的安全风险问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于模型的LQR控制需要繁琐的系统辨识，而纯RL控制训练期间可能对结构造成损害。研究发现即使基于错误模型的LQR控制器也能优于无控制情况，因此提出混合框架。

Method: 使用基于随机选择模型的LQR控制器来指导RL控制器训练，LQR策略不依赖真实或近似结构模型，保持整体框架的无模型特性。

Result: 该混合方法消除了对显式系统模型的依赖，同时最小化了朴素RL实现中的探索风险。

Conclusion: 这是首个解决RL振动控制训练安全挑战并提供验证解决方案的研究，混合框架在保持无模型优势的同时确保了训练安全性。
...
TL;DR: 提出了一种基于信息论的方法来识别和分析RNN中的信息中继节点，通过量化节点间输入输出的互信息来追踪信息流动路径，并在不同RNN架构上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 理解RNN内部动态对于提高其可解释性和改进设计至关重要，需要开发能够识别关键信息传输节点的方法。

Method: 使用信息论方法量化节点间输入输出向量的互信息，识别信息中继节点，并在合成和真实时间序列分类任务中应用，包括LSTM和GRU等架构，还进行了节点敲除实验。

Result: 揭示了不同架构中信息中继的独特模式，提供了信息如何随时间处理和维持的见解，节点敲除实验证实了识别节点的重要性。
...
TL;DR: NcPU是一个非对比式PU学习框架，通过NoiSNCL损失函数和PLD标签消歧方案，在不依赖辅助信息的情况下解决了PU学习中表示学习的挑战，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有PU学习方法在复杂数据集上表现远不如监督学习方法（如CIFAR-100上有14.26%差距），主要瓶颈是在不可靠监督下学习判别性表示的挑战。

Method: 提出NcPU框架：1）NoiSNCL损失函数，在不可靠监督下对齐类内表示；2）PLD方案，通过基于遗憾的标签更新提供保守的负监督。两者在EM框架下可以迭代互益。

Result: 实验表明：1）NoiSNCL使简单PU方法达到竞争性性能；2）NcPU在多个数据集上显著优于最先进的PU方法，包括灾害后建筑损伤映射等实际应用场景。

Conclusion: NcPU有效解决了PU学习中的表示学习瓶颈，在不依赖辅助信息的情况下实现了显著性能提升，具有实际应用价值。
...
TL;DR: 受微眼动启发，提出通过轻量级位置编码扰动来探测大语言模型潜在错误行为的方法，无需微调即可检测事实性、安全性、毒性等多种问题。


<details>
  <summary>Details</summary>
Motivation: 从人类视觉感知中的微眼动现象获得灵感，微眼动能揭示隐藏的感知动态，类似地希望找到能揭示LLM潜在错误行为的探测方法。

Method: 使用轻量级位置编码扰动作为探测手段，无需微调或任务特定监督，通过分析模型对这些扰动的响应来检测错误行为。

Result: 在多个先进LLM上的实验表明，该方法能有效检测事实性、安全性、毒性及后门攻击等各类错误行为，且计算效率高。

Conclusion: 预训练的LLM已编码了标记自身错误所需的内在证据，微眼动启发的干预为检测和缓解不良行为提供了有效途径。
...
TL;DR: ThinKV是一个思想自适应的KV缓存压缩框架，通过混合量化-驱逐策略显著减少大模型推理时的KV缓存占用，在保持接近无损精度的同时提升推理吞吐量。
...
Method: 基于注意力稀疏性识别不同重要性的思想类型，采用混合量化-驱逐策略，按思想重要性分配token精度，并随着推理轨迹演化逐步驱逐不太重要的思想token。

Result: 在数学和编程基准测试中，ThinKV仅使用不到5%的原始KV缓存就实现了接近无损的精度，推理吞吐量比最先进基线提升高达5.8倍。

Conclusion: ThinKV通过思想自适应压缩有效解决了KV缓存内存瓶颈问题，为大模型高效推理提供了实用解决方案。
...
TL;DR: 提出了一种基于域适应的车辆延误估计框架GBBW，通过重新加权源域数据来提高模型在不同交叉口的泛化能力，在57个异质交叉口数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型假设训练和测试数据分布相同，但现实中交叉口在道路几何、信号配时和驾驶行为等方面存在差异，导致模型泛化能力差。需要解决域偏移问题来提高延误估计的准确性。

Method: 采用域适应框架，将数据分为源域和目标域，提取关键交通特征，使用目标域的小规模标注子集进行微调。提出GBBW模型，基于与目标域的相似性重新加权源域数据。

Result: 在亚利桑那州Pima County的57个异质交叉口数据集上测试，与8种先进ML回归模型和7种基于实例的域适应方法相比，GBBW框架提供了更准确和鲁棒的延误估计。
...
TL;DR: 这篇综述系统回顾了基于深度学习的2D MRI到3D形状重建方法，涵盖点云、网格、形状感知和体积模型四类主要方法，分析了技术现状、局限性和临床应用。


<details>
  <summary>Details</summary>
Motivation: 3D形状重建在医学疾病诊断、治疗规划和计算建模中日益重要，需要系统梳理当前方法现状以推动更鲁棒、可泛化和临床有效的解决方案。

Method: 采用文献综述方法，分析四类主要重建方法：点云模型、基于网格的方法、形状感知模型和体积模型，评估其技术基础、局限性和在不同解剖结构中的应用。

Result: 提供了从心脏到神经再到肺部成像的广泛概述，分析了模型在病变解剖中的临床适用性，以及训练测试数据的影响，考察了公开数据集、计算需求和评估指标。

Conclusion: 指出了新兴研究方向包括多模态集成和跨模态框架，旨在为研究人员提供结构化概览，以识别推进深度学习在医学3D重建中发展的机会。
...
TL;DR: 该论文研究了两层神经网络训练损失梯度的低秩结构，放宽了数据各向同性和参数独立性的假设，发现在各向异性和病态数据条件下，梯度仍近似低秩且由两个秩一分量主导。


<details>
  <summary>Details</summary>
Motivation: 研究在更真实的数据条件下（非各向同性、病态、非独立数据）神经网络梯度的结构特性，以理解训练动态和正则化机制。

Method: 采用尖峰数据模型，分析均值场和神经正切核两种缩放机制，理论推导梯度结构并验证正则化效果。

Result: 梯度近似低秩，由两个秩一分量主导：一个与数据残差对齐，另一个与输入数据中的尖峰对齐；数据特性、缩放机制和激活函数影响这两个分量的平衡。

Conclusion: 神经网络梯度在复杂数据条件下仍保持低秩结构，标准正则化方法能选择性地调节不同梯度分量，理论预测得到实验验证。
...
TL;DR: 提出了一个量子启发的内在维度估计基准QuIIEst，包含无限个已知ID的拓扑非平凡流形家族，用于评估IDE方法在复杂流形上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有IDE方法在真实数据集上估计结果差异很大，需要更复杂的基准来评估这些方法。流形假说认为数据集位于低内在维度的潜在流形上，但现有基准流形过于简单。

Method: 基于量子光学方法构建QuIIEst基准，能够嵌入任意齐次空间并允许曲率修改和添加噪声。测试了多种IDE方法在该基准上的表现。

Result: IDE方法在QuIIEst流形上的准确性普遍低于现有基准，且随着曲率不均匀性增加性能下降很小，表明基准本身具有较高难度。在分形Hofstadter蝴蝶上测试了IDE方法提取非流形空间有效维度的能力。

Conclusion: QuIIEst基准为IDE方法提供了更具挑战性的测试平台，揭示了现有方法在复杂流形上的局限性，并为非流形空间的有效维度估计提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 研究从视频数据中发现动作表示的潜在动作策略学习框架的可识别性问题，理解离散动作表示在实践中成功的原因。

Method: 形式化描述动作表示的期望特性，分析统计优势和潜在不可识别性来源，证明熵正则化LAPO目标在适当条件下能够识别满足期望的动作表示。

Result: 证明了在合适条件下，熵正则化的LAPO目标能够识别出满足期望特性的动作表示，为离散动作表示的实际表现提供了理论解释。

Conclusion: 熵正则化的LAPO框架在适当条件下能够识别出满足期望特性的动作表示，这解释了为什么离散动作表示在实践中表现良好。
...
Method: 从变分互信息下界出发，推导出两种训练范式：SDMI需要交替优化，必须使用停止梯度操作；JMI允许通过对称架构进行联合优化。预测器网络和统计正则化器分别作为两种范式中互信息目标的可行替代方案。

Result: 证明了现有许多自监督表示学习方法都是这两种范式的具体实例或近似，为不同架构组件的选择提供了理论解释。

Conclusion: 该研究为自监督表示学习方法的架构设计提供了理论依据，表明学习目标决定了可能的优化策略和模型设计选择，超越了经验性便利的层面。
...
TL;DR: 提出了一种量化数据集各向异性程度的度量方法，通过双样本神经网络分类器测试来区分原始数据集与其随机增强版本，揭示了基准点云数据集中存在的高度对齐现象。


<details>
  <summary>Details</summary>
Motivation: 对称感知方法（如数据增强和等变架构）假设变换后的数据点在测试分布中具有高概率或"重要性"，但需要评估这一假设的有效性。

Method: 使用双样本神经网络分类器测试来量化数据集各向异性程度，区分原始数据集和随机增强版本，并在合成数据集和基准点云数据集上进行验证。

Result: 在多个基准点云数据集中发现了出乎意料的高度对齐现象；理论上证明分布对称性破坏会阻止不变方法达到最优性能；经验发现等变方法的效益具有数据集依赖性。

Conclusion: 理解等变性（包括其何时有效及原因）可能需要重新思考数据中的对称性偏差，对称感知方法的有效性取决于具体数据集的特征。
...
TL;DR: 提出了RheOFormer，一种基于自注意力机制的生成算子学习方法，用于高效学习复杂流体流动的空间相互作用和特征，准确预测不同复杂流体的非线性力学行为。
...
Method: 使用自注意力机制的生成算子学习方法（RheOFormer），通过Transformer架构学习复杂流体流动的空间相互作用和特征。

Result: RheOFormer能够准确学习不同复杂流体的标量和张量非线性力学，预测其流动的时空演化，即使在有限数据集上训练也表现良好，具有强泛化能力和计算效率。
...
TL;DR: 本文提出了选择性欠拟合的概念，认为更好的扩散模型会在输入空间的某些区域更准确地近似分数函数，而在其他区域欠拟合，这对于理解扩散模型的泛化能力至关重要。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在训练时学习分数函数用于生成样本，但一个基本未解的问题是：它们实际学习的是哪个分数？理论上，完全匹配经验分数的模型只会复制训练数据而无法生成新样本。

Method: 引入选择性欠拟合概念，通过表征这些区域并设计实证干预来验证该观点，分析扩散模型在不同输入区域的分数函数近似准确性。

Result: 研究结果表明选择性欠拟合对于理解扩散模型至关重要，为模型的泛化和生成性能提供了新的可测试见解。

Conclusion: 扩散模型不是在所有地方都欠拟合分数函数，而是通过选择性欠拟合在特定区域保持准确性，这解释了它们能够生成新颖样本的能力。
...
TL;DR: PRISM是一个轻量级的模型无关方法，用于掩码扩散模型的推理时自校正，无需重新训练即可检测和修正低质量token


<details>
  <summary>Details</summary>
Motivation: 虽然掩码扩散模型在离散空间生成建模方面很有前景，但其自校正能力尚未充分研究。现有方法要么需要重构模型架构/训练，要么依赖不精确的token质量代理，限制了适用性
...
Conclusion: PRISM为预训练掩码扩散模型提供了一种轻量级、模型无关的自校正方法，有效提升了推理质量
...
TL;DR: 提出基于Pandora's Box问题的LLM推理时优化框架，通过UCB算法实现接近最优的停止策略，在保持性能的同时减少15-35%的生成次数。


<details>
  <summary>Details</summary>
Motivation: LLM生成需要在输出质量和推理成本之间取得平衡，特别是在需要多次生成时，传统方法如Best-of-N采样效率低下。

Method: 将每次生成视为打开一个代价盒子的Pandora's Box问题，开发UCB风格的算法，并结合Bradley-Terry变换处理不同提示的奖励缩放问题。
...
Conclusion: 该框架使基于梯度的自由能方法能够整合复杂和机器学习的集体变量，拓展了生物化学和材料模拟的应用范围。
...
<details>
  <summary>Details</summary>
Motivation: 当前神经压缩方法虽然压缩率高，但基于卷积的解码器计算复杂且成本高，阻碍了技术应用，需要解决解码器瓶颈问题。
...
TL;DR: 这篇综述论文系统性地分析了边缘人工智能(Edge AI)的发展历程、当前现状和未来方向，涵盖了部署位置、处理能力、应用领域和硬件类型等多个维度，并探讨了关键技术、挑战和新兴机遇。


<details>
  <summary>Details</summary>
Motivation: 随着边缘计算的发展，将AI能力直接嵌入网络边缘设备的需求日益增长，这能够实现实时处理、提升隐私保护和降低延迟。本文旨在为研究者和从业者提供一个全面的Edge AI框架。

Method: 采用PRISMA指南进行系统性分析，通过多维度分类法（包括部署位置、处理能力如TinyML和联邦学习、应用领域、硬件类型）来审视Edge AI的发展历程，从早期的内容分发网络和雾计算到现代的设备端智能。

Result: 分析了Edge AI的核心使能技术（专用硬件加速器、优化软件、通信协议），评估了资源限制、安全性、模型管理、功耗和连接性等关键挑战，并识别了神经形态硬件、持续学习算法、边缘-云协作和可信集成等新兴机遇。

Conclusion: Edge AI代表了将智能直接嵌入边缘设备的重要范式转变，通过系统性分析为未来研究提供了全面框架，强调了在资源受限环境中实现高效、安全和可信AI的关键技术路径。
...
TL;DR: SoftAdaClip是一种差分隐私训练方法，用基于tanh的平滑变换替代硬裁剪，在限制敏感性的同时保留梯度相对大小，显著减少子群体差异。
...
Method: 提出SoftAdaClip方法，用平滑的tanh变换替代硬裁剪，结合自适应机制来保持梯度相对大小，同时限制敏感性。

Result: 在MIMIC-III、GOSSIS-eICU和Adult Income数据集上的实验显示，SoftAdaClip相比DP-SGD减少子群体差异达87%，相比Adaptive-DPSGD减少48%，且这些减少具有统计显著性。
...
TL;DR: 提出了Local Linear Attention (LLA)，一种基于非参数统计理论的新型注意力机制，通过测试时回归视角推导，在理论分析和实验验证中优于线性注意力和Softmax注意力。


<details>
  <summary>Details</summary>
Motivation: 虽然Softmax注意力的高效替代方案已被广泛研究，但基于理论洞察的更具表达力的机制（即使计算成本更高）相对较少探索。

Method: 从测试时回归角度推导出局部线性注意力(LLA)，提出两种内存高效原语解决计算复杂度问题，开发FlashLLA硬件高效块状算法，并实现定制推理内核减少内存开销。

Result: LLA在测试时回归、上下文回归、关联回忆和状态跟踪任务中有效适应非平稳性，在测试时训练和上下文学习中优于强基线，并显示出在大规模模型中可扩展性和适用性的有希望证据。

Conclusion: LLA通过偏差-方差权衡分析在理论上有优势，实验证明其在多种任务中表现优异，为大规模模型提供了有前景的注意力机制替代方案。
...
TL;DR: SCOPED是一种快速通用的扩散模型OOD检测方法，通过结合Jacobian迹和得分函数平方范数，将前向传播次数减少一个数量级，在保持竞争力的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的OOD检测方法计算成本高，需要大量前向传播，限制了在实际部署中的可靠性。需要开发既快速又准确的OOD检测方法。

Method: SCOPED结合Jacobian迹和得分函数平方范数作为测试统计量，使用核密度估计估计分布内密度，仅需单次前向传播和一次Jacobian-向量乘积，利用Hutchinson迹估计器提高效率。

Result: 在四个视觉基准测试中，SCOPED实现了竞争性或最先进的精确召回分数，计算成本显著降低。该方法还推广到具有共享状态和动作空间的机器人控制任务。
...
TL;DR: 该论文分析了MBPO在DeepMind Control Suite中性能下降的原因，识别了动态模型与奖励模型尺度不匹配以及目标表示选择不当两个关键问题，并通过解决这些问题使MBPO在多数任务中超越了SAC。


<details>
  <summary>Details</summary>
Motivation: 研究MBPO在OpenAI Gym中表现优异但在DeepMind Control Suite中性能下降的原因，揭示环境特定假设如何隐式影响算法设计，并探索如何解决这些失败模式以实现更好的策略改进。

Method: 分析MBPO在DMC任务中的失败原因，识别动态模型与奖励模型尺度不匹配导致的评论家低估问题，以及目标表示选择不当导致的模型方差膨胀问题，并针对这些问题提出解决方案。

Result: 通过解决识别出的失败模式，MBPO在7个DMC任务中的5个超越了SAC，同时保持了在OpenAI Gym中的优异性能，实现了之前无法实现的策略改进。
...
Method: 考虑常见的现实噪声模型（如错误标记和不确定性），分析有限步偏好优化的泛化性能，基于偏好数据分布和样本数量分析噪声率对泛化的影响。

Result: 理论分析表明泛化性能随不同类型噪声和噪声率而衰减，实证验证证实了研究发现在当代LLMs中的实际相关性。

Conclusion: 研究为开发符合人类偏好的AI系统提供了有价值的见解，特别是在存在噪声反馈的现实条件下。
...
TL;DR: 提出了一种基于响应长度的动态采样策略优化方法LSPO，用于改进强化学习与可验证奖励的训练效果。


<details>
  <summary>Details</summary>
Motivation: 受LLM过度思考现象研究的启发，旨在通过动态选择训练数据来提高RLVR算法的效率和效果。

Method: LSPO是一种元RLVR算法，根据平均响应长度在每个训练步骤中动态选择训练数据。

Result: 在多个基础模型和数据集上的评估表明，LSPO能够持续提高学习效果。

Conclusion: 通过消融研究验证了将长度信号融入动态采样的有效性，为未来研究指明了方向。
...
TL;DR: 该论文提出了一个稳定性-可塑性原则来解释离线到在线强化学习中的不一致性，认为在线微调时应保留预训练策略或离线数据集中更好的知识，同时保持足够的可塑性。
...
Method: 提出了稳定性-可塑性原则，将在线微调分为三个不同的机制，每个机制需要不同的稳定性属性，并通过大规模实证研究验证该框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统高斯过程(GP)在高维不规则变量(如分类、序数变量)的贝叶斯优化中表现不佳，需要更强大的代理模型来处理复杂的高维优化问题。

Method: 采用LLM作为代理模型，结合LoRA微调和VBLL框架，构建轻量级且支持递归更新的LoRA-VBLL方法，并使用加权集成(ENS)自动选择超参数。

Result: 在多个高维基准测试和实际分子优化任务中，所提出的(ENS-)LoRA-VBLL方法表现出优越性能。

Conclusion: 基于LLM的代理模型结合LoRA-VBLL框架为高维黑盒优化提供了高效解决方案，在保持计算轻量的同时实现了优异的优化性能。
...
TL;DR: PEL-NAS是一种基于LLM的神经架构搜索方法，通过搜索空间分区、架构提示协同进化和零成本预测器，解决了传统方法搜索成本高和LLM方法探索偏差的问题，在保持精度的同时显著降低延迟和搜索时间。


<details>
  <summary>Details</summary>
Motivation: 传统超网方法需要多个GPU天，而LLM驱动的方法存在探索偏差，无法在整个搜索空间中发现不同延迟范围的架构。需要一种能同时优化精度和延迟、降低搜索成本的方法。

Method: 1) 基于复杂度的搜索空间分区引擎；2) LLM驱动的架构提示协同进化算子；3) 零成本预测器避免从头训练大量候选架构。

Result: 在HW-NAS-Bench上，PEL-NAS实现了更高的HV、更低的IGD，在相似精度下延迟比基线降低高达54%，搜索成本从几天降至几分钟。

Conclusion: PEL-NAS通过创新的搜索空间分区和提示协同进化机制，有效解决了LLM驱动NAS的探索偏差问题，实现了高效、低成本的硬件感知神经架构搜索。
...
TL;DR: 提出密度比加权行为克隆方法，使用小规模干净参考集通过二元判别器估计轨迹级密度比，在行为克隆目标中加权以优先学习干净专家行为，有效应对离线强化学习数据中的对抗性污染问题。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习数据集常被对抗性污染、系统错误或低质量样本污染，导致标准行为克隆和离线RL方法性能下降，需要开发能够从污染数据中稳健学习的方法。

Method: 使用小规模干净参考集训练二元判别器估计轨迹级密度比，对密度比进行裁剪后作为权重应用于行为克隆目标，优先学习干净专家行为，无需了解污染机制。

Result: 在连续控制基准测试中，即使在高污染率下仍能保持接近最优性能，优于传统BC、BCQ和BRAC等基线方法。

Conclusion: 密度比加权行为克隆方法能够有效应对离线强化学习数据污染问题，具有理论保证和实际有效性，为安全关键应用提供了稳健的模仿学习解决方案。
...
TL;DR: 本文提出了对抗攻击可迁移性的一个基本区别：在输入数据空间的攻击可以迁移，而在模型表示空间的攻击则不能迁移，除非表示空间有几何对齐。


<details>
  <summary>Details</summary>
Motivation: 解释为什么图像对抗样本和文本越狱攻击可以在不同模型间迁移，而视觉语言模型的图像越狱攻击却无法迁移的现象。

Method: 通过四个不同场景提供理论和实证证据：1）在简单设置中数学证明这一区别；2）构建针对图像分类器的表示空间攻击；3）构建针对语言模型的表示空间攻击；4）构建针对视觉语言模型的数据空间攻击。

Result: 数据空间攻击成功迁移到新的视觉语言模型，而表示空间攻击只有在潜在几何充分对齐时才能迁移。表示空间攻击对目标模型有效但无法迁移。

Conclusion: 对抗迁移不是所有攻击的固有属性，而是取决于其操作域——共享的数据空间与模型独特的表示空间，这对构建更鲁棒的模型具有重要启示。
...
TL;DR: 提出两种新的多智能体LLM答案聚合算法OW和ISP，利用一阶和二阶信息改进多数投票方法，在理论和实验上均优于标准多数投票。


<details>
  <summary>Details</summary>
Motivation: 随着多智能体LLM推理的发展，如何有效聚合多个LLM的答案成为关键挑战。标准多数投票方法平等对待所有答案，忽略了模型间的潜在异质性和相关性。

Method: 设计了两种新算法：最优权重(OW)和逆惊喜流行度(ISP)，利用一阶和二阶信息来改进聚合效果。

Result: 在合成数据集、LLM微调基准(UltraFeedback、MMLU)和真实医疗场景(ARMMAN)上的实验表明，新方法始终优于多数投票。

Conclusion: OW和ISP算法在理论和实践上都优于多数投票，为构建鲁棒的多智能体LLM管道提供了性能提升和概念洞见。
...
TL;DR: 提出一种端到端方法，通过动作空间设计改进ICU脓毒症休克患者的双血管加压药给药策略，结合离线保守Q学习和新型循环建模，提高临床可解释性和采用率。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习在临床决策支持系统中因不可操作给药决策而受到从业者质疑的问题，特别是ICU脓毒症休克患者的血管加压药给药控制。

Method: 采用动作空间设计支持离散、连续和定向给药策略，结合离线保守Q学习和新型循环建模在重放缓冲区中捕捉ICU时间序列数据的时序依赖性。

Result: 在eICU和MIMIC数据集上的实证结果显示，动作空间设计显著影响学习到的行为策略，患者生存改善概率提高超过15%，同时与既定临床方案保持一致。

Conclusion: 设计的动作空间在保持疗效的同时提高了可解释性并促进临床采用，证明了动作空间设计对学习行为策略的深远影响。
...
Method: 引入概率节点-关系等变性，通过迭代采样随机游走、编码序列、序列模型嵌入和池化聚合来表示节点和关系

Result: 在新诊断数据集Petals上完美解决现有模型失败的问题，在54个知识图谱上实现最先进的实体和关系预测性能

Conclusion: 概率等变性是提升知识图谱基础模型表达能力的关键，Flock模型是KG同构不变链接级函数的通用逼近器
...
TL;DR: 提出预测性偏好学习（PPL）方法，通过人类干预的隐式偏好信号来预测未来状态，将单次干预扩展到未来多个时间步，从而提高学习效率和减少人类演示需求。


<details>
  <summary>Details</summary>
Motivation: 现有交互式模仿学习方法只修正当前状态下的动作，而不调整未来可能更危险状态下的动作，这限制了学习效果和安全性。

Method: PPL方法将每次人类干预引导到未来L个时间步（偏好视野），假设在这些未来状态下智能体采取相同动作且人类进行相同干预，然后在这些未来状态上应用偏好优化。

Result: 在自动驾驶和机器人操作基准测试中验证了方法的效率和通用性，显著提高了学习效率并减少了所需的人类演示。

Conclusion: 理论分析表明选择合适的偏好视野L可以在风险状态覆盖和标签正确性之间取得平衡，从而限制算法最优性差距。
...
TL;DR: 开发了一个预测兽医药物安全结果的框架，使用FDA的128万份报告，通过数据预处理、特征工程和多种机器学习模型，能够准确预测动物用药后的死亡与恢复结果。


<details>
  <summary>Details</summary>
Motivation: 保护动物福利和人类食品安全，通过预测药物不良反应来识别可能导致食品中违规残留的高风险药物事件。

Method: 使用FDA兽医医学中心的128万份报告，进行数据预处理、特征标准化、缺失值填补，整合药物理化性质，评估多种监督学习模型和集成方法，采用欠采样和过采样处理类别不平衡问题。

Result: 集成方法和CatBoost表现最佳，精确度、召回率和F1分数达到0.95，通过AUM伪标记提高了少数类检测能力，SHAP分析识别出与致命结果相关的生物合理预测因子。

Conclusion: 结合严格数据工程、先进机器学习和可解释AI，能够实现准确、可解释的兽医安全结果预测，支持早期高风险药物事件检测和监管决策。
...
<details>
  <summary>Details</summary>
Motivation: 现有碳强度预测工具存在三个主要限制：需要电网特定电力结构数据、依赖单独电网特定模型难以实现全球覆盖、缺乏不确定性估计。

Method: 利用时间序列基础模型的通用性，仅使用历史碳强度数据和单一通用模型，支持零样本预测和插补任务。

Result: 在214个全球电网中零样本预测MAPE为15.82%；在13个基准电网中平均MAPE为9.59%，尾部预测MAPE为16.54%，提供95%覆盖率的预测区间；完全微调后在插补任务上比统计基线提升1.2-3.9倍。

Conclusion: CarbonX可在数据有限的情况下轻松应用于任何电网并保持强劲性能，是实用的全球规模脱碳工具。
...
TL;DR: 论文提出了两种改进二值化神经网络验证的整数规划方法：新的多类别线性目标函数和利用BNN递归结构的有效不等式生成技术，显著提高了验证效率。


<details>
  <summary>Details</summary>
Motivation: 二值化神经网络验证问题通常被建模为整数规划问题，但传统方法存在大M约束导致的整数间隙问题，难以高效求解。

Method: 1. 为多类别分类场景设计新的线性目标函数；2. 利用BNN的递归结构生成有效的切割平面不等式。
...
TL;DR: 本文提出Round-Trip Reinforcement Learning (RTRL)框架，通过双向转换一致性作为奖励信号来训练化学大语言模型，显著提升了模型性能和一致性。
...
Method: 提出RTRL框架，使用双向转换成功作为奖励信号进行强化学习训练；进一步提出迭代变体，让正向和反向映射交替训练彼此，形成自改进循环。
...
TL;DR: 提出了一种绕过提示防护的新攻击方法，通过利用提示防护与主LLM之间的资源不对称性，成功越狱多个生产模型，揭示了轻量级提示防护的固有漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，确保AI安全和对齐至关重要。轻量级提示防护因易于实现和更新而流行，但本文旨在揭示其局限性。

Method: 利用提示防护与主LLM之间的资源不对称性，编码一个轻量级防护无法解码但主模型可以解码的越狱提示。

Result: 该方法在Google Gemini、DeepSeek Chat、Grok和Mistral Le Chat等高度保护的聊天接口中持续成功越狱生产模型，同时保持响应质量。

Conclusion: 揭示了现代LLM架构中轻量级提示防护的固有攻击面，强调需要将防御重点从阻止恶意输入转向防止恶意输出。
...
TL;DR: 提出一个将Python算法编译为GPU可运行二进制文件的框架，通过CNN实现PUSCH接收机中的信道估计功能，为6G网络AI原生系统提供可扩展的AI/ML集成方案。


<details>
  <summary>Details</summary>
Motivation: 6G向AI原生无线系统转型，需要将数字信号处理和机器学习无缝集成到蜂窝网络软件栈中，使网络生命周期更接近AI系统。

Method: 开发一个稳健框架，将基于Python的算法编译成GPU可运行的二进制文件，在NVIDIA AI Aerial平台上实现，使用CNN进行PUSCH接收机的信道估计。

Result: 实现了在数字孪生和实时测试平台中高效执行信道估计功能，确保在NVIDIA GPU上的最高性能。

Conclusion: 该框架为下一代蜂窝系统可扩展集成AI/ML模型奠定了基础，对实现原生智能6G网络愿景至关重要。
...
TL;DR: TSci是首个基于LLM的智能代理框架，用于通用时间序列预测，通过四个专门代理（Curator、Planner、Forecaster、Reporter）实现自动化预处理、模型选择和预测，在八个基准测试中显著优于统计和LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列预测面临数千个短而嘈杂的序列，传统方法需要大量人工预处理和验证工作，且现有模型泛化能力差，亟需一个最小化人工干预的通用领域无关框架。

Method: TSci框架包含四个专门代理：Curator进行LLM引导的诊断和预处理；Planner通过多模态诊断缩小模型选择空间；Forecaster执行模型拟合、验证和自适应选择最佳配置；Reporter生成全面透明的报告。

Result: 在八个基准测试中，TSci持续优于统计和LLM基线方法，分别将预测误差平均降低了10.4%和38.2%，并生成清晰严谨的报告。

Conclusion: TSci将预测工作流程转变为可解释、可扩展的白盒系统，通过透明自然语言推理和全面报告，显著提高了时间序列预测的自动化和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法忽略反事实推理中的溯因步骤，导致高估LLM性能，需要开发更全面的评估框架来提升LLM的因果理解能力。

Method: 引入可执行反事实框架，要求完整的反事实推理三步（溯因、干预、预测），通过代码和数学问题创建可扩展的合成数据，并比较监督微调与强化学习的效果。

Result: SOTA模型在反事实推理中准确率下降25-40%；监督微调提升域内性能但损害OOD泛化；强化学习在代码和数学问题上分别获得1.5-2倍提升，并能泛化到新领域。

Conclusion: 强化学习能诱导核心认知行为并实现跨领域泛化，为提升LLM反事实推理能力提供了有前景的途径。
...
TL;DR: MIRA是一种无需训练、在推理时对齐的扩散模型方法，通过图像空间的KL代理正则化来防止奖励黑客攻击，在保持提示一致性的同时提高奖励分数。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像虽然符合文本提示，但往往无法满足用户特定的标量奖励标准（如美学评分）。现有的推理时对齐方法存在奖励黑客问题，即模型产生高评分但偏离原始提示的图像。

Method: 提出MIRA方法，引入基于分数的图像空间KL代理正则化，约束采样轨迹以防止奖励黑客攻击。还提出了MIRA-DPO，将偏好优化映射到推理时，适用于不可微分的奖励。

Result: 在SDv1.5和SDXL模型上，针对多种奖励（美学评分、HPSv2、PickScore）和公共数据集，MIRA相对于强基线实现了超过60%的胜率，同时保持了提示一致性。

Conclusion: MIRA通过图像空间约束有效解决了推理时对齐中的奖励黑客问题，在提高奖励分数的同时保持了生成图像与提示的一致性，且无需微调。
...
TL;DR: 本文分析了RLHF中KL散度正则化的不同实现方式，证明了'k1 in reward'（如PPO）是Reverse KL正则化的理论基础，并揭示了'k2 as loss'在on-policy条件下与'k1 in reward'梯度等价，而'k3 as loss'（如GRPO）只是有偏的一阶近似。
...
Method: 建立统一框架连接两种实现风格：将数学项kn作为策略评分函数的分离系数（'kn in reward'）或作为直接传播梯度的损失函数（'kn as loss'），证明后者总可以通过前者的等效梯度系数来分析。

Result: 证明了'k1 in reward'是Reverse KL正则化的理论基础；在on-policy条件下，'k2 as loss'与'k1 in reward'梯度等价；'k3 as loss'只是有偏的一阶近似；常见的off-policy实现因忽略重要性采样而有偏。
...
TL;DR: 提出了IBCD（干预贝叶斯因果发现）框架，利用干预数据改进因果发现，通过建模总因果效应矩阵并采用稀疏先验，实现大规模任务中的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法在大规模任务中性能不佳且无法量化不确定性，而高通量基因组扰动数据为改进模型识别提供了机会。

Method: 采用经验贝叶斯框架，建模总因果效应矩阵的似然（近似矩阵正态分布），使用spike-and-slab horseshoe先验，从观测数据中学习数据驱动的网络结构权重。

Result: 在广泛模拟中，IBCD在结构恢复方面优于现有基线方法；在521个基因的CRISPR扰动数据中，边后验包含概率能够识别稳健的图结构。

Conclusion: IBCD框架通过有效利用干预数据和不确定性量化，显著提升了大规模因果发现任务的性能。
...
TL;DR: TetriServe是一个针对Diffusion Transformer模型的动态并行服务系统，通过步骤级序列并行和轮次调度机制，在异构工作负载下显著提高SLO达成率和GPU利用率。
...
Method: 提出步骤级序列并行策略，动态调整单个请求的并行度；引入轮次调度机制，包括时间离散化、步骤级并行度适配和请求联合打包。

Result: 在先进的DiT模型上评估显示，TetriServe相比现有解决方案能实现高达32%的SLO达成率提升，且不降低图像质量。
...
TL;DR: 该研究探讨了强化学习(RL)能否推动蛋白质语言模型(PLMs)超越其预训练先验，发现潜在的序列-结构-功能规则。通过在四个蛋白质设计领域进行实验，发现RL能持续提高成功率和采样效率，性能提升取决于任务空间、奖励保真度和策略容量的三因素交互作用。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习是否能够推动蛋白质语言模型超越其预训练先验，揭示监督学习未能捕捉的序列-结构-功能关系，为蛋白质设计提供更有效的优化方法。

Method: 将强化学习与蛋白质语言模型结合，在抗菌肽设计、激酶变体优化、抗体工程和逆折叠四个领域进行实验，使用多样化的RL算法和模型类别。

Result: RL在所有基准测试中持续提高成功率和采样效率。当奖励准确且信息丰富、策略容量充足、任务有改进空间时，性能提升显著；当奖励噪声大或容量受限时，增益会饱和。

Conclusion: 为蛋白质设计中的RL应用提供了实用指导：优先进行奖励建模和校准，匹配算法和正则化强度与任务难度，在边际收益最大的地方分配容量。
...
<details>
  <summary>Details</summary>
Motivation: 传统的梯度裁剪使用固定的硬阈值，限制了灵活性且忽略了梯度分布动态。需要一种更灵活、自适应的梯度调制方法。

Method: SPAMP框架跟踪局部梯度统计，动态估计阈值，并应用基于幂的变换以可微分方式调制更新幅度。将裁剪和预热重新解释为控制有效更新尺度的双重机制。

Result: 在图像和语言任务上的广泛实验表明，SPAMP在稳定性、收敛性和鲁棒性方面优于现有方法。

Conclusion: SPAMP提供了一个原则性的替代方案，取代了僵硬的启发式方法，通过统一的梯度整形框架改进了深度网络训练。
...
TL;DR: TRAAC是一种在线后训练强化学习方法，通过自适应压缩推理步骤来解决模型在复杂推理任务中的欠适应性问题，在保持准确率的同时显著减少推理长度。


<details>
  <summary>Details</summary>
Motivation: 当前模型在复杂推理任务中面临欠适应性问题：简单问题推理过长（过度思考），复杂问题推理不足（思考不足）。需要平衡推理长度与任务难度。

Method: 提出TRAAC方法，利用模型在长推理轨迹上的自注意力机制识别重要步骤并剪枝冗余步骤，同时估计任务难度并将其纳入训练奖励，学习根据难度分配推理预算。

Result: 在多个任务上，TRAAC相比基础模型平均准确率提升8.4%，推理长度减少36.8%；相比最佳RL基线准确率提升7.9%，长度减少29.4%。在非数学数据集上也表现出良好的泛化能力。

Conclusion: TRAAC通过任务难度校准和基于注意力的压缩，能够根据任务难度精细调整推理预算，在多样任务上实现准确率和效率的提升。
...
TL;DR: NoRo是一个用于帕金森病远程监测的噪声鲁棒UPDRS预测框架，通过构建对比对和特征增强来提高预测模型在不同噪声环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 帕金森病远程监测中存在患者测量误差、环境噪声和数据包丢失三种噪声，导致UPDRS预测误差增加，需要开发噪声鲁棒的预测方法。

Method: 首先将语音特征按选定特征值分组构建对比对，然后训练多层感知机编码器生成噪声鲁棒特征，最后将原始特征与鲁棒特征拼接作为增强特征输入预测模型。

Result: 实验表明NoRo能够成功增强各种下游预测模型在不同噪声环境下的UPDRS预测鲁棒性。

Conclusion: NoRo框架有效解决了帕金森病远程监测中的噪声问题，提高了UPDRS预测的准确性和可靠性。
...
TL;DR: 该论文提出使用自旋转移矩磁隧道结(STT-MTJs)生成真随机数，以解决生成式AI模型中伪随机数生成器的安全漏洞问题，显著降低了不安全输出。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能模型使用的确定性伪随机数生成器会产生可预测模式，容易受到攻击者利用，而传统防御方法存在显著的能耗和延迟开销。

Method: 通过嵌入STT-MTJs硬件生成的真随机比特，构建高度并行的FPGA辅助原型计算系统，并将硬件随机比特集成到在CIFAR-10上训练的生成对抗网络中。

Result: 系统能以兆比特/秒的速度提供真随机数，通过NIST随机性测试，与低质量随机数生成器基线相比，将不安全输出减少了高达18.6倍。
...
TL;DR: 从统计物理学角度分析VAE中的后验坍塌现象，揭示其是由数据结构和模型超参数共同控制的相变过程，并识别了临界超参数阈值。


<details>
  <summary>Details</summary>
Motivation: 研究变分自编码器（VAE）中后验坍塌现象的本质，传统观点认为这是优化失败，但本文从统计物理学角度提出新解释。

Method: 通过分析后验坍塌相关平凡解的稳定性，识别临界超参数阈值，并在合成和真实数据集上验证临界行为。

Result: 发现了表征后验坍塌的相变临界边界，表现为近似后验与先验分布之间KL散度的不连续性，验证了相变的存在。

Conclusion: 后验坍塌不是单纯的优化失败，而是数据结构和变分约束相互作用产生的相变现象，为深度生成模型的可训练性和表示能力提供了新见解。
...
TL;DR: 本文挑战了SFT高分能提升RL后性能的传统观点，发现SFT分数可能偏向简单或同质数据，不能可靠预测RL效果。提出了泛化损失和Pass@large k作为更好的RL结果预测指标。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理后训练采用SFT和RL两个独立阶段，但高SFT分数是否能转化为RL后的性能提升存在疑问。作者发现这种关联并不总是成立，需要更可靠的预测指标。

Method: 训练了数百个12B参数模型，使用SFT和GRPO进行RLVR，在7个数学基准上进行了256次重复评估，消耗超过100万GPU小时。实验涵盖Llama3、Mistral-Nemo、Qwen3等模型和多个SOTA数据集。

Result: 基于泛化损失和Pass@large k的预测比直接使用SFT前性能预测更准确，R²系数和Spearman秩相关系数提升高达0.5（2倍）。发现SFT训练独特样本1个epoch不如训练一半样本2个epoch，仅训练短样本虽然SFT表现好但RL后结果更差。

Conclusion: SFT分数不能可靠预测RL效果，泛化损失和Pass@large k是更好的预测指标。相同SFT预算下，训练数据多样性和训练策略对最终RL结果有重要影响。
...
TL;DR: 大规模实证研究表明，在预训练中混合约30%的改写合成数据与自然网页数据，可以在较大数据预算下实现5-10倍的训练加速，而纯合成数据训练效果较差。


<details>
  <summary>Details</summary>
Motivation: 高质量训练数据对LLM扩展至关重要但供应有限，合成数据技术有望突破这一限制，但需要系统验证其有效性。

Method: 使用统一协议和扩展定律进行大规模实证研究（>1000个LLM，>10万GPU小时），比较自然网页数据、多种合成数据类型及其混合的效果。

Result: 纯改写合成数据训练效果不优于自然数据；1/3改写合成数据与2/3自然数据混合可加速5-10倍；纯教科书风格合成数据在下游任务表现较差；最佳合成数据比例约30%。

Conclusion: 合成数据在预训练中具有条件性益处，改写合成数据混合训练可显著加速且无模型崩溃，而纯生成合成数据可能引发模型崩溃。
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer隐含假设欧几里得几何，限制了在具有非欧几里得结构数据上的有效性。现有方法需要预先选择单一几何，缺乏对混合几何特性的适应性。

Method: CAT架构包含三个几何注意力分支（欧几里得、双曲、球面），通过可微分门控机制动态路由每个token到合适的几何分支，每个分支使用针对各自流形优化的几何特定操作。

Result: 在知识图谱补全基准测试（FB15k-237, WN18RR）上，CAT相比固定几何基线在MRR和Hits@10指标上提升约10%，仅增加5%参数且推理时间相当。

Conclusion: 学习的几何自适应优于任何单一固定几何，CAT为语言、视觉和多模态领域的混合几何架构提供了可扩展和可解释的基础。
...
TL;DR: 提出了一种组合模式水印框架，用于检测和定位水印LLM输出中的后生成编辑，通过词汇表分区和组合模式嵌入水印，并设计了轻量级统计方法来检测和定位编辑。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，LLM生成的内容可能经过人工修订或欺骗性修改，因此需要检测和定位这些后生成编辑，以保护水印的有效性。

Method: 采用组合模式水印框架，将词汇表划分为不相交子集，在生成过程中强制实施确定性组合模式来嵌入水印，并设计全局统计量检测水印和轻量级局部统计量定位编辑。

Result: 在开源LLM上评估了多种编辑场景，展示了在编辑定位方面的强大实证性能，使用Type-I错误率和检测准确率作为评估指标。

Conclusion: 提出的组合模式水印框架能够有效检测和定位水印LLM输出中的后生成编辑，为保护专有语言模型提供了重要技术手段。
...
TL;DR: 提出了一种基于支撑基分解的新框架，用于高效近似注意力计算，突破了传统有界条目假设的限制，实现了次二次时间复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决softmax注意力二次复杂度瓶颈，现有次二次近似算法需要限制性有界条目假设，这在现代大语言模型中很少成立。

Method: 利用查询和键矩阵条目的次高斯性质，将大条目和小条目分离，对稀疏部分精确计算，对稠密部分使用多项式近似，并扩展到多阈值设置消除分布假设。

Result: 建立了严格的理论保证，证明了次二次运行时间，并为多项式注意力的经验成功提供了首个理论证明。

Conclusion: 支撑基分解框架能够有效近似softmax注意力，突破了传统限制，为大语言模型的扩展提供了更实用的高效注意力计算方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨域持续学习方法需要完全标记的源域，这在隐私受限环境中不可行。本文旨在解决无源跨域持续学习问题，完全禁止使用源域样本。

Method: REFEREE方法结合源预训练模型和大规模视觉语言模型，采用频率感知提示技术生成增强样本，使用不确定性加权策略处理噪声伪标签，通过核线性判别分析缓解灾难性遗忘。

Result: 严格的数值研究证实该方法优于现有方法，即使现有方法可以访问源域样本，REFEREE仍能显著超越它们。

Conclusion: REFEREE方法成功解决了无源跨域持续学习问题，在禁止使用源域样本的情况下实现了优越性能，为隐私受限环境提供了可行解决方案。
...
TL;DR: 提出了一种名为Elsa的神经网络剪枝方法，能够在保持高模型精度的同时实现高达90%的极端稀疏度，突破了当前方法在50-60%稀疏度下的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络剪枝方法在大型语言模型上无法超越中等稀疏度水平（50-60%）而不严重损害模型精度，这限制了模型压缩的进一步发展。

Method: 通过识别当前实践中基于代理目标公式的局限性，采用基于ADMM的标准约束优化技术，直接有效地解决这些问题。还提出了量化变体Elsa-L以适应超大型模型。

Result: 在广泛的模型和规模上的实验表明，Elsa显著优于现有方法，例如在LLaMA-2-7B模型上90%稀疏度时，困惑度比现有最佳方法低7.8倍。Elsa-L能够扩展到极大模型（27B）并具有理论收敛保证。

Conclusion: 这项工作在推进LLM稀疏化前沿方面取得了有意义的进展，表明在迄今探索有限的方向上仍存在显著的进一步改进机会。
...
TL;DR: AsyPPO是一种针对大语言模型RL训练的新框架，通过轻量级mini-critics解决传统价值函数在LLM规模下的计算瓶颈，提升学习稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 当前RL4LLM方法避免使用显式critics，因为传统价值函数在LLM规模下计算昂贵，且在稀疏奖励和长推理视野下容易失败。

Method: 使用一组轻量级mini-critics，每个在不相交的提示分片上训练，利用critics间的不确定性来优化策略更新，包括屏蔽优势值和过滤高分歧状态。

Result: 在仅5000个样本上训练后，AsyPPO在多个基准测试中持续提升学习稳定性和性能，在Qwen3-4b-Base上比经典PPO提升超过6%，在Qwen3-8b-Base和Qwen3-14b-Base上提升约3%。

Conclusion: 架构创新对于可扩展、高效算法的重要性，AsyPPO成功恢复了critics的作用同时保持大模型设置下的效率。
...
TL;DR: TimeHUT是一种通过层次化均匀性-容忍度平衡来学习时间序列表示的新方法，在分类任务上显著优于现有方法，在异常检测上获得竞争性结果


<details>
  <summary>Details</summary>
Motivation: 旨在通过平衡嵌入空间中的均匀性和容忍度特性来学习强健的时间序列表示，同时捕捉实例级和时间级信息

Method: 使用层次化设置学习实例级和时间级信息；在对比损失中集成温度调度器来平衡均匀性和容忍度；使用层次化角度边际损失在时间序列的正负对之间创建几何边界

Result: 在128个UCR和30个UAE数据集上的分类任务中显著优于先前方法，在Yahoo和KPI数据集上的异常检测获得竞争性结果

Conclusion: TimeHUT通过层次化均匀性-容忍度平衡有效提升了时间序列表示学习性能，在分类任务上表现优异
...
<details>
  <summary>Details</summary>
Motivation: KAN网络虽然能恢复符号表示，但其独特架构使得传统剪枝方法不可靠，需要一种对输入参数化不敏感的重要性评估方法。

Method: 使用Shapley值归因来量化每个节点的实际贡献，确保重要性排名的一致性，实现网络压缩。

Result: 在合成和真实数据集上的实验表明，ShapKAN能保持真实节点重要性，同时实现有效的网络压缩。

Conclusion: ShapKAN增强了KAN的可解释性优势，促进了在资源受限环境中的部署。
...
<details>
  <summary>Details</summary>
Motivation: 传统的简单融合技术无法有效处理多模态数据中的噪声、缺失或语义冲突等问题，导致在识别细微情感时性能不佳。

Method: 采用双门融合机制，基于信息熵和模态重要性自适应调整特征权重，在单模态编码和跨模态交互后优先选择信息丰富的线索。

Result: 在CMU-MOSI和CMU-MOSEI数据集上的实验表明，AGFN在准确率上显著优于强基线模型，能有效识别细微情感且性能稳健。

Conclusion: AGFN通过减少特征位置与预测误差之间的相关性，学习更广泛的特征分布，从而增强了泛化能力，创建了更稳健的多模态特征表示。
...
TL;DR: 提出了一种基于悲观原则的离线数据驱动分类优化框架PASTA，能够在数据覆盖不足的情况下实现最优预期收益，并在多种选择模型下建立了首个有限样本遗憾界。


<details>
  <summary>Details</summary>
Motivation: 解决分类优化问题中由于数据覆盖不足导致的挑战，企业缺乏对基础选择模型的先验知识，需要基于历史客户选择数据确定最优分类。

Method: 引入悲观分类优化(PASTA)框架，利用悲观原则在一般选择模型下实现最优预期收益，仅要求离线数据分布包含最优分类而非覆盖所有可行分类。

Result: 建立了多种常用选择模型（包括多项式logit和嵌套logit模型）下的首个有限样本遗憾界，证明了PASTA在样本和模型复杂度方面的极小极大最优性。

Conclusion: PASTA框架在理论上具有最优性能保证，数值实验表明其优于现有基线方法，为离线数据驱动的分类优化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统表示相似性方法独立对齐每层网络，导致结果不对称、缺乏全局对齐分数，难以处理不同深度的网络。这些限制源于忽略了全局激活结构和限制映射为刚性的一对一层对应关系。

Method: HOT框架联合推断软性、全局一致的层到层耦合和神经元级传输计划，允许源神经元将质量分布到多个目标层，在边际约束下最小化总传输成本。

Result: 在视觉模型、大语言模型和人类视觉皮层记录上的评估显示，HOT在对齐质量上匹配或超越了标准成对匹配方法，揭示了平滑、细粒度的分层对应关系。

Conclusion: HOT实现了更丰富、更可解释的表示比较，特别是在网络架构或深度不同的情况下，通过全局优化自然产生结构化模式。
...
TL;DR: 该研究比较了自监督学习模型ActiNet+HMM与传统随机森林+HMM在手腕加速度计数据上识别活动强度的性能，发现ActiNet模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 在大规模流行病学研究中，需要可靠准确的人类活动识别模型来研究体力活动与健康结果的关联，但现有模型性能有待提升。

Method: 使用151名CAPTURE-24参与者的数据，训练18层改进ResNet-V2自监督模型ActiNet，结合隐马尔可夫模型平滑，并与基线随机森林+HMM模型进行比较。

Result: ActiNet模型平均宏F1得分为0.82，Cohen's kappa得分为0.86，优于随机森林模型的0.77和0.81，且在不同年龄和性别亚组中表现一致。

Conclusion: 研究鼓励在未来的流行病学研究中使用ActiNet模型从手腕加速度计数据中提取活动强度标签。
...
TL;DR: 本文提出了一种无人机辅助的联邦多模态学习框架，通过联合优化无人机感知调度、功率控制、轨迹规划、资源分配和基站资源管理来最小化系统延迟，并提供收敛性分析。


<details>
  <summary>Details</summary>
Motivation: 利用无人机进行多模态数据收集和模型训练，克服单模态系统的局限性，提高模型精度和泛化能力，同时优化联邦多模态学习系统的延迟问题。

Method: 采用块坐标下降和逐次凸逼近技术的迭代优化算法，联合优化无人机感知调度、功率控制、轨迹规划、资源分配和基站资源管理。

Result: 数值实验表明，该框架在不同数据设置下，在系统延迟和模型训练性能方面优于现有方法。

Conclusion: 提出的无人机辅助联邦多模态学习框架能有效降低系统延迟，提高模型性能，并为非凸损失函数下的收敛性提供了理论保证。
...
TL;DR: BD Attention (BDA) 是基于基分解的注意力算法重构，实现了无损的注意力加速，无需重新训练即可减少25%的权重和32%的KV投影时间，对模型性能影响可忽略不计。


<details>
  <summary>Details</summary>
Motivation: 注意力机制是大型语言模型和视觉语言模型的核心操作，现有优化方法如FlashAttention主要关注I/O层面的系统优化，缺乏数学上保证的无损算法级加速方案。

Method: 利用基分解的矩阵恒等式，将多头投影重构为紧凑形式，保持精确输出，提供架构无关的数学保证加速。

Result: 在DeepSeek-V2-Lite (16B, FP16)上，仅需4秒离线准备，无需重新训练，在GPU上实现32%的KV投影加速和25%的权重减小，困惑度仅增加0.02%(FP16)或0.0004%(FP32)。

Conclusion: BDA是首个理论上精确的无损注意力加速方法，与现有工程级优化互补，为注意力机制提供了数学保证的算法级加速方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒TD学习的收敛保证仅限于表格MDP或依赖限制性折扣因子假设，需要填补鲁棒RL算法实证成功与理论保证之间的关键差距。

Method: 结合双时间尺度随机逼近更新和外循环目标网络更新，针对总变差距离和Wasserstein距离不确定性集设计鲁棒TD学习。

Result: 建立了Õ(1/ε²)的样本复杂度以获得ε-准确的价值估计，填补了鲁棒与非鲁棒对应算法之间的理论差距。

Conclusion: 该方法可相对直接地扩展到具有函数逼近的鲁棒Q学习，为分布鲁棒强化学习提供了有效的模型无关解决方案。
...
TL;DR: 本文提出使用深度神经网络(DNN)替代传统离散选择模型(DCM)来建模工作地点选择，DNN在某些方面表现优于DCM，但两种模型在不同场景下各有优势。


<details>
  <summary>Details</summary>
Motivation: 传统离散选择模型在准确反映个人决策过程方面存在挑战，需要探索更有效的方法来理解复杂的工作地点选择模式。

Method: 采用深度神经网络方法对工作地点选择进行建模，并与传统离散选择模型进行比较分析。

Result: DNN在工作地点选择建模中显示出作为DCM稳健替代方案的潜力。两种模型都能有效复制工作机会的影响，但DNN在某些方面表现更好，而DCM在评估个人属性对工作距离影响时与数据更吻合。

Conclusion: 在工作地点选择分析中，应根据具体应用需求选择合适的模型，DCM在短距离场景表现优异，DNN在长距离场景表现与DCM相当。
...
Method: 使用无监督动态特征选择方法，为每个实例识别并移除图像中的误导或冗余信息，确保只有最相关特征贡献到潜在空间。

Result: 在图像数据集上的实验表明，配备无监督DFS的模型在聚类和图像生成等任务中显著提高了泛化性能，计算成本增加很小。
...
TL;DR: 本文分析了差分隐私随机梯度下降(DPSGD)对模型公平性的影响，发现直接优化差分隐私模型的超参数虽然不能可靠缓解DPSGD的差异性影响，但相比重用非隐私模型的超参数能改善效用-公平性权衡。
...
Result: DPSGD对不同指标的差异性影响不一致；直接优化超参数不能可靠缓解差异性影响，但能改善效用-公平性权衡；DPSGD-Global-Adapt对超参数选择不稳健。

Conclusion: 超参数调优会带来额外的隐私泄露，需要谨慎平衡隐私、效用和公平性；DPSGD-Global-Adapt不是稳健的解决方案。
...
TL;DR: 提出了Granular-GRPO框架，通过奇异随机采样策略和多粒度优势集成模块，解决了现有方法中奖励信号稀疏和狭窄的问题，显著提升了流模型的偏好对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能有效探索高价值样本，但由于稀疏和狭窄的奖励信号，导致偏好对齐效果不佳。需要更精确和全面的奖励评估机制来改善流模型的强化学习效果。

Method: 1. 奇异随机采样策略：支持逐步随机探索，同时强制奖励与注入噪声之间的高相关性；2. 多粒度优势集成模块：聚合多个扩散尺度的优势计算，提供更全面鲁棒的采样方向评估。

Result: 在各种奖励模型上的实验表明，G²RPO显著优于现有的基于流的GRPO基线方法，证明了其有效性和鲁棒性。

Conclusion: G²RPO框架通过精确的奖励评估和全面的采样方向分析，成功解决了流模型强化学习中的偏好对齐问题，为生成模型与人类偏好的对齐提供了有效解决方案。
...
TL;DR: 本文收集并统一了图像逆问题中各种学习正则化方法的代码实现，通过系统比较揭示了不同方法的优缺点，并提供了实用指南。
...
Method: 通过收集和统一现有代码到一个共同框架中，建立统一的视角来系统比较不同方法。

Result: 统一框架允许系统比较各种方法，突出了它们的优势和局限性，为未来潜力提供了有价值的见解。

Conclusion: 该研究填补了学习正则化方法比较的空白，提供了实用的指导方针和未来发展方向。
...
TL;DR: Octax是一个基于JAX的高性能经典街机游戏环境套件，为强化学习研究提供GPU加速的Atari替代方案，实现数量级的速度提升并保持原始游戏机制完美保真。


<details>
  <summary>Details</summary>
Motivation: 强化学习研究需要多样化、具有挑战性且可扩展的环境。现代视频游戏虽然动态丰富，但计算成本高且不适合大规模实验。

Method: 基于CHIP-8仿真在JAX中实现经典街机游戏环境，提供端到端的GPU替代方案，支持图像环境，涵盖解谜、动作和策略类型。

Result: JAX实现相比传统CPU仿真器实现数量级加速，同时保持原始游戏机制完美保真。在多个游戏中训练RL代理显示训练速度和可扩展性的显著改进。

Conclusion: Octax是大型强化学习实验的理想平台，其模块化设计使研究人员能够轻松扩展套件或使用大语言模型生成新环境。
...
TL;DR: 提出Anchored Posterior Sampling (APS)方法，用于预训练离散扩散基础模型的后验采样，解决了现有方法在梯度引导、连续松弛和维度灾难方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型为分类数据（如文本和图像）提供了统一框架，具有更快推理、更精细控制和无需训练即可进行贝叶斯推理的优势，特别适合后验采样。但现有方法面临梯度引导信号稀疏、连续松弛限制应用和维度灾难等问题。

Method: 引入APS方法，基于两个关键创新：在离散嵌入空间中使用量化期望实现类梯度引导，以及使用锚定重掩码进行自适应解码。

Result: 在标准基准测试中，在线性和非线性逆问题上实现了离散扩散采样器的最先进性能，并在无需训练的风格化和文本引导编辑中展示了优势。

Conclusion: APS方法成功克服了离散扩散后验采样的关键挑战，为预训练扩散模型的后验采样提供了有效解决方案。
...
TL;DR: 该论文提出两种训练策略来解决学习非规范哈密顿动力学时的数值不稳定性问题，通过直接学习向量场或通过方案学习时间离散动力学。
...
TL;DR: 提出了一种名为概念神经元选择（CNS）的新方法，用于在持续学习场景下更新扩散模型，通过识别与目标概念相关的神经元并进行增量微调，有效缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中增量更新扩散模型具有实用性但计算挑战大，需要解决灾难性遗忘问题并保持零样本文本到图像生成能力。
...
Conclusion: CNS是一种简单而有效的持续学习方法，能够在保持模型原有能力的同时实现高效的概念个性化。
...
TL;DR: 对胸部X光合成数据应用后处理隐私过滤管道的严格评估显示，现有过滤器特异性有限且不一致，仅对真实图像具有高敏感性，但无法可靠检测训练数据生成的近似重复样本。
...
Result: 当前过滤器表现出有限的特异性和一致性，仅对真实图像实现高敏感性，但无法可靠检测从训练数据生成的近似重复样本。

Conclusion: 后处理过滤方法存在关键局限性，可能提供虚假的安全感，同时仍暴露不可接受的患者信息水平。在敏感应用中自信部署这些方法之前，需要过滤器设计的实质性改进。
...
Method: 丢弃传统模型的时间条件动力学，学习隐式能量景观的平衡梯度，在推理时采用基于优化的采样过程，使用可调节步长、自适应优化器和自适应计算。

Result: 在ImageNet 256×256上达到1.90 FID，超越扩散/流模型的生成性能，并能处理部分噪声图像去噪、OOD检测和图像合成等任务。

Conclusion: EqM通过统一平衡景观替代时间条件速度，为流模型和基于能量的模型之间提供了更紧密的桥梁，实现了优化驱动的推理路径。
...
TL;DR: 提出了宽-窄-宽（沙漏）MLP块，与传统窄-宽-窄设计相反，在扩展维度上使用跳跃连接，在窄瓶颈中进行残差计算，实现了更好的性能-参数帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 挑战传统MLP的窄-宽-窄设计惯例，探索在扩展维度上进行跳跃连接、在窄瓶颈中进行计算的替代架构。

Method: 设计沙漏MLP块，初始投影将输入提升到扩展维度，该投影可在训练期间保持随机初始化不变，通过系统架构搜索评估性能-参数帕累托前沿。

Result: 沙漏架构在生成任务上始终优于传统设计，随着参数预算增加，最优配置倾向于更深的网络、更宽的跳跃连接和更窄的瓶颈。

Conclusion: 研究结果表明需要重新考虑现代架构中跳跃连接的位置，这种设计模式可扩展到Transformer和其他残差网络。
...
TL;DR: Sparse Query Attention (SQA)通过减少查询头的数量来降低注意力机制的计算复杂度，在长序列处理中可实现高达3倍的吞吐量提升，且对模型质量影响极小。


<details>
  <summary>Details</summary>
Motivation: 传统多头注意力机制(MHA)的二次计算复杂度限制了长序列处理的可扩展性，现有方法如MQA和GQA虽然解决了内存带宽瓶颈，但未减少注意力得分计算所需的FLOPs。

Method: 提出稀疏查询注意力(SQA)架构，通过减少查询头数量而非键/值头来直接降低注意力计算复杂度，并提供了理论基础和架构变体。

Result: 在32k-200k长序列基准测试中，SQA在计算受限场景下可实现高达3倍的吞吐量提升，且初步小规模实验中模型质量影响很小。

Conclusion: SQA为构建更高效和可扩展的模型提供了强大工具，是在开发Reactive Transformer架构时偶然发现的创新方法。
...
TL;DR: 提出了一种顺序不变强化学习框架，用于黑盒组合优化，通过随机生成顺序训练自回归生成模型，提高样本效率和搜索空间多样性。
...
Conclusion: 该方法通过顺序不变性有效提升了组合优化的样本效率和性能，在广泛基准测试中表现优异。
...
TL;DR: 该论文提出了一种结合传统模型和LLM代理的预选择方法，通过数据集描述和统计信息来缩减AutoML搜索空间，显著降低计算开销的同时仍能选择最佳模型。


<details>
  <summary>Details</summary>
Motivation: 现有的AutoML方法依赖耗尽的超参数搜索，需要自动训练和测试不同模型，而预选择预测作为一种有前景的替代方案，能够通过智能预选模型来绕过这种搜索，但在文献中仍未被充分探索。

Method: 利用传统模型和大型语言模型(LLM)代理，基于数据集描述和统计信息来缩减AutoML库的搜索空间。该方法应用于AWS AutoGluon投资组合数据集，这是一个包含175个表格分类数据集的最先进AutoML基准。

Result: 提出的方法显著减少了计算开销，同时仍然能够为给定数据集选择最佳模型。

Conclusion: 该方法为AutoML工作流程提供了转变，通过预选择模型来优化搜索过程，在保持性能的同时大幅降低计算成本。
...
Result: 在行业启发的检索任务中，CNML在跨模态和模态内设置中显著优于算法和神经基线方法。学习到的表示能有效迁移到下游任务并泛化到更复杂的公式。
...
Conclusion: LieNLSD是首个能够确定非线性项无穷小生成元数量及其显式表达式的有效方法，在复杂对称性发现任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决从最小经验中学习新任务的问题，通过有效复用先前任务的知识来实现元学习，特别关注组合式解决方案

Method: 学习一个生成模型来捕获任务族共享的基础组件及其统计信息，将学习新任务转化为概率推理问题，通过高度约束的假设测试寻找解决方案
...
<details>
  <summary>Details</summary>
Motivation: 扩展经典的在线凸优化框架，处理在线对抗约束问题，在成本和约束函数都由对手任意选择且约束函数可能没有共同可行点的最一般情况下提供更好的性能保证。

Method: 通过将约束学习问题转化为具有特殊构造代理成本函数的标准在线凸优化问题实例来解决。

Result: 获得了改进的通用动态遗憾和累积约束违反边界，在对抗性约束设置下优于现有最优结果。

Conclusion: 提出的模块化算法框架能够有效处理最一般的在线对抗约束优化问题，为约束在线学习提供了新的解决方案。
...
TL;DR: 本文分析了梯度空间的动态特性，发现大部分梯度能量集中在小的子空间中，但仍有显著部分存在于残差空间中。基于这些洞察，提出了GrassWalk和GrassJump随机算法，在LLaMA模型预训练中实现了最先进的内存节省和性能提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练常受限于极端内存需求，其中优化器状态占主导地位。现有方法通过将梯度投影到低维子空间来降低内存成本，但需要更深入理解梯度空间动态特性。

Method: 分析了梯度空间及其子空间的动态特性，发现核心子空间影响力随时间推移和网络深度增加而减弱，且梯度空间呈现近平坦曲率。基于这些洞察，开发了GrassWalk和GrassJump随机算法，利用子空间特性进行优化。

Result: 提出的算法在LLaMA-1B和LLaMA-7B预训练中实现了最先进的内存节省，同时提升了模型性能。

Conclusion: 梯度空间分析揭示了重要的动态特性，基于这些洞察设计的随机算法能有效解决大语言模型训练中的内存瓶颈问题，在保持性能的同时显著降低内存需求。
...
TL;DR: 提出了多边际时间薛定谔桥匹配(MMtSBM)方法，用于从非配对数据中生成视频，通过因子化方式将迭代马尔可夫拟合算法扩展到多个边际，能够恢复高维静态数据中的隐藏动态过程。


<details>
  <summary>Details</summary>
Motivation: 许多自然动态过程只能通过静态样本快照观察，重建其时间演化以理解动态特性对科学研究很重要。现有方法在时间轴上传输数据但高维可扩展性差且需要严格假设。

Method: 扩展扩散薛定谔桥匹配的理论保证和实证效率，通过因子化方式将迭代马尔可夫拟合算法推导到多个边际，建立多边际薛定谔桥作为恢复静态数据中隐藏动态的实用原则方法。

Result: 在玩具示例上保留理论特性，在真实世界数据集如100维转录组轨迹推断上达到最先进性能，首次在非常高维图像设置中恢复耦合和动态。

Conclusion: 多边际薛定谔桥是恢复静态数据中隐藏动态的实用和原则性方法，为从静态观察中理解动态过程提供了有效工具。
...
TL;DR: 提出了一种基于注意力Transformer的多模态基础模型，整合电子健康记录、医学影像、遗传学和可穿戴设备数据，用于早期疾病诊断。
...
Method: 使用专用编码器将各模态数据映射到共享潜在空间，通过多头注意力和残差归一化进行融合，支持多任务预训练。
...
TL;DR: 本文探索了Tsetlin Machine（TM）在大规模多通道图像分类中的应用，提出了一种生成局部解释和全局类别表示的方法，在保持可解释性的同时实现了与深度学习模型相竞争的性能。


<details>
  <summary>Details</summary>
Motivation: Tsetlin Machine作为一种基于有限状态自动机和命题逻辑的机器学习范式，相比神经网络具有更好的可解释性。本文旨在探索TM架构在大规模多通道（RGB）图像分类中的适用性。
...
Conclusion: TM在复杂大规模训练环境中仍能保持可解释性，同时实现有竞争力的性能。这有助于更好地理解TM子句，并为将TM应用于更复杂多样的数据集提供了见解。
...
TL;DR: 该论文提出了RoGRAD框架，通过检索增强生成和迭代图对比学习来解决图神经网络在复合缺陷下的性能问题，相比传统GNN和LLM增强方法有显著改进。
...
Method: 提出RoGRAD框架，采用检索增强生成(RAG)技术，通过迭代图对比学习注入类别一致且多样化的增强数据，将LLM增强从静态信号注入转变为动态精炼过程。

Result: 在广泛实验中，RoGRAD显著优于传统GNN和LLM增强基线方法，平均改进达到82.43%。

Conclusion: RoGRAD是首个迭代式LLM增强框架，通过动态精炼机制有效提升了图神经网络在复合缺陷下的鲁棒性，挑战了LLM增强始终优于传统方法的假设。
...
Method: 使用三因子分解U、S、V⊤，约束U和V在Stiefel流形上保持正交性，采用几何优化设计将任何欧几里得优化器转换为黎曼优化器。

Result: 在常识推理、数学和代码生成、图像分类和图像生成等广泛的下游任务中，该方法优于当前最先进的LoRA变体。

Conclusion: 几何感知的LoRA扩展方法能够有效学习子空间，同时保持与现有微调管道的兼容性，在多个任务上表现出优越性能。
...
TL;DR: 本文针对多类别对抗鲁棒分类问题，推导了任意损失函数下鲁棒风险最小化问题的对偶和重心重构，特别针对交叉熵、幂形式和二次损失提供了显式表征，揭示了对抗鲁棒性与α-公平打包问题及广义重心问题的联系。


<details>
  <summary>Details</summary>
Motivation: 现有对抗鲁棒性研究主要关注0-1损失，缺乏对更广泛损失函数（如交叉熵）的理论分析，限制了鲁棒分类器的设计和风险评估。

Method: 通过数学推导获得鲁棒风险最小化问题的对偶和重心重构，特别针对交叉熵、幂形式和二次损失提供显式表征，并建立与α-公平打包问题和广义重心问题的理论联系。

Result: 获得了对抗风险的尖锐下界，在数值实验中用交叉熵损失函数得到了更紧致的下界，验证了理论结果的有效性。

Conclusion: 该工作扩展了对抗鲁棒性理论框架，为超越0-1损失设置的鲁棒分类器设计提供了理论基础和计算工具，揭示了对抗鲁棒性与优化问题的深层联系。
...
TL;DR: 提出了Moon框架，一种基于监督模态转换的多变量时间序列异常检测方法，通过将数值时间序列转换为图像表示，结合多模态CNN和SHAP解释器，显著提升了检测效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有MTS异常检测方法面临三个主要挑战：无监督方法依赖误差阈值导致不准确；半监督方法未充分利用异常标签；监督方法难以捕捉局部关系且计算成本高。

Method: 1. 引入多元马尔可夫转移场(MV-MTF)将数值时间序列转换为图像表示；2. 使用多模态CNN集成数值和图像数据；3. 基于SHAP的异常解释器识别关键变量。

Result: 在6个真实世界MTS数据集上的实验表明，Moon在效率上提升达93%，准确性提升4%，解释性能提升10.8%，优于6种最先进方法。
...
TL;DR: 该论文将传统的后处理模型校准方法（如直方图分箱和温度缩放）引入联邦学习环境，解决了在客户端异构性和差分隐私约束下的模型校准问题。
...
Method: 将集中式校准方法（直方图分箱和温度缩放）转移到联邦环境，设计了在强客户端异构性下的新操作方法。

Result: 研究发现联邦温度缩放最适合差分隐私联邦学习，而加权分箱方法在不需要差分隐私时表现最佳。

Conclusion: 该工作成功将后处理模型校准技术整合到联邦学习中，并提出了缓解异构性下性能下降的策略。
...
TL;DR: PepCompass是一个几何感知的肽探索和优化框架，通过定义解码器诱导的黎曼流形族来捕获局部几何结构，提出局部探索方法，在体外验证中发现了25种高活性抗菌肽。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型在抗菌肽发现中忽略了解码器诱导的几何结构，依赖平坦欧几里得度量，导致探索和优化失真低效。现有流形方法假设固定内在维度，在实际肽数据中严重失效。

Method: 提出PepCompass框架：1) 定义κ-稳定黎曼流形族的联合体；2) 二阶黎曼布朗高效采样；3) 切空间突变枚举；4) 局部枚举贝叶斯优化；5) 势最小化测地线搜索。

Result: 体外验证证实有效性：测地线搜索产生4个新种子，局部枚举贝叶斯优化发现25种高活性肽，具有广谱活性，包括对抗耐药菌株。
...
Method: GraphNC包含两个组件：1) 异常分数分布对齐(ScoreDA) - 将模型异常分数与教师模型分数分布对齐；2) 基于扰动的正态性正则化(NormReg) - 在表示空间通过扰动一致性损失使正常节点表示更紧凑。

Result: 该方法通过分数对齐使正常和异常类别的异常分数更可分离，同时通过表示正则化减轻教师模型不准确分数的影响。

Conclusion: GraphNC框架能够有效校准图正态性，在半监督图异常检测中取得更好的性能。
...
TL;DR: 提出了一种针对表格数据的对比学习框架，通过策略性选择正对样本和使用监督/自监督对比学习，显著减少偏见，同时保持预测准确性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在日常生活中的普及，开发公平无偏的模型变得至关重要。虽然表示学习在去偏方面已被证明有效，但表格数据中学习公平表示的问题仍未充分探索。

Method: 设计专门的对比学习框架，通过策略性选择正对样本，结合监督和自监督对比学习来学习表格数据的公平表示。

Result: 与现有最先进的表格数据对比学习模型相比，显著减少了偏见，同时在准确性和下游任务性能上保持最小权衡。

Conclusion: 该框架有效缓解了表格数据中的偏见问题，证明了学习公平表示在保持模型性能的同时提升公平性的可行性。
...
TL;DR: 该论文对密集连接深度神经网络进行数学建模，在深层极限下分析其学习问题，证明了从网络学习问题到连续时间对应问题的收敛性。


<details>
  <summary>Details</summary>
Motivation: 密集连接已成为深度神经网络的关键设计原则，但缺乏严格的数学分析。本文旨在为密集连接DNNs提供数学基础，理解其训练稳定性。

Method: 提出密集非局部（DNL）框架，将密集连接网络建模为非线性积分方程，从最优控制角度研究训练问题，使用分段线性扩展和Γ-收敛分析。

Result: 证明了最优值的收敛性和最小化子的子序列收敛性，表明密集连接架构可以提供深度模型训练的稳定性。

Conclusion: 密集连接DNNs可以建模为非线性积分方程，该数学框架为理解此类网络提供了理论基础，并揭示了其训练稳定性优势。
...
TL;DR: AMF-VI是一种自适应混合流变分推断方法，通过异质混合互补流模型（MAF、RealNVP、RBIG）的两阶段训练，在多种后验分布上实现更稳健的变分推断。


<details>
  <summary>Details</summary>
Motivation: 单一流模型在不同质量分布上表现不一致，需要开发能适应多种后验分布形状的稳健变分推断方法。

Method: 两阶段训练：1）顺序专家训练单个流模型；2）基于似然的全局权重自适应估计，无需逐样本门控或架构修改。

Result: 在六种典型后验分布上，AMF-VI相比单一流基线获得更低的负对数似然，在Wasserstein-2距离和最大均值差异指标上表现更稳定。

Conclusion: 自适应混合多样化流模型为跨不同后验分布的稳健变分推断提供了可靠途径，同时保留了每个专家的归纳偏置。
...
TL;DR: PPGen是一个生物物理模型，将PPG信号与可解释的生理和光学参数联系起来，通过混合摊销推理(HAI)实现从PPG信号中快速、稳健地估计生理参数。
...
Method: 引入PPGen生物物理模型，并提出混合摊销推理(HAI)方法，从PPG信号中估计生理参数并校正模型错误设定。

Result: 在广泛的模拟实验中，HAI能够在不同噪声和传感器条件下准确推断生理参数。

Conclusion: PPG模型可以在保持深度学习特征保真度的同时，支持临床解释和硬件设计。
...
TL;DR: 本文提出了一种通过最大似然估计对Flow Matching进行微调的方法，以解决训练-推理差距问题，在图像生成和机器人操作任务中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: Flow Matching在训练阶段无法评估模型输出，存在训练-推理差距，特别是在需要高精度的机器人操作场景中。此外，FM对直线预定义路径的过度追求可能引入刚度等问题。

Method: 提出基于最大似然估计的微调方法，包括直接微调和基于残差的微调。通过特定架构设计，残差微调可以融入收缩特性，增强模型的鲁棒性和可解释性。

Result: 在图像生成和机器人操作任务上的实验结果表明，该方法能可靠地提升FM的推理性能。

Conclusion: 通过最大似然估计微调FM模型可以有效缩小训练-推理差距，提高模型在精确任务中的表现，特别是在机器人操作领域具有重要价值。
...
TL;DR: KAIROS是一个非自回归时间序列预测框架，通过直接建模分段级多峰分布，实现实时决策支持，在保持高性能的同时大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: Web应用需要快速响应的时间序列预测来支持实时决策，如资源规划和异常响应，而现有方法存在误差累积或预测过度平滑的问题。

Method: 采用非自回归设计，直接建模分段级多峰分布，避免误差累积，实现即时推理。

Result: 在六个广泛使用的基准测试中表现出强大的零样本泛化能力，性能与最先进的基础模型相当，但推理成本显著降低。

Conclusion: KAIROS展示了非自回归设计作为时间序列基础模型可扩展范式的重要性，为实时Web应用提供了高效的预测解决方案。
...
TL;DR: 本文提出了一种无需精心构建模型库的权重空间学习方法，直接从Hugging Face等非结构化模型仓库中学习权重表示，克服了传统方法对大型模型库的依赖。


<details>
  <summary>Details</summary>
Motivation: 传统权重空间学习需要大量精心构建的模型库，这需要大量计算资源且限制了表示学习的规模和灵活性。本文旨在摆脱这一限制，直接从异构模型仓库中学习权重表示。

Method: 提出新的权重空间骨干网络，专门设计用于处理非结构化模型群体，直接从Hugging Face等异构模型仓库中训练权重空间表示。

Result: 在Hugging Face模型上训练的权重空间表示表现出色，通常优于在实验室生成模型库上训练的骨干网络，且能够泛化到未见过的数据模态。

Conclusion: 高质量权重空间表示可以在非结构化环境中学习，精心构建的模型库并非不可或缺，这克服了权重空间学习社区当前面临的重要限制。
...
TL;DR: 提出了PENEX（惩罚指数损失），一种新的多类指数损失公式，理论上有依据且适合一阶方法优化，在计算机视觉和语言任务中表现出比现有方法更好的正则化效果。


<details>
  <summary>Details</summary>
Motivation: AdaBoost在实践中随着弱学习器数量增加而泛化良好，但其指数损失对误分类数据点惩罚过重。现有多类指数损失公式不适合一阶方法优化。

Method: 引入PENEX损失函数，理论证明其隐式最大化数据点边界，梯度增量隐式参数化boosting框架中的弱学习器。
...
<details>
  <summary>Details</summary>
Motivation: 伊朗作为世界第二大天然气资源国，面临人口增长和能源消耗增加导致的冬季气压下降和天然气供应中断问题，需要控制居民部门的天然气消费。
...
Conclusion: 机器学习方法，特别是混合模型，可有效管理和预测天然气消费，有助于更高效的资源管理和减少季节性短缺，地理和气候因素在预测建模中具有重要影响。
...
TL;DR: 提出一个端到端框架，用于在大规模空间匹配任务中实现精确的召回率控制，通过神经网络排序器和多方法集成显著降低阈值方差。


<details>
  <summary>Details</summary>
Motivation: 在空间匹配任务中，精确控制召回率至关重要，传统置信区间方法存在超调和方差大的问题，需要更稳定的解决方案。

Method: 使用边界框过滤和CSR候选表示减少配对枚举，训练轻量级神经网络排序器，通过分位数分层校准集和多阈值估计器集成来降低方差。

Result: 在两个真实地籍数据集上，该方法能稳定达到召回目标，减少冗余验证，并在单个TPU核心上高效运行。

Conclusion: 该框架实现了精确的召回控制，显著优于传统方法，为大规模空间匹配任务提供了可靠解决方案。
...
TL;DR: DECOR是一个用于线性高斯结构方程模型的单步似然估计器，联合学习DAG和相关噪声模型，在存在潜在混杂的情况下实现结构学习。


<details>
  <summary>Details</summary>
Motivation: 现有连续方法在误差独立时表现良好，而去混杂先行的流程依赖于普遍因子结构或非线性。需要一种能处理潜在混杂的统一方法。

Method: 提出DECOR估计器，交替进行平滑无环图更新和凸噪声更新，可包含轻量弓形互补惩罚或后验调和步骤。

Result: 在合成基准测试中，DECOR匹配或优于强基线方法，特别在混杂非普遍时表现稳健，在普遍混杂下仍保持竞争力。

Conclusion: DECOR为存在潜在混杂的线性高斯SEMs提供了一个有效的联合学习框架，具有理论保证和实际性能优势。
...
<details>
  <summary>Details</summary>
Motivation: 开发经济高效且高性能的催化剂对于加速可再生能源采用和确保稳定能源供应至关重要，特别是在氢能存储领域。
...
TL;DR: 将扩散模型中的分类器自由引导方法扩展到经典策略梯度方法，提出策略梯度引导(PGG)，通过插值条件分支和无条件分支实现无需重新训练的行为调节。


<details>
  <summary>Details</summary>
Motivation: 将扩散模型中成功的引导技术应用到强化学习中，实现无需重新训练的可控行为调节。

Method: 在策略梯度中增加无条件分支，通过插值条件分支和无条件分支实现引导，理论证明在优势估计下归一化项消失。

Result: 在离散和连续控制基准测试中，条件丢弃在简单离散任务和低样本情况下有效，适度引导训练能提高稳定性、样本效率和可控性。

Conclusion: 引导技术可以成功应用于标准在线策略方法，为可控在线强化学习开辟新方向。
...
TL;DR: 提出了Flatness-Aware SGLD (fSGLD)方法，通过随机权重扰动隐式捕获曲率信息，有效寻找平坦最小值，在保持SGD计算成本的同时提升泛化性能。
...
Method: 使用随机权重扰动(RWP)评估随机梯度，优化随机平滑目标函数，隐式捕获曲率信息，通过适当耦合逆温度和扰动尺度来正则化Hessian迹。

Result: 理论证明fSGLD的平稳测度集中在全局最小化器附近，实验在噪声标签和大规模视觉任务中显示优于或可比基线算法，Hessian谱分析确认收敛到更平坦最小值。

Conclusion: fSGLD在保持SGD计算效率的同时，能有效寻找平坦最小值，提升泛化能力和鲁棒性，为随机权重扰动的益处提供了严格理论解释。
...
<details>
  <summary>Details</summary>
Motivation: 传统逆强化学习方法产生的"黑盒"奖励模型难以解释和调试，需要开发能够生成可解释、可验证的奖励函数的方法。

Method: GRACE方法结合大型语言模型和进化搜索，直接从专家轨迹中逆向工程出基于代码的可执行奖励函数。

Result: 在BabyAI和AndroidWorld基准测试中，GRACE高效学习了高度准确的奖励函数，即使在复杂的多任务设置中也能表现良好。生成的奖励函数能够产生强大的策略，性能优于竞争性的模仿学习和使用真实奖励的在线强化学习方法。

Conclusion: GRACE能够生成可解释的代码奖励函数，在多任务设置中构建复杂的奖励API，为逆强化学习提供了透明且可验证的解决方案。
...
TL;DR: 该论文介绍了2025年PhysioNet挑战赛，旨在开发从心电图识别恰加斯病的算法方法，利用多个数据集和增强技术来提高模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 恰加斯病是一种寄生虫感染，在拉丁美洲和美国流行，慢性阶段会导致心血管疾病。由于血清学检测能力有限，但恰加斯心肌病常在心电图中表现，因此需要开发算法来优先安排患者进行检测和治疗。

Method: 利用多个数据集（包含弱标签和强标签），进行数据增强以提高模型鲁棒性，并采用评估指标来模拟当地血清学检测能力，将机器学习问题构建为分诊任务。

Result: 挑战赛吸引了来自111个团队的630多名参与者，提交了1300多个参赛作品，代表了全球学术界和工业界的多样化方法。

Conclusion: 该挑战赛通过心电图识别恰加斯病的算法开发，为解决该疾病检测能力有限的问题提供了创新方法，并促进了全球研究合作。
...
TL;DR: Poolformer是一种序列到序列模型，用循环层和池化操作替代自注意力机制，解决了长序列处理中自注意力二次复杂度的问题，在音频处理任务中表现优于现有最先进模型。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制在长序列处理中存在二次复杂度问题，限制了其实际应用。研究者希望开发一种更高效的序列建模方法，特别是在处理音频等自然具有长序列的数据时。

Method: 使用SkipBlocks递归定义模型结构，包含残差块、下采样池化层、嵌套SkipBlock、上采样池化层和额外残差块。用循环层替代自注意力，通过池化操作减少序列长度。

Result: 池化操作显著加速训练，改善感知指标（FID和IS），防止过拟合。在原始音频处理任务中，Poolformer超越了SaShiMi和Mamba等最先进模型。深层处理长程依赖，浅层处理短期特征。

Conclusion: Poolformer为长序列建模提供了高效解决方案，未来可扩展到文本、视觉和多模态应用，基于Poolformer的大语言模型有望有效处理图像和视频的密集表示。
...
TL;DR: StockBench是一个专门评估LLM智能体在股票交易中表现的基准测试，通过多个月的模拟交易环境测试模型的交易决策能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准主要测试静态知识问答，无法捕捉交易的动态迭代特性，而金融领域对经济价值和决策风险至关重要。

Method: 构建无污染基准，让智能体接收每日市场信号（价格、基本面、新闻），做出买卖持有决策，使用金融指标评估表现。

Result: 大多数LLM智能体难以超越简单的买入持有基准，但部分模型显示出获得更高回报和更好风险管理的能力。

Conclusion: 静态金融知识能力不一定转化为成功的交易策略，StockBench为可复现研究提供开源资源。
...
TL;DR: DiFFPO是一个统一框架，通过强化学习训练掩码扩散大语言模型，使其推理能力更强且速度更快。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散大语言模型训练方法存在似然性难以处理的问题，且缺乏对推理效率的优化。

Method: 提出基于离策略强化学习的代理策略训练，采用两阶段似然近似和重要性采样校正，并联合训练高效采样器/控制器。

Result: 在数学和规划基准任务上，相比仅训练模型，实现了更好的准确性和更低的函数评估次数，改进了推理计算效率的帕累托前沿。

Conclusion: DiFFPO框架通过强化学习有效提升了扩散大语言模型的推理质量和效率，为模型优化提供了新方向。
...
TL;DR: 提出C2AL方法，通过分析数据子结构和利用部分冲突的辅助标签来正则化共享表示，解决推荐系统中用户群体异质性导致的模型偏向中心分布、忽略头尾区域的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据由具有不同条件分布的异质群体组成，而大规模推荐模型在单一全局目标下训练会假设用户群体同质性。随着模型规模和复杂度增加，模型会偏向主导分布模式，忽视头尾区域，限制学习能力并导致注意力权重不活跃或神经元死亡。

Method: 分析数据集子结构，通过辅助学习暴露具有强烈分布对比的子结构。利用部分冲突的辅助标签来正则化共享表示，定制注意力层学习过程以保持与少数群体的互信息，同时提升全局性能。

Result: 在包含数十亿数据点的生产数据集上评估六个SOTA模型，实验显示提出的方法能够捕捉细粒度的用户-广告交互，整体归一化熵降低达0.16%，在目标少数群体上收益超过0.30%。

Conclusion: 注意力机制在因子分解机中可发挥共享嵌入选择的关键作用，通过分析数据子结构和利用冲突辅助标签的方法能有效解决推荐系统中的分布不平衡问题，提升模型对少数群体的学习能力。
...
TL;DR: 本文研究了基于扩散变换器的时序数据插补方法的统计效率，推导了样本复杂度界限，并分析了缺失模式对插补性能的影响。


<details>
  <summary>Details</summary>
Motivation: 扩散生成式插补方法虽在实践中表现优异，但其在捕捉缺失值与观测值间复杂时空依赖关系的理论理解仍有限，需要从统计效率角度进行理论分析。

Method: 基于条件扩散变换器，推导了条件得分函数的近似理论，建立了样本复杂度界限，并通过混合掩码训练策略验证理论发现。

Result: 获得了紧密的缺失值置信区域，发现插补效率和准确性受缺失模式显著影响，混合掩码训练能提升性能。

Conclusion: 扩散变换器在时序数据插补中具有统计效率保证，缺失模式是关键影响因素，混合掩码训练是有效的优化策略。
...
TL;DR: 提出一种基于copula的方法，能够从现有的多步时间序列基础模型中高效生成具有相关性的预测样本路径，替代昂贵的自回归采样。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型只能预测每个时间步的独立边际分布，无法生成具有现实相关性结构的完整联合预测分布，而自回归采样成本极高。

Method: 使用copula方法，在单次前向传递中从多步时间序列基础模型生成具有相关性的样本路径。

Result: 该方法比自回归采样快几个数量级，并通过减轻误差累积现象提高了样本路径质量。

Conclusion: copula方法能够高效生成准确且具有相关性的样本路径，解决了现有模型在联合预测分布生成方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然Transformer一直是主导架构，但xLSTM等替代方案在保持竞争力的同时提供了线性复杂度。研究两者的扩展行为可以为未来模型设计和部署提供指导。

Method: 采用IsoFLOP和参数拟合方法，在80M-7B模型规模和2B-2T训练token范围内研究xLSTM的扩展行为；考察最优模型大小对上下文长度的依赖性；分析推理时扩展特性。

Result: xLSTM在典型LLM训练和推理场景中比Transformer扩展更有利，且随着上下文长度的增加，xLSTM的优势更加显著。

Conclusion: xLSTM在长上下文场景中具有更好的扩展性，这为未来大语言模型架构选择提供了重要参考。
...
TL;DR: 提出了一种基于正无标签学习的异常检测方案PUL-Inter-Slice Defender，用于防御5G网络中的分布式切片移动性攻击，在受污染训练数据下仍能保持高检测性能。
...
Method: 结合长短期记忆自动编码器和K-Means聚类的正无标签学习方法，利用3GPP关键性能指标和性能测量计数器作为特征来检测攻击变种。

Result: 在基于free5GC和UERANSIM的5G测试平台上评估，当训练数据包含10%到40%攻击污染时，F1分数超过98.50%，优于其他对比方法。

Conclusion: PUL-Inter-Slice Defender能够有效检测分布式切片移动性攻击，在受污染训练数据环境下表现稳健，为5G网络安全提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习优化器如Muon每步更新所有层，但作者认为这种全网络更新在理论和实践上都可能是次优的，需要探索更高效的训练策略。

Method: 提出Drop-Muon框架，每步随机选择部分层进行更新，结合渐进式训练效率和层特定的非欧几里得更新，提供收敛性理论保证。

Result: 在CNN实验中，Drop-Muon始终优于全网络更新的Muon，达到相同精度时wall-clock时间快1.4倍，成本分析显示全网络更新仅在特定层平滑度关系下最优。

Conclusion: 研究结果表明需要改变大规模模型训练方式，Drop-Muon为全网络更新提供了高效且理论可靠的新选择。
...
TL;DR: ExGRPO通过识别推理经验价值指标（正确性和熵），组织并优先利用有价值的经验，使用混合策略目标平衡探索与经验利用，显著提升推理性能并稳定训练。


<details>
  <summary>Details</summary>
Motivation: 标准的在线策略训练在单次更新后丢弃经验，导致计算效率低下和不稳定性。现有研究未充分探索经验特性对大型推理模型学习动态的影响。

Method: 提出ExGRPO框架：识别经验价值指标（正确性和熵），组织并优先利用有价值的经验，采用混合策略目标平衡探索与经验利用。

Result: 在五个骨干模型（1.5B-8B参数）上，ExGRPO在数学/通用基准上平均比在线策略RLVR提升3.5/7.6分，并在强弱模型上都能稳定训练。
...
TL;DR: 本文研究了纯Transformer架构在分子建模中的潜力，发现无需预定义图结构或物理先验，Transformer可以直接在笛卡尔坐标上训练，达到与最先进等变图神经网络相当的分子能量和力预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统的图神经网络(GNNs)在分子机器学习中依赖预定义的图结构(如固定半径截断或k近邻)，这种硬编码的图结构限制了表达能力(由于固定感受野)并降低了推理速度。本文旨在探索纯Transformer是否能在没有预定义图或物理先验的情况下近似分子能量和力。

Method: 使用未经修改的标准Transformer架构，直接在笛卡尔坐标上训练，不依赖预定义的图结构或物理先验。在OMol25数据集上与最先进的等变GNN在匹配的训练计算预算下进行比较。

Result: Transformer在分子能量和力预测上达到了与最先进等变GNN相当的竞争性平均绝对误差。Transformer学会了物理一致的注意力模式(如注意力权重随原子间距离反比衰减)，并能灵活适应不同分子环境。标准Transformer还展现出可预测的训练资源缩放改进。

Conclusion: Transformer能够自适应地学习到GNNs的许多有利特性，挑战了硬编码图归纳偏置的必要性，为分子建模指出了标准化、可扩展架构的方向。
...
TL;DR: 使用强化学习来对抗动态反应式干扰，通过自适应调整传输功率、调制方式和信道选择来优化吞吐量。
...
<details>
  <summary>Details</summary>
Motivation: RF信号传播建模对于环境理解和无线网络诊断至关重要，但复杂环境中的精确预测仍具挑战性，因为信号会与障碍物发生吸收和反射等相互作用。

Method: 提出Diffusion^2方法，使用RF-3D编码器从3D数据中捕获RF相关特征，并通过多尺度嵌入模拟实际RF信号传播过程。

Result: 在合成和真实世界测量中的评估显示，Diffusion^2在各种频率带和环境条件下准确估计RF信号行为，误差仅为1.9dB，比现有方法快27倍。

Conclusion: 该方法在RF信号传播建模领域取得了显著进展，能够高效准确地预测复杂环境中的RF信号行为。
...
TL;DR: 提出了一个更完整、现实和具有挑战性的交通预测基准数据集，包含两个主要城市的道路网络，最大数据集包含近10万个路段，并提出了可扩展的图神经网络方法。


<details>
  <summary>Details</summary>
Motivation: 当前交通预测基准数据集存在显著缺陷：缺少道路连接信息、道路属性信息有限、路段数量不足，且主要关注城际高速公路而非更具挑战性的城市道路网络。

Method: 发布包含丰富道路特征和细粒度交通数据的城市道路网络数据集，并提出不使用专用时间序列处理模块的图神经网络方法，以提高可扩展性。

Result: 创建了包含近10万个路段的数据集（比现有数据集增加10倍以上），提出的方法在可扩展性和预测性能方面都优于当前主流时空模型。

Conclusion: 该数据集和建模方法为交通预测研究提供了有价值的资源，能够支持更全面的交通预测系统开发。
...
TL;DR: 本文指出当前LLM不确定性估计方法评估存在缺陷，提出使用多种风险指标和Elo评分来改进评估的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法评估存在严重不一致性，容易被人为操纵性能表现，需要更鲁棒的评估方法。

Method: 提出使用多种替代风险指标进行风险相关性实验，包括在QA任务中对多个LLM-as-a-judge变体进行边际化，以及探索结构化任务、分布外和扰动检测任务。

Result: 多LLM-as-a-judge变体的边际化能减少评估偏差，结构化任务和扰动检测任务提供更鲁棒可控的风险指标。

Conclusion: 使用多种风险指标和Elo评分系统能够为NLG不确定性估计算法提供更客观和鲁棒的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在多轮交互场景下容易受到对抗攻击，现有方法主要依赖人工红队测试或预定义模板，未能充分探索多轮攻击的复杂对话动态和策略规划

Method: 将对话建模为序列决策问题，结合强化学习和树搜索，无需人工标注数据，系统性地探索多轮攻击策略

Result: 在10个目标模型上攻击成功率比现有最优方法提高25.9%以上，并能发现新的攻击策略

Conclusion: DialTree-RPO框架能有效发现多轮对话攻击漏洞，为AI安全评估提供了自动化解决方案
...
TL;DR: 提出了Interactive Training框架，允许在神经网络训练过程中进行实时反馈驱动干预，通过控制服务器实现用户或AI代理与训练过程的动态交互。
...
Method: 使用控制服务器作为中介，允许用户动态调整优化器超参数、训练数据和模型检查点，支持人类专家或AI代理的实时干预。
...
TL;DR: 提出知识蒸馏检测任务，旨在判断学生模型是否从给定教师模型蒸馏而来，仅使用学生模型权重和教师API。开发模型无关框架，结合无数据输入合成和统计分数计算，适用于分类和生成模型。


<details>
  <summary>Details</summary>
Motivation: 解决模型来源验证和未经授权通过蒸馏复制的担忧，保护模型知识产权。

Method: 使用模型无关框架，通过无数据输入合成生成样本，计算统计分数来检测蒸馏关系。

Result: 在CIFAR-10上检测准确率提升59.6%，ImageNet上提升71.2%，文本到图像生成任务提升20.0%。

Conclusion: 提出的知识蒸馏检测方法能有效识别模型间的蒸馏关系，为模型来源验证提供实用工具。
...
TL;DR: 该研究为扩散模型成功归因于其适应数据低维几何结构的能力提供了理论和实证证据，通过分析分数匹配中的隐式正则化效应，表明平滑分数函数会产生沿数据流形的平滑效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在多个领域表现出卓越的泛化能力，但其机制尚不完全清楚。基于流形假设，研究者推测这种成功源于模型适应数据低维几何结构的能力，本文旨在为此提供证据。

Method: 通过研究经验分数匹配目标最小化器的平滑效应，分析隐式正则化的作用。理论分析和实证检验平滑分数函数（或等价地在对数密度域平滑）的效果。

Result: 理论和实证结果证实，平滑分数函数会产生沿数据流形的切向平滑。此外，研究表明通过选择适当的平滑方法可以控制扩散模型泛化所沿的流形。

Conclusion: 该工作为扩散模型成功源于适应数据低维几何结构的假设提供了支持证据，揭示了分数匹配中隐式正则化如何促进沿数据流形的平滑泛化。
...
<details>
  <summary>Details</summary>
Motivation: 传统LPCA方法在高噪声环境下因邻域大小选择困难而表现不佳，需要先验的几何和噪声特征知识

Method: 通过正交化图拉普拉斯低频特征向量的梯度来估计切空间，利用数据的全局结构指导局部切空间估计

Result: LEGO方法在噪声环境下比LPCA显著更稳健，在流形学习、边界检测和局部本征维度估计等下游任务中表现优异
...
TL;DR: KaVa框架通过从教师的压缩KV缓存中蒸馏知识到潜在推理学生，解决了潜在推理缺乏监督的问题，结合了思维链的准确性和潜在推理的效率。


<details>
  <summary>Details</summary>
Motivation: 思维链推理计算成本高且包含冗余信息，而潜在推理缺乏有效监督，限制了其在复杂自然语言推理中的效果。

Method: 通过自蒸馏方法，利用压缩KV缓存作为监督信号，将教师模型的KV轨迹与潜在推理学生对齐，使用连续潜在令牌表示灵活性。

Result: KaVa在潜在推理基准上表现优于强基线，从方程到自然语言推理的退化更小，可扩展到更大骨干网络同时保持效率。
