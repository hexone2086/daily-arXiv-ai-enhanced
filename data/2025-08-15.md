...
Conclusion: SPFF能有效聚焦食物类别特征，过滤无关图像块，提升分类准确性。
...
TL;DR: DINOv3通过数据扩展、Gram anchoring和后期优化策略，提出了一种通用的视觉基础模型，无需微调即可在多种任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 自监督学习无需人工标注，能从多样数据中学习通用视觉表示，DINOv3旨在实现这一目标。

Method: 结合数据扩展与模型规模优化，引入Gram anchoring解决密集特征退化问题，并采用后期策略增强模型灵活性。

Result: DINOv3在多种视觉任务中表现优异，超越现有自监督和弱监督模型。

Conclusion: DINOv3为视觉任务提供了通用且高性能的解决方案，推动了自监督学习的发展。
...
Method: 使用对比语言-图像预训练（CLIP）进行零样本评估，设计了10种文本提示，并结合预训练神经网络进行分析。

Result: 在多种变形生成技术和媒介下，该方法实现了通用化的变形攻击检测，并能预测最相关的文本片段。

Conclusion: 多模态学习方法在变形攻击检测中表现出色，结合文本提示提升了检测的可解释性和泛化能力。
...
Conclusion: 该方法高效准确，为身体组成与健康关系研究提供了新工具和新假设。
...
TL;DR: 本文提出了一种新的多模态融合方法MANGO，通过可逆交叉注意力层和三种新的交叉注意力机制，实现了显式、可解释的多模态学习。
...
Method: 提出可逆交叉注意力（ICA）层和三种新交叉注意力机制（MMCA、IMCA、LICA），并开发基于归一化流的多模态模型。

Result: 在语义分割、图像到图像翻译和电影类型分类三个任务上取得了最先进的性能。

Conclusion: MANGO方法通过显式建模多模态特征关联，显著提升了多模态学习的性能和可解释性。
...
TL;DR: 研究探讨了结合真实与合成图像提升西瓜病害分类模型性能的方法，发现少量真实图像与大量合成图像的混合使用显著提高了模型表现。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型为农业计算机视觉提供了新的图像生成方式，但缺乏对真实与合成图像结合效果的研究。

Method: 使用EfficientNetV2-L模型，比较五种数据集处理方式（纯真实、纯合成、不同比例混合等），评估分类性能。

Result: 混合数据集（H3-H4）显著提升了F1分数至1.00，表明少量真实图像与大量合成图像的结合效果最佳。

Conclusion: 合成图像不能完全替代真实图像，两者混合使用可最大化模型性能。
...
Method: 提出了一种可扩展的合成数据生成框架，结合参数高效微调（PEFT）方法，用于提升VLMs和目标检测器（如YOLO和DETR）的性能。
...
TL;DR: 3D高斯泼溅（3DGS）作为一种新兴的视图合成方法，具有快速训练/渲染和高质量视觉效果的优点。本文提出了一种名为EntropyGS的因子化和参数化熵编码方法，用于压缩3DGS高斯属性，实现了约30倍的码率降低，同时保持相似的渲染质量。
...
Method: 首先对3DGS高斯属性进行相关性和统计分析，发现球谐AC属性服从拉普拉斯分布，而旋转、缩放和不透明度可用高斯混合分布近似。随后提出EntropyGS方法，通过估计每个属性的分布参数进行熵编码，并根据属性类型自适应量化。

Result: EntropyGS在基准数据集上实现了约30倍的码率降低，同时保持与输入3DGS数据相似的渲染质量，且编码和解码速度快。

Conclusion: EntropyGS是一种高效的3DGS高斯属性压缩方法，适用于存储和传输场景，具有显著的码率降低和快速的编码/解码性能。
...
TL;DR: 本文综述了深度学习在裂缝检测中的最新趋势，包括学习范式的转变、泛化能力的提升和数据多样化的进展，并提出了新的3DCrack数据集和基准测试。
...
Method: 系统分析了学习范式、泛化能力和数据多样化的趋势，并引入3DCrack数据集进行基准测试。

Result: 总结了代表性工作，建立了深度学习方法的基准，并提供了未来研究方向。

Conclusion: 本文为裂缝检测领域的方法演进和未来方向提供了见解。
...
TL;DR: MRFD方法通过多区域融合解码减少LVLMs的幻觉，提升事实性，无需模型更新。


<details>
  <summary>Details</summary>
Motivation: LVLMs在多模态任务中表现优异，但常因视觉信息验证能力有限而产生与输入不一致的幻觉文本。

Method: 提出MRFD方法，利用交叉注意力识别显著区域，生成初始响应，基于JSD计算可靠性权重，并通过一致性感知融合预测。

Result: 实验表明MRFD显著减少幻觉并提升事实性。

Conclusion: MRFD是一种无需训练的解码方法，有效提升LVLMs的事实性。
...
TL;DR: 论文研究了基于外观的视线估计（PoG）在个体差异和头部姿态变化下的鲁棒性问题，提出了动态校准策略以提升估计器的性能。


<details>
  <summary>Details</summary>
Motivation: 由于个体差异，现有的PoG估计器需要针对个人进行校准，但校准后的估计器对头部姿态变化敏感。本文旨在探索影响校准估计器的关键因素，并提出鲁棒的校准策略。

Method: 构建MobilePoG基准数据集，分析校准点和头部姿态多样性对估计精度的影响，并提出动态校准策略，用户通过移动手机引入头部姿态变化。

Result: 实验表明，校准过程中引入更广泛的头部姿态能提升估计器对姿态变化的鲁棒性。动态校准策略比传统方法更有效。

Conclusion: 动态校准策略在用户友好且高效的过程中提升了PoG估计器的鲁棒性，减少了对头部姿态变化的敏感性。
...
Result: 在COCO-2014数据集上验证，CLIP Score、FID和SSIM指标表现优异，有效平衡语义对齐与结构保真度。
...
TL;DR: 该论文提出了JRDB-Reasoning基准，通过定义推理复杂性、自适应查询引擎和扩展数据集，解决了现有视觉推理基准的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理基准缺乏对推理复杂性的明确定义、任务定制能力以及结构化的工作流程注释，限制了视觉语言模型在复杂环境中的评估。

Method: 论文通过形式化推理复杂性，引入自适应查询引擎生成可定制问题，并扩展JRDB数据集以包含人-物交互和几何关系注释。

Result: 创建了JRDB-Reasoning基准，支持对视觉推理框架的细粒度评估和跨推理级别的动态评估。

Conclusion: 提出的方法和基准填补了视觉推理领域的空白，为复杂环境中的视觉语言模型评估提供了新工具。
...
TL;DR: 提出了一种基于相位一致性加权最小绝对偏差（PCWLAD）的子像素模板匹配方法，用于提高多模态光学图像的匹配精度。


<details>
  <summary>Details</summary>
Motivation: 多模态光学图像匹配因不同光谱响应导致的非线性辐射和几何形变差异而精度下降，需改进匹配方法。

Method: 方法分为粗匹配（使用SSIM）和精细匹配（使用WLAD），结合相位一致性和结构滤波技术。

Result: 在三种数据集上测试，PCWLAD在匹配正确率和RMSE上优于现有八种方法，平均匹配精度约0.4像素。
...
Method: InterSyn包含两个模块：INS（交错交互合成模块）联合建模单人及交互行为，REC（相对协调优化模块）优化角色间的动态同步。

Result: 实验表明，InterSyn生成的运动序列在文本-运动对齐性和多样性上优于现有方法，为运动合成设定了新标准。

Conclusion: InterSyn通过联合学习策略提升了交互运动的真实性和协调性，未来将开源代码以推动进一步研究。
...
Conclusion: OoD分割对安全关键应用至关重要，未来研究需解决现有挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统生成模型通常使用替代目标（如似然或重构损失）训练，这些目标与感知质量、语义准确性或物理真实性不一致。RL提供了一种优化不可微分、偏好驱动和时间结构化目标的框架。

Method: 论文系统回顾了RL在图像、视频和3D/4D生成中的方法，包括RL作为微调机制和结构组件的角色。
...
TL;DR: SpaRC-AD是一种基于查询的端到端相机-雷达融合框架，通过稀疏3D特征对齐和多普勒速度估计，显著提升了自动驾驶任务中的性能。
...
Method: 采用相机-雷达融合框架，结合稀疏3D特征对齐和多普勒速度估计，优化3D场景表示。

Result: 在3D检测、多目标跟踪、在线地图、运动预测和轨迹规划等任务中显著超越纯视觉基线。

Conclusion: SpaRC-AD在安全关键场景中表现出色，验证了雷达融合在精确运动理解和长时程轨迹预测中的有效性。
...
Method: 提取基准测试中的概念/技能，确定其偏向（概念或技能），选择匹配度最高的指令进行训练。
...
TL;DR: Glo-DMU是一种基于深度学习的肾小球超微结构量化框架，可同时分析三种关键超微结构特征，提升肾脏病理诊断效率。
...
Method: 提出Glo-DMU框架，包含三个深度模型：超微结构分割、滤过屏障区域分类和电子致密物检测模型。

Result: 在115名患者的真实诊断场景中验证，结果与病理报告一致，具备高精度和高通量特性。

Conclusion: Glo-DMU为肾脏病理学家提供了高效、自动化的辅助工具，可同时量化多种超微结构特征。
...
<details>
  <summary>Details</summary>
Motivation: 解决历史文献和现代手写体的字符识别问题，提升OCR技术的准确性和适用性。
...
Conclusion: 研究为OCR技术提供了新思路，并指出了未来研究方向。
...
TL;DR: AtomDiffuser是一个时间感知的降解建模框架，用于分离STEM数据中的样本漂移和辐射衰减，提升原子分辨率下材料动态的建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以明确分离时间分辨STEM数据中的空间漂移和辐射损伤导致的信号损失，限制了原子分辨率下材料动态的准确建模。

Method: 提出AtomDiffuser框架，通过预测STEM帧间的仿射变换和空间变化衰减图，将样本漂移和辐射衰减解耦。

Result: AtomDiffuser在合成和真实cryo-STEM数据上表现良好，支持高分辨率降解推断和漂移对齐。

Conclusion: AtomDiffuser为可视化与量化辐射诱导的原子不稳定性相关降解模式提供了工具，推动了原子分辨率下材料动态的研究。
...
TL;DR: 论文提出了一种新方法，通过直接提示多模态视觉语言模型（VLMs）来判断不同频率下的模式可见性，以估计其对比敏感度函数（CSF），评估其与人类感知的一致性。


<details>
  <summary>Details</summary>
Motivation: 评估VLMs与人类感知的一致性，以了解它们如何感知低级视觉特征，尤其是对比敏感度函数（CSF）。

Method: 使用带通滤波噪声图像和多样化提示，直接提示VLMs判断不同对比度下的模式可见性，模拟心理物理学实验。
...
TL;DR: 论文提出了一种通过联合生成图像和其对应的内在场景属性（如深度、分割图）的方法，以解决现有图像生成模型在空间一致性和布局上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型依赖图像-文本对或单独使用内在属性作为条件输入，导致生成图像空间不一致和失真。本文利用内在场景属性提供丰富的底层结构信息。

Method: 从预训练估计器中提取内在场景属性，通过自编码器聚合为单一潜在变量，并在预训练的潜在扩散模型（LDM）基础上联合去噪图像和内在属性域。

Result: 实验表明，该方法能纠正空间不一致性，生成更自然的场景布局，同时保持基础模型（如Stable Diffusion）的保真度和文本对齐性。

Conclusion: 联合生成图像和内在属性能隐式捕捉场景结构，生成更一致和真实的图像。
...
Method: 提出 NSegment+，仅对标签施加弹性变形而保持图像不变，以学习对标签不一致性的鲁棒表示。

Result: 在多个数据集上平均 mIoU 提升显著（最高 +3.39），且可与其他训练技巧结合进一步优化。
...
TL;DR: 提出了一种新方法，通过修改文本嵌入空间中的delta向量来抑制扩散模型中与特定词汇强关联的内容，并进一步优化为选择性抑制方法（SSDV）。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像（T2I）扩散模型中特定词汇与内容强关联的问题，例如生成“查理·卓别林”时无法避免“胡子”的出现。

Method: 引入delta向量修改文本嵌入，削弱不想要内容的影响；提出SSDV方法，将delta向量应用于交叉注意力机制；优化delta向量以支持个性化T2I模型。
...
Conclusion: SC-Lane通过动态坡度特征融合和时间一致性优化，实现了3D车道检测的先进性能。
...
TL;DR: NanoControl提出了一种高效的DiT可控文本到图像生成方法，通过LoRA风格控制模块和KV-Context增强机制，显著减少计算开销。
...
Method: 采用实例相关损失、自适应残差语义提示框架和对比学习语义一致性损失，以及增量提示初始化策略。
...
Method: 采用协作师生网络（CoSD-TSNet）和渐进式自调训练策略（PaST），结合伪标签分配器（PLA）。

Result: 在DroneVehicle数据集上优于现有方法。

Conclusion: DOD-SA通过单模态标注和跨模态知识迁移，显著提升了检测性能。
...
TL;DR: 论文提出了一种自动化识别电气布局图中符号的方法，并发布了标注数据集DELP，开发了轻量级工具SkeySpot，性能最佳模型为YOLOv8（mAP 82.5%）。


<details>
  <summary>Details</summary>
Motivation: 传统建筑电气布局图多为扫描文档，缺乏机器可读性，导致大规模解析耗时且易错。自动化符号识别可提升效率，支持成本估算、维护和合规性检查。

Method: 构建了DELP数据集（45份电气布局图，标注2450个实例，34类符号），并基于预训练目标检测模型（如YOLOv8）进行系统评估。

Result: YOLOv8表现最佳（mAP 82.5%），开发的SkeySpot工具支持实时检测、分类和量化电气符号，输出标准化结果。

Conclusion: 该方法降低了企业对CAD系统的依赖，减少人工标注，提升中小企业在建筑行业中的数字化能力，支持标准化和可持续发展。
...
<details>
  <summary>Details</summary>
Motivation: 探索人类视觉感知是否源于图像统计，以及生物启发模型是否能在无监督下学习感知度量。

Method: 使用PerceptNet进行端到端优化，完成自编码、去噪、去模糊和稀疏正则化等任务。

Result: 编码器阶段（V1-like层）与人类感知判断高度相关，且在适度噪声、模糊和稀疏性下表现最佳。

Conclusion: 视觉系统可能针对特定失真和稀疏性水平进行优化，生物启发模型无需人类监督即可学习感知度量。
...
Method: TS-Mamba通过构建视频轨迹选择相似token，采用轨迹感知移位Mamba聚合模块（TSMA）进行信息聚合，并设计移位SSMs块补偿扫描损失。

Result: 在三个VSR测试数据集上，TS-Mamba在多数情况下优于六种基准模型，计算复杂度降低22.7%。

Conclusion: TS-Mamba结合轨迹建模和Mamba，显著提升了在线VSR的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习分割对标注数据的高度依赖问题，尤其是在脑MRI等体积图像中手动标注费时费力。

Method: 1. 自动分箱体素强度生成合成图像训练初始模型；2. 使用卷积AE评估未标注数据的预测质量；3. 选择高质量伪标签微调网络。

Result: 在分布外数据上，性能接近使用更多标注图像训练的模型，且AE质量评估优于一致性评估。

Conclusion: 领域随机化与AE质量控制结合，为极有限标注数据的半监督分割提供了有效策略，减轻标注负担。
...
TL;DR: 本文评估了三种雷达数据处理方法（DBSCAN、匈牙利算法和卡尔曼滤波）在毫米波雷达动作识别中的性能，分析了它们的单独及组合效果，并提出了改进方案。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达在隐私保护的动作识别中具有潜力，但数据稀疏且噪声多，现有方法缺乏全面评估。

Method: 使用MiliPoint数据集，单独及组合评估三种方法，并提出改进措施。

Result: 分析了每种方法的准确性和计算成本，展示了组合方法的优势与权衡。

Conclusion: 研究结果为未来毫米波雷达动作识别系统的设计提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: STAS是肺腺癌的一种新型侵袭模式，与肿瘤复发和生存率降低相关，但其诊断因病理特征复杂且易被忽视而具有挑战性，亟需深度学习辅助。

Method: 研究整合了多中心数据集，采用双分支架构和Transformer编码，结合多模式注意力模块动态筛选STAS相关区域，并通过相似性正则化约束减少特征冗余。

Result: STAMP在STAS-SXY、STAS-TXY和STAS-TCGA数据集上的AUC分别为0.8058、0.8017和0.7928，优于临床水平。

Conclusion: STAMP框架在多中心数据集中表现出色，为STAS诊断提供了高效且准确的深度学习解决方案。
...
Method: TweezeEdit通过梯度驱动正则化，在一致性模型的指导下，直接路径注入目标提示语义。

Result: 实验表明，TweezeEdit在语义保留和目标对齐上优于现有方法，仅需12步（1.6秒/次编辑）。

Conclusion: TweezeEdit高效且适用于实时应用，为图像编辑提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D高斯溅射在场景优化中缺乏足够的几何约束，导致高频纹理和锐利不连续区域的细节模糊。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅基于第一人称视频或音乐预测舞蹈动作，而结合两者的研究较少。第一人称视角常遮挡身体，且需动作与音乐同步，增加了任务难度。

Method: 提出EgoMusic Motion Network，核心为Skeleton Mamba模型，结合扩散模型和Mamba技术，显式捕捉人体骨骼结构。

Result: 实验表明，该方法明显优于现有技术，并能有效泛化到真实数据。

Conclusion: 新方法通过结合视觉和音乐输入，显著提升了舞蹈动作预测的准确性和泛化能力。
...
TL;DR: 该综述论文系统分析了视觉推理的五大类型（关系、符号、时序、因果和常识推理），并探讨了相关架构、评估协议及其局限性，同时提出了未来研究方向。
...
Result: 总结了评估协议的局限性（如泛化性、可重复性和解释性），并提出了视觉推理领域的关键开放挑战。

Conclusion: 未来研究应关注感知与推理的结合，以构建透明、可信且跨领域适应的人工智能系统，特别是在自动驾驶和医疗诊断等关键领域。
...
TL;DR: 论文提出了一种医学图像定位方法Med-GLIP，并构建了大规模数据集Med-GLIP-5M，解决了现有研究中模态覆盖不足、标注粒度粗和缺乏统一框架的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像定位是智能诊断、视觉问答和报告生成的基础任务，但现有研究受限于模态覆盖不足、标注粒度粗和缺乏统一框架。

Method: 构建了包含530万区域标注的Med-GLIP-5M数据集，并提出模态感知的Med-GLIP框架，通过隐式学习层级语义理解。

Result: Med-GLIP在多个基准测试中表现优于现有方法，其空间输出显著提升了下游任务（如医学VQA和报告生成）的性能。

Conclusion: Med-GLIP和Med-GLIP-5M为医学图像定位提供了统一的解决方案，并在实验中验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中显著目标检测存在目标尺度变化大和背景对比度低的问题，现有方法难以有效整合全局和局部特征。

Method: 采用VSS编码器提取多尺度特征，设计DS-HGAM模块增强跨层交互和结构感知，LEVSS解码器结合自适应扫描和MCAEM模块提升局部建模能力。

Result: 实验证明GCRPNet在显著目标检测任务中达到最先进性能。

Conclusion: GCRPNet通过整合全局和局部特征，显著提升了遥感图像中显著目标检测的效果。
...
Conclusion: PSScreen通过双流学习和特征对齐，有效解决了多数据集训练的挑战，显著提升了疾病筛查性能。
...
TL;DR: 研究提出了一种基于AR的手术导航新方法，用于模拟脑室外引流导管放置，通过实时工具跟踪提高精度。
...
Result: 实时工具跟踪显著提高了所有精度指标，用户主观评价也更偏好此方法。

Conclusion: 实时AR工具跟踪在手术导航中具有显著优势，未来可进一步优化应用。
...
Conclusion: RAP通过动态检索和提示增强，显著提升了OOD检测性能，验证了其模块的有效性。
...
TL;DR: 提出了一种名为PTQAT的混合量化方法，结合PTQ和QAT的优势，通过选择性微调关键层提升量化模型性能，同时减少计算开销。
...
Method: 选择关键层进行QAT微调，其余层采用PTQ，优先微调输出差异小的层以减少量化误差传播。

Result: 在3D感知任务中表现优于纯QAT，提升检测、分割和预测任务的性能，同时减少50%的微调层。

Conclusion: PTQAT是一种高效且通用的量化方法，适用于多种模型和任务，显著提升性能并减少计算成本。
...
TL;DR: HM-Talker提出了一种结合隐式和显式运动特征的混合运动表示方法，通过跨模态解缠模块和混合运动建模模块，生成高保真、时序一致的说话头部视频。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动说话头部视频生成方法因缺乏显式发音先验，常导致运动模糊和唇部抖动，影响视频质量。

Method: HM-Talker结合隐式和显式运动特征，使用跨模态解缠模块提取互补特征，并通过混合运动建模模块动态融合特征以实现身份无关学习。
...
TL;DR: 论文提出了一种基于SAM编码器的遥感变化检测方法，结合时空特征增强和多尺度解码器融合，并通过交叉熵掩码损失处理类别不平衡问题，在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 利用基础模型（如SAM）的通用表示能力，提升遥感变化检测任务的性能，尤其是在多尺度和类别不平衡场景下。

Method: 通过微调SAM编码器，结合时空特征增强（STFE）和多尺度解码器融合（MSDF），并提出交叉熵掩码（CEM）损失函数。

Result: 在Levir-CD、WHU-CD、CLCD和S2Looking四个数据集上优于现有方法，尤其在S2Looking数据集上F1分数提升了2.5%。

Conclusion: 该方法通过结合SAM的通用表示和提出的技术，显著提升了遥感变化检测的性能。
...
TL;DR: 论文提出了一种基于多模态代理的灵活方法，用于解决基于参考的视频对象分割任务，通过动态工作流和大型语言模型的推理能力，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要高计算复杂性和手动标注，而现有基于视觉语言基础模型的训练免费方法缺乏灵活性。

Method: 利用大型语言模型生成动态工作流，与多模态专用工具交互，逐步识别目标对象。

Result: 在RVOS和Ref-AVS任务上表现优于现有方法。

Conclusion: 多模态代理方法为动态任务提供了更灵活和自适应的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估框架缺乏对人类复杂意图理解和共情反馈的细粒度评估，阻碍了MLLMs的发展。

Method: 引入HumanSense基准，结合多模态输入（视觉、音频、文本），采用多阶段、模态渐进强化学习增强推理能力。

Result: 实验显示领先的MLLMs仍有改进空间，全模态模型在任务中表现更优，推理能力是关键。

Conclusion: 通过上下文分析和推理能力提升，可显著改善MLLMs的人类中心交互表现，且无需训练的提示设计也能提升非推理模型性能。
...
TL;DR: EvTurb提出了一种基于事件流的湍流去除框架，通过两步事件引导网络分别处理模糊和倾斜失真，并引入首个真实湍流数据集TurbEvent。
...
Conclusion: EvTurb有效解决了湍流失真问题，并提供了真实数据集支持。
...
TL;DR: 论文提出P$^3$A框架，针对自动驾驶中的2D目标检测，优化高分辨率数据集的黑盒攻击，引入新指标PASR和改进的损失函数LCSL，提升攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于迁移的黑盒攻击方法在自动驾驶场景中因评价指标和低分辨率数据的限制，攻击效果被高估，实际成功率低。

Method: 提出PASR指标衡量攻击效果，设计LCSL损失函数提升迁移性，并引入PSPP预处理保持高分辨率数据攻击效果。

Result: P$^3$A在未见模型和高分辨率数据集上优于现有攻击方法，验证了其有效性和实用性。

Conclusion: P$^3$A为自动驾驶系统的安全漏洞提供了更准确的评估和更有效的攻击方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统上采样方法（如子像素卷积）效率高但难以重建高频细节并易引入伪影，FGA旨在解决这些问题。

Method: FGA结合了基于傅里叶特征的多层感知机、跨分辨率相关注意力层和频域L1损失，以提升频谱保真度。

Result: 实验表明，FGA在五种超分辨率骨干网络上均表现优异，PSNR平均提升0.12~0.14 dB，频域一致性提升29%。
...
Method: FIND-Net采用快速傅里叶卷积（FFC）层和可训练高斯滤波，将MAR视为频域和空间域的混合任务。

Result: 在合成数据集上，FIND-Net显著优于现有MAR方法，MAE降低3.07%，SSIM增加0.18%，PSNR提高0.90%。

Conclusion: FIND-Net在临床CT扫描中表现出色，既能有效抑制伪影，又能最小化对干净解剖区域的修改，具有临床潜力。
...
TL;DR: 论文提出了一种名为Chamfer Guidance的训练自由引导方法，通过少量真实样本图像提升合成数据的多样性和质量，并在ImageNet-1k和地理多样性基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前条件图像生成模型在提升生成质量的同时牺牲了多样性，限制了其作为合成训练数据的实用性。现有方法未充分考虑合成数据与真实数据之间的分布偏移。

Method: 提出Chamfer Guidance，利用少量真实样本图像来表征合成数据的质量和多样性，无需训练即可实现引导。

Result: 在ImageNet-1k和标准地理多样性基准测试中，Chamfer Guidance在仅使用2张真实样本时达到96.4%的精确度和86.4%的分布覆盖率，使用32张样本时进一步提升至97.5%和92.7%。下游分类器训练中，准确率提升高达15%（分布内）和16%（分布外）。

Conclusion: Chamfer Guidance显著提升了合成数据的多样性和质量，同时减少了计算开销，为生成模型的实用性提供了有效解决方案。
...
TL;DR: ChatENV是一个交互式视觉语言模型，结合卫星图像和环境传感器数据，用于环境监测和推理。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉编码器对图像采集和处理的细微参数的敏感性，这些参数通常被忽视，但可能对模型性能产生重要影响。

Method: 分析图像采集过程中的参数和细微变换，研究其对视觉表示和语义预测的影响。

Result: 发现这些参数会被系统地编码到视觉表示中，且对语义预测有显著影响，具体效果取决于参数与语义标签的相关性。

Conclusion: 图像采集和处理的细微参数对视觉编码器的性能有重要影响，需在模型设计和评估中加以考虑。
...
<details>
  <summary>Details</summary>
Motivation: 传统跛行检测方法依赖手动设计的特征，效率低且不灵活。本研究旨在通过自动学习运动特征，提高检测准确性和适用性。

Method: 使用T-LEAP姿态估计模型提取奶牛行走视频中9个关键点的运动序列，输入BLSTM分类器进行二元跛行分类。

Result: 最佳架构的分类准确率达到85%，优于传统方法的80%，且仅需1秒视频数据即可检测。
...
TL;DR: 论文提出了一种名为SemPT的新框架，通过共享属性级知识解决视觉迁移学习中的泛化挑战，并在多个基准数据集上取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 视觉迁移学习在未见类别上存在挑战，现有方法依赖稀疏标签或分散的LLM生成描述，导致知识表示碎片化。

Method: SemPT采用两步提示策略提取共享视觉属性并生成属性级描述，通过视觉引导加权减少噪声，并联合对齐图像和文本嵌入。

Result: 在15个基准数据集上的实验表明，SemPT在多种设置下均达到最优性能。

Conclusion: SemPT通过属性级知识增强文本嵌入，平衡了已知类别的区分性和未知类别的可迁移性。
...
TL;DR: 论文提出UniBench300统一基准，解决多模态视觉目标跟踪任务中的训练与测试不一致问题，并通过序列化整合和持续学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多模态视觉目标跟踪任务中因数据传感器类型混合训练导致性能下降，缺乏统一基准。

Method: 引入UniBench300统一基准，采用序列化整合方式，结合持续学习优化性能。

Result: 实验表明UniBench300显著提升性能，持续学习支持稳定整合，性能下降与网络容量负相关。

Conclusion: UniBench300和持续学习方法有效解决多模态任务整合问题，为未来研究提供新方向。
...
TL;DR: AddressVLM通过结合卫星图像和街景图像，提升了大视觉语言模型（LVLM）在细粒度地址定位中的性能。


<details>
  <summary>Details</summary>
Motivation: LVLM在粗粒度地理定位（如国家或城市级别）表现优异，但在城市内部的细粒度街道级定位中表现不佳。

Method: 提出两阶段训练协议：跨视角对齐调整和地址定位调整，结合卫星图像和街景图像，并引入自动标签生成机制。
...
TL;DR: 本文提出了一种构建高质量人脸数据集的方法，用于训练人脸识别模型，通过混合专家策略清理数据、生成合成身份，并结合课程学习策略，最终在竞赛中获得第一名。
...
Method: 1. 清理HSFace数据集，使用混合专家策略（MoE）结合聚类和GPT-4o辅助验证；2. 生成合成身份，使用Stable Diffusion和Vec2Face扩展；3. 采用课程学习策略训练模型。

Result: 构建的数据集在竞赛中获得第一名，实验表明其在10K、20K和100K身份规模下均能提升模型性能。

Conclusion: 混合真实与合成数据的策略能高效构建高质量数据集，课程学习进一步优化模型训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测因目标尺寸小、强度弱和运动模式复杂而具有挑战性，现有方法仅建模低阶特征关联且局限于单一时间尺度。

Method: HyperTea包含三个模块：GTEM（全局时间增强）、LTEM（局部时间增强）和TAM（时间对齐模块），结合CNN、RNN和HGNN。
...
TL;DR: 提出了一种结合隐式神经表示和物理正则化的方法，用于胎儿脑MRI中的T2映射，减少了扫描时间并提高了运动鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 胎儿脑MRI中T2映射在0.55T中场强下具有潜力，但现有方法因多次采集和运动干扰导致效率低下。

Method: 联合重建多个回波时间（TE）的数据，结合隐式神经表示和物理正则化，实现信息共享并保持解剖和定量T2保真度。

Result: 在模拟胎儿脑和成人数据中表现优异，并首次在0.55T下实现胎儿T2映射。

Conclusion: 该方法减少了每个TE所需的扫描次数，展示了在胎儿脑MRI中的潜力。
...
TL;DR: 提出IADGPT框架，通过三阶段训练策略提升工业异常检测（FS-IAD）性能，结合语言输出实现异常定位与推理。
...
Method: 采用三阶段渐进训练策略，前两阶段学习基础工业知识和差异感知，第三阶段基于上下文学习提升泛化能力。

Result: IADGPT在异常检测、定位和推理任务中表现优异，实验验证其性能提升。

Conclusion: IADGPT通过模仿人类质检方式，显著提升FS-IAD任务效果，并开源数据集支持研究。
...
TL;DR: 论文提出了一种轻量级的视图转换框架TUNet，利用预训练扩散模型的高保真生成能力，从单张输入图像合成新视角图像，解决了现有方法模糊重建和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要微调大型扩散模型或从头训练，成本高且重建效果模糊、泛化能力差，因此探索一种轻量级框架以直接利用预训练模型的能力。

Method: 基于DDIM反演的潜在空间，使用相机姿态条件化的U-Net（TUNet）预测目标视图的潜在表示，并提出融合策略以保留纹理和细节。

Result: 在MVImgNet上的实验表明，该方法优于现有方法。

Conclusion: 提出的轻量级框架能高效利用预训练扩散模型，显著提升新视角合成的质量和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在动态范围上的限制导致复杂交通环境（如夜间驾驶、隧道）中的全局对比度下降和高频细节丢失，影响了目标检测的准确性。
...
Conclusion: MCFNet通过结合事件相机的高动态范围信息，有效解决了复杂光照条件下的目标检测问题，性能显著提升。
...
TL;DR: 论文提出CountCluster方法，通过聚类对象交叉注意力图来准确生成指定数量的对象，无需外部工具或额外训练。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成图像时难以准确反映输入提示中的对象数量，尤其是在早期去噪步骤中。

Method: CountCluster在推理时根据注意力分数将对象交叉注意力图划分为k个聚类，并优化潜在空间以对齐目标分布。

Result: 方法在对象计数准确性上平均提升18.5%，优于现有方法。

Conclusion: CountCluster有效解决了扩散模型中对象数量控制的问题，具有广泛适用性。
...
TL;DR: SegDAC是一种基于分割的视觉强化学习方法，利用SAM和YOLO-World实现对象中心分解和语义分割，通过Transformer架构动态选择关注的分割区域，显著提升了视觉泛化能力和样本效率。
...
Method: SegDAC结合SAM进行对象中心分解，YOLO-World通过文本提示语义分割，采用Transformer架构动态选择分割区域，无需人工标注。

Result: 在Maniskill3基准测试中，SegDAC在视觉泛化能力上表现优异，最难任务性能翻倍，样本效率超越或匹配现有方法。

Conclusion: SegDAC通过对象中心分割和动态选择机制，显著提升了视觉强化学习的泛化能力和效率。
...
TL;DR: 论文提出了一种用于实时处理未聚焦SAR数据的神经网络，以解决卫星资源限制问题，并验证了在FPGA上部署的可行性。


<details>
  <summary>Details</summary>
Motivation: 解决传统SAR数据处理算法在卫星上因资源限制而难以实时处理的问题，减少数据传输量。

Method: 设计并评估了用于实时推理的神经网络，处理Sentinel-1的Stripmap和IW模式数据。

Result: 验证了模型在FPGA上部署的可行性，并实现了船只与风力发电机的二分类任务。

Conclusion: 神经网络可在资源受限的卫星上实现实时SAR数据处理，为近实时监测提供可能。
...
Method: 引入表面模型（Surface Model）进行准确的鸟瞰图投影，并提出SimRefiner模块通过局部-全局残差校正优化相似性矩阵。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用token索引值训练自回归模型，忽略了代码本中丰富的token相似性信息。k-means聚类在代码本特征空间中表现不佳。

Method: 提出DCPE方法，采用基于实例的距离替代基于质心的距离，并通过凝聚合并技术解决token空间差异问题。
...
TL;DR: 论文提出了EgoCross基准测试，用于评估多模态大语言模型（MLLMs）在跨域场景下的表现，发现现有模型在非日常生活领域的泛化能力不足。


<details>
  <summary>Details</summary>
Motivation: 现有EgocentricQA研究局限于日常活动，而实际应用需应对视觉和语义差异显著的跨域场景，因此需要更全面的评估工具。

Method: 构建EgoCross基准，覆盖手术、工业、极限运动和动物视角四个领域，包含1,000个QA对和798个视频片段，支持OpenQA和CloseQA格式。

Result: 实验表明，现有MLLMs在跨域场景中表现不佳，泛化能力有限。

Conclusion: EgoCross为提升领域自适应和鲁棒性视频理解提供了基础，未来可通过微调和强化学习等方法改进。
...
TL;DR: 论文提出了一种名为ConGCD的新方法，通过分解视觉基元和跨知识比较来模拟人类认知过程，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法在泛化类别发现（GCD）中主要关注优化目标函数，而人类感知系统在识别新类别对象方面表现优异，因此需要一种更接近人类认知的解决方案。

Method: ConGCD通过高层语义重建建立基元导向的表征，并通过解构绑定类内共享属性。同时，采用主导和上下文共识单元捕捉类区分模式和分布不变性，动态优化激活路径。

Result: 在粗粒度和细粒度基准测试中，ConGCD表现出色，证明了其作为共识感知范式的有效性。

Conclusion: ConGCD通过模拟人类认知过程，提供了一种有效的泛化类别发现方法，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 评估合成数据训练的巩膜分割模型性能，探索隐私保护生物识别的可行性。

Method: 竞赛分两赛道：纯合成数据训练和混合数据训练，九组提交多样化模型设计。
...
Conclusion: 通过几何基元表示和匹配机制，显著提升了对称性检测的精度和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造技术带来了社会问题，现有检测方法在未知伪造技术上表现不佳，亟需具有强泛化能力的检测技术。

Method: 采用FGL策略捕捉已知与未知伪造技术的差异信息，并设计DPNet网络动态感知伪造痕迹的特征和关系。

Result: 实验表明，该方法在不同场景下泛化能力强，能有效应对未知伪造挑战。

Conclusion: FGL策略和DPNet网络为深度伪造检测提供了鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在高质量视频生成中表现优异，但其迭代去噪过程缓慢且长序列的二次注意力成本高昂，导致推理瓶颈。

Method: 提出BLADE框架，包括自适应块稀疏注意力机制（ASA）和基于轨迹分布匹配（TDM）的稀疏感知步蒸馏范式。

Result: 在CogVideoX-5B和Wan2.1-1.3B模型上，BLADE实现了最高14.10倍的推理加速，同时质量显著提升。

Conclusion: BLADE通过联合训练策略有效解决了视频生成中的效率和质量问题，为未来研究提供了新方向。
...
TL;DR: DARC是一种新型模型驱动的群配准框架，用于高效构建无偏且具有高解剖保真度的图谱，并支持多种应用。


<details>
  <summary>Details</summary>
Motivation: 解决数据驱动方法在群配准中依赖大数据集、泛化性差的问题，以及模型驱动方法在大规模3D数据中的可扩展性和优化挑战。

Method: 采用坐标下降策略和中心性激活函数，支持多种图像差异度量，高效处理任意数量的3D图像。

Result: DARC生成了无偏且高保真的图谱，并在一次性分割和形状合成应用中表现优异。

Conclusion: DARC提供了一种灵活、泛化性强且资源高效的图谱构建和应用框架。
...
### [88] [Mobile-Friendly Deep Learning for Plant Disease Detection: A Lightweight CNN Benchmark Across 101 Classes of 33 Crops](https://arxiv.org/abs/2508.10817)
...
TL;DR: 论文分析了主流视觉语言模型（VLMs）在时空物理推理任务上的不足，提出了一种结合监督微调和规则强化学习的方法，显著提升了模型性能，但泛化能力仍有限。
...
Method: 对主流VLMs进行全面诊断分析，发现其表现不佳；采用监督微调结合规则强化学习的方法改进Qwen2.5-VL-7B模型。

Result: 改进后的模型在时空物理推理任务上表现显著提升，超越主流专有模型，但泛化能力仍不足。

Conclusion: 尽管方法有效，模型对新物理场景的泛化能力有限，凸显了时空物理推理领域仍需新方法。
...
TL;DR: AEGIS是一个新的大规模基准数据集，专注于检测高度逼真和语义复杂的AI生成视频，包含10,000多个真实和合成视频，并提供多模态注释，以支持真实性检测和相关任务。


<details>
  <summary>Details</summary>
Motivation: 当前视频真实性检测的基准存在局限性，无法有效评估现代视觉语言模型对复杂伪造视频的检测能力。

Method: 构建AEGIS数据集，包含多样化的真实和合成视频，并设计具有挑战性的子集和多模态注释。

Result: 实验表明，现有模型在AEGIS最具挑战性的子集上检测能力有限，突显了其复杂性和逼真度。

Conclusion: AEGIS为视频真实性检测研究提供了关键基准，推动了更鲁棒、可靠的方法的发展。
...
TL;DR: 提出TriFlowSR框架，解决现有RefSR方法在信息对齐和数据集质量上的不足，并引入首个UHD地标场景RefSR数据集Landmark-4K。


<details>
  <summary>Details</summary>
Motivation: 现有基于ControlNet的RefSR方法难以有效对齐LR图像和参考HR图像的信息，且现有数据集分辨率低、质量差，导致参考图像缺乏细节。

Method: 提出TriFlowSR框架，设计Reference Matching Strategy以实现LR图像与参考HR图像的模式匹配，并构建Landmark-4K数据集。
...
TL;DR: 提出了一种基于用户交互的新型视频活体检测方法，通过控制面部靠近摄像头的动作结合光流分析，显著提高了对真实面部和攻击的区分能力。


<details>
  <summary>Details</summary>
Motivation: 现有活体检测方法在区分真实面部与多种攻击（如打印照片、屏幕显示、面具和视频重放）时效果有限，需要更可靠的方法。

Method: 设计了一种用户交互场景，要求参与者缓慢将面部靠近摄像头，结合光流分析和神经网络分类器处理RGB帧和光流数据。

Result: 该方法通过提取面部体积信息，显著提高了活体检测的准确性，优于被动方法。

Conclusion: 提出的交互式光流分析方法为活体检测提供了更可靠的解决方案。
...
TL;DR: VasoMIM是一种针对X射线血管造影的自监督学习框架，通过整合血管解剖知识改进传统MIM的局限性，显著提升血管分割性能。
...
TL;DR: 本文提出了一种名为OF-Diff的方法，通过提取对象先验形状和双分支扩散模型，显著提高了遥感图像生成的保真度和多样性。
...
Result: OF-Diff在多个关键质量指标上优于现有方法，特别是多态和小目标类别的性能显著提升，如飞机、船只和车辆的mAP分别提高了8.3%、7.7%和4.0%。

Conclusion: OF-Diff通过改进生成对象的保真度和多样性，为高精度遥感图像生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 提升UI代理在定位和导航任务中的性能，减少对大量训练数据的依赖。

Method: 采用强化微调（RFT）和Qwen2.5-VL模型，设计奖励函数和数据清理策略，并提出自进化轨迹历史对齐与稀疏动作增强方法。

Result: 7B和72B模型在Screenspot-V2/Pro基准上分别达到94.1%/50.8%和95.3%/61.9%，在AndroidWorld导航任务中分别达到49.1%和65.9%成功率。

Conclusion: UI-Venus为开源社区提供了SOTA模型、数据清理协议和自进化框架，推动了UI代理研究的进一步发展。
...
Method: 采用教师-学生范式，多基线输入，教师预测用于监督学生，并引入遮挡感知注意力图。

Result: 在KITTI 2015和2012基准测试中表现优于现有自监督方法，提升了遮挡和非遮挡区域的预测。
...
TL;DR: 论文提出AdaptFED，改进TransFed框架，通过任务感知客户端嵌入和低秩超网络条件化，提升联邦学习的适应性和可扩展性。
...
Method: 引入任务感知客户端嵌入和低秩超网络条件化，优化焦点调制层。

Result: 在八个数据集上验证了方法的优越性，尤其在源无关和跨任务联邦学习中。

Conclusion: AdaptFED扩展了焦点调制在联邦学习中的应用，为更自适应、可扩展的Transformer联邦系统铺平道路。
...
TL;DR: PhysHPO提出了一种分层跨模态直接偏好优化框架，通过四个层次的粒度优化视频生成，提升物理合理性和视频质量。
...
Method: PhysHPO通过实例、状态、运动和语义四个层次进行偏好对齐，并引入自动化数据选择流程。
...
Method: 使用BraTS数据集的多序列MRI和临床特征构建VQA基准，评估GPT-4o、GPT-5-nano、GPT-5-mini和GPT-5在零样本链式推理任务中的表现。

Result: GPT-5-mini以44.19%的准确率领先，但所有模型的准确率均未达到临床可用水平。
...
Method: 从Sketchfab收集并筛选858K高分辨率3D模型，包括158K PBR材质模型，并提供骨架和动画子集（TexVerse-Skeleton和TexVerse-Animation）。

Result: TexVerse提供1.6M高质量3D实例，附带详细注释，支持纹理合成、PBR材质开发、动画等应用。

Conclusion: TexVerse为3D图形和视觉任务提供了丰富的高质量数据资源，具有广泛的应用潜力。
...
TL;DR: Medico 2025挑战赛专注于胃肠道影像的可解释视觉问答（VQA），旨在开发可解释的人工智能（XAI）模型，提供符合医学推理的答案和解释。


<details>
  <summary>Details</summary>
Motivation: 推动医学影像分析中可信赖AI的发展，通过结合定量性能指标和专家评审的解释性评估，提升临床决策支持能力。

Method: 挑战赛包含两个子任务：1）使用Kvasir-VQA-x1数据集回答多样化的视觉问题；2）生成多模态解释以支持临床决策。

Result: Kvasir-VQA-x1数据集包含6,500张图像和159,549个复杂问答对，作为基准用于模型评估。

Conclusion: 该挑战赛旨在通过可解释的AI模型，提升医学影像分析的可靠性和临床实用性。
...
<details>
  <summary>Details</summary>
Motivation: 传统卡通和动画制作需要大量手动工作，现有AI方法分阶段处理易导致错误累积和伪影。

Method: ToonComposer结合稀疏草图注入和空间低秩适配器，统一中间帧生成与上色，支持单草图输入或多草图精确控制。

Result: 在PKBench测试中，ToonComposer在视觉质量、运动一致性和生产效率上优于现有方法。

Conclusion: ToonComposer为AI辅助卡通制作提供了更灵活、高效的解决方案。
...
TL;DR: STream3R是一种基于Transformer的3D重建方法，通过流式框架和因果注意力高效处理图像序列，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多视图重建方法依赖昂贵的全局优化或简单内存机制，难以扩展。STream3R旨在解决这一问题。

Method: 采用解码器Transformer和因果注意力，从大规模3D数据中学习几何先验，适用于动态场景。

Result: 在静态和动态场景基准测试中均优于现有方法，兼容LLM式训练基础设施。
...
Result: 在四个地球观测数据集上评估，MAESTRO在多时相动态任务中表现最佳，同时在单时相任务中保持竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频类增量学习方法要么存储密集样本导致内存效率低，要么存储稀疏样本牺牲性能。

Method: ESSENTIAL结合稀疏特征存储的片段记忆和语义提示的语义记忆，通过交叉注意力模块检索密集特征。
...
TL;DR: 本文提出了一种统一的跨领域3D人体运动建模方法HiC，通过单阶段训练解决了现有方法依赖领域特定组件和多阶段训练的问题。
...
Result: HiC在泛化能力、数据规模和性能上优于PiC，展示了其灵活性和扩展性。

Conclusion: HiC为构建统一的跨领域3D人体运动模型提供了潜力。
...
TL;DR: Puppeteer是一个自动化3D模型绑定和动画生成的框架，通过预测骨骼结构、推断皮肤权重和优化动画流程，显著提升了效率和效果。


<details>
  <summary>Details</summary>
Motivation: 解决静态3D模型动态化过程中的专家依赖问题，提升内容创作效率。

Method: 结合自回归变换器预测骨骼结构，注意力架构推断皮肤权重，以及基于优化的动画流程。

Result: 在多个基准测试中表现优异，骨骼预测和皮肤权重质量显著优于现有技术。

Conclusion: Puppeteer能够高效处理多样化的3D内容，生成稳定的高质量动画。
...
TL;DR: 本文提出了一种新型量子隐式神经表示（QINR），称为量子视觉场（QVF），用于2D图像和3D几何场学习。QVF通过神经振幅编码将经典数据嵌入量子态向量，并采用完全纠缠的可学习参数化量子电路设计，实现了高效稳定的训练。实验表明，QVF在视觉表示准确性上优于现有量子方法和经典基线。


<details>
  <summary>Details</summary>
Motivation: 量子隐式神经表示（QINR）是一种新兴范式，但其架构设计、量子力学性质利用、训练效率以及与经典模块的交互仍存在挑战。本文旨在通过QVF解决这些问题，并探索其在视觉表示中的潜力。

Method: QVF采用神经振幅编码将数据嵌入量子态向量，并设计了一种完全纠缠的可学习参数化量子电路，在实希尔伯特空间中执行量子操作，确保数值稳定性和快速收敛。

Result: 实验证明，QVF在视觉表示准确性上优于现有量子方法和经典基线，尤其在细节学习方面表现优异。此外，QVF在2D和3D场补全及3D形状插值中展示了实际应用潜力。

Conclusion: QVF为量子视觉表示提供了一种高效且稳定的解决方案，展示了量子计算在视觉任务中的潜力，并为未来研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS信号缺失的室内环境中机器人全局定位问题，利用现有WiFi基础设施和osmAG提供高效、可扩展的解决方案。

Method: 离线阶段通过迭代优化算法定位WiFi接入点（AP），在线阶段结合osmAG地图进行实时机器人定位。

Result: 离线阶段AP定位误差3.79米（比三边测量提升35.3%），在线阶段指纹区误差3.12米（比KNN指纹提升8.77%），非指纹区误差3.83米（提升81.05%）。

Conclusion: 该框架在复杂多楼层环境中验证有效，提供了一种可扩展、低成本的室内机器人定位方案，解决了机器人绑架问题。
...
<details>
  <summary>Details</summary>
Motivation: 在复杂动态环境中，由于时空约束多，为数值求解器提供初始猜测困难。

Method: 利用GCS和ST-GCS框架，无需初始猜测即可生成最优无碰撞轨迹，并探索了通用约束的适配策略。
...
<details>
  <summary>Details</summary>
Motivation: 外骨骼机器人需要在实时动态环境中灵活应对变化，因此需要一种能够同时规划脚部接触和连续轨迹的方法。

Method: 采用Hankel矩阵建模系统动力学，结合步间（S2S）转换，通过在线重新规划实现鲁棒行走。

Result: 在Atalante外骨骼上验证了该框架，显示出更高的鲁棒性和适应性。

Conclusion: HDDPC框架为外骨骼机器人提供了一种高效的统一解决方案，适用于动态环境中的运动合成。
...
TL;DR: ReconVLA是一种重建式视觉-语言-动作模型，通过隐式接地范式引导视觉注意力到目标区域，提升机器人操作的精确性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在分配视觉注意力时存在分散问题，无法准确聚焦目标区域。

Method: 提出ReconVLA模型，利用扩散变换器重建图像中的注视区域，以学习细粒度表示并准确分配视觉注意力。

Result: 实验表明，ReconVLA在仿真和真实环境中均表现出精确操作和泛化能力。

Conclusion: ReconVLA通过隐式接地范式有效提升了视觉注意力的准确性和任务执行能力。
...
TL;DR: BEASST是一种基于行为熵梯度的自适应源搜索框架，用于移动机器人在复杂未知环境中高效平衡探索与开发。


<details>
  <summary>Details</summary>
Motivation: 解决移动机器人在未知环境中源搜索的效率问题，通过动态调整行为策略以适应信号可靠性和任务紧迫性。

Method: 利用行为熵和Prelec概率加权函数定义目标函数，动态调整机器人行为从风险规避到风险寻求。

Result: 在DARPA SubT和多房间场景中，BEASST比现有方法路径缩短15%，源定位速度快20%。
...
Method: 利用多模态大语言模型（MLLM）生成候选响应，结合奖励函数评估，并通过GRPO算法优化模型。
...
TL;DR: 论文提出了一种语义感知框架，通过集成大语言模型到任务规划中，提升上肢外骨骼的辅助能力，使其能根据任务语义调整配置并实时响应异常。
...
Method: 框架包括透明模式捕捉用户意图、语义信息提取、自动配置辅助参数、基于扩散的异常检测及实时重规划、在线轨迹优化和阻抗控制。

Result: 实验表明，该方法能有效适应用户认知、适应语义变化任务并可靠响应异常。

Conclusion: 提出的语义感知框架显著提升了外骨骼的辅助能力，为家庭护理场景提供了更安全、更灵活的解决方案。
...
TL;DR: 论文提出了一种从稀疏数据生成密集LiDAR反射图像的创新框架，解决了低成本LiDAR在稀疏数据上的限制，并展示了其在多种应用中的潜力。


<details>
  <summary>Details</summary>
Motivation: 传统上，主动光学传感未被视为主流视觉模态，但技术进步使其重新定义视觉边界。低成本LiDAR因数据稀疏性受限，需要解决方案以实现更广泛应用。

Method: 利用非重复扫描LiDAR（NRS-LiDAR）特性，提出反射校准和动态场景转换方法，构建密集反射图像生成框架。

Result: 开发了LiDAR反射图像密集化数据集和专用网络，成功应用于环路闭合和交通车道检测等场景。

Conclusion: 该框架为低成本LiDAR的广泛应用提供了可行方案，展示了主动视觉在机器人感知任务中的潜力。
...
Method: 调研了分层和端到端决策范式，以及大模型如何增强高层规划、低层执行和反馈；同时探讨了大模型如何增强模仿学习和强化学习。

Result: 大模型显著提升了具身AI的感知、交互、规划和学习能力，世界模型在决策和学习中发挥了关键作用。

Conclusion: 尽管取得了进展，但仍存在挑战，这些挑战可能成为未来的研究方向。
...
Method: 开发了一种识别错误轨迹偏离的方法，并设计技术自动生成感知和动作的自校正数据，通过多次迭代逐步优化模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常采用单智能体强化学习或多智能体强化学习用于多机器人任务，但缺乏针对单个人形机器人运动的协作异构MARL方法。
...
<details>
  <summary>Details</summary>
Motivation: 中小企业和研究人员在缺乏机器人专业知识的情况下，部署和维护机器人系统面临挑战，且容易依赖外部专家。

Method: 使用Python实现了一个原型，通过抽象层统一不同制造商和型号的机器人接口。

Result: 开发了一个专注于简化现代机器人系统接口的框架，并以Yaskawa Motoman GP4为例进行了验证。

Conclusion: 该框架有效降低了机器人系统的部署难度，减少了对外部专家的依赖。
...
Method: 使用核密度估计（KDPE）结合流形感知核，对动作轨迹进行过滤。

Result: 在模拟单臂任务和真实机器人实验中，KDPE性能优于Diffusion Policy。

Conclusion: KDPE通过有效过滤有害轨迹，提升了机器人策略的稳定性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决四足机器人全身运动与多任务控制的难题，特别是在平衡多任务学习时的挑战。

Method: 引入轨迹库与自适应课程采样机制，提出轨迹-速度预测策略网络，结合模拟数据与课程奖励。

Result: 仿真中验证了方法的必要性与有效性，真实实验中在Go2机器人上展示了多任务执行的良好性能。

Conclusion: MLM框架成功实现了四足机器人的全身运动与多任务控制，并具备零样本迁移能力。
...
Result: 小故事能有效揭示罕见失败案例，促进机器人能力的透明化，并记录研究中常被忽略的意外行为。
...
TL;DR: 论文提出了一种结合深度神经网络（DNN）和鲁棒自适应控制（RAC）的分层控制策略，用于重型轮式移动机器人（WMR），以平衡精确控制和安全性。


<details>
  <summary>Details</summary>
Motivation: 重型WMR在严格国际标准下运行，易受故障和干扰影响，传统黑盒DNN方法难以满足需求。

Method: 设计了分层控制策略：DNN作为主控策略，RAC作为低层安全层，高层安全层监控系统性能。

Result: 实验验证了该策略在6,000 kg WMR上的有效性，保证了系统稳定性和安全性。
...
TL;DR: 本文介绍了一个基于SOFA的开源、用户友好型仿真工具，专门用于模拟磁性软体机器人，填补了现有平台对磁性材料支持的不足。
...
Result: 通过四个基准模型（梁、三指和四指夹持器、蝴蝶）验证了工具的功能性，且易于使用。

Conclusion: 该工具为磁性软体机器人的理论和实际设计搭建了桥梁，未来将通过实验验证和与行业标准有限元求解器的比较进一步提升精度。
...
<details>
  <summary>Details</summary>
Motivation: 传统探索策略仅关注最大化探索面积，未考虑环境中的关键区域对探索效率的影响。

Method: 使用神经网络生成显著性地图，识别对探索至关重要的区域，并基于此优化现有探索策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统SoT由专家手动设计，限制了其适应性和可访问性。本文旨在通过自动学习框架解决这一问题。
...
TL;DR: 论文提出SET感知因素框架，用于系统分析环境因素对机器人感知的影响，以提升自主系统的安全性和公众信任。


<details>
  <summary>Details</summary>
Motivation: 自主系统的部署引发安全和信任问题，感知失败是主要风险之一，需系统性解决方案。

Method: 引入SET框架，利用SET状态树和因素树分类和建模感知任务中的不确定性。

Result: 开发感知因素模型，量化任务不确定性，提供透明和标准化的风险识别方法。

Conclusion: SET框架为自主系统感知风险提供系统分析工具，增强安全性和公众信任。
...
<details>
  <summary>Details</summary>
Motivation: 随着社交机器人在各种社交场景中的应用增加，模拟人类与机器人互动中的自我披露行为变得迫切。目前缺乏计算模型来准确模拟这一行为。

Method: 开发了一种基于情感识别文献的多模态注意力网络，使用自收集的自我披露视频语料库进行训练，并提出了新的损失函数——尺度保持交叉熵损失。

Result: 最佳模型在新损失函数下F1得分为0.83，比基线模型提高了0.48。
...
TL;DR: 提出了一种基于李群的视觉-惯性-测距紧耦合系统（CVIRO），通过联合估计机器人状态和UWB锚点状态，确保系统的一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决UWB辅助VIO系统中因观测性不一致和锚点位置假设导致的定位性能下降问题。

Method: 将UWB锚点状态纳入系统状态，利用李群的不变误差特性保持观测一致性。

Result: CVIRO在仿真和实验中表现出优于现有方法的定位准确性和一致性。

Conclusion: CVIRO通过联合估计和观测性保持，显著提升了UWB辅助VIO系统的性能。
...
Result: A2C算法在累积奖励（10.0 vs 9.263025）和收敛速度（2,000 vs 63,000时间步）上显著优于PPO算法，并能高效满足多样化任务目标。

Conclusion: 强化学习为低地球轨道任务规划提供了一种计算高效且可扩展的解决方案，验证了A2C算法在连续轨道控制中的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 商业预测工具依赖专有数据和封闭模型，限制了公众获取高精度预测的能力。OpenFPL旨在通过开源方法解决这一问题。

Method: 基于2020-21至2023-24赛季的公开数据，开发了针对不同位置的集成模型。

Result: OpenFPL在2024-25赛季的测试中表现与商业服务相当，且在高回报球员预测上更优。

Conclusion: OpenFPL为玩家提供了开源且高精度的预测工具，支持长期和短期决策。
...
TL;DR: xRFM算法结合特征学习核机与树结构，在表格数据上表现优于31种其他方法，包括GBDTs和TabPFNv2，并在回归和分类任务中均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 尽管表格数据推断在现代科技中至关重要，但其最佳实践仍主要基于GBDTs，缺乏创新。本文旨在通过结合神经网络和特征学习方法，开发更先进的表格数据处理技术。

Method: 提出xRFM算法，结合特征学习核机与树结构，既能适应数据的局部结构，又能扩展到大规模训练数据。

Result: xRFM在100个回归数据集上表现最佳，在200个分类数据集上与最佳方法竞争，且优于GBDTs。此外，xRFM通过平均梯度外积提供原生可解释性。

Conclusion: xRFM是一种高效且可解释的表格数据处理方法，性能优于现有技术，为表格数据推断提供了新的解决方案。
...
TL;DR: PEARL研究首次大规模评估了基于强化学习（RL）的个性化健康干预，结果显示RL组在1个月和2个月时的步数显著增加。


<details>
  <summary>Details</summary>
Motivation: 解决全球普遍缺乏身体活动的问题，探索移动健康（mHealth）干预的潜力，尤其是基于强化学习的个性化干预。

Method: 13,463名Fitbit用户随机分为四组（控制、随机、固定、RL），RL组通过算法个性化推送行为科学启发的提示。

Result: RL组在1个月和2个月时的步数显著高于其他组，1个月时平均每天多走296步（p=0.0002）。

Conclusion: 行为科学启发的强化学习方法可有效个性化数字健康干预，促进身体活动。
...
TL;DR: 论文探讨时间序列预测中模型稳定性的重要性，提出模型诱导随机性概念，并通过案例研究验证集成模型在提升稳定性方面的优势。


<details>
  <summary>Details</summary>
Motivation: 需求计划者更看重预测的一致性和稳定性，而非仅关注准确性提升。模型预测的剧烈波动会增加人工干预，降低对ML模型的信任。

Method: 通过案例研究，评估多种先进时间序列预测模型（如Chronos、DeepAR等）在公开数据集上的稳定性和准确性。
...
TL;DR: 提出了一种针对扩散大语言模型的约束解码方法，确保生成内容符合上下文无关文法。


<details>
  <summary>Details</summary>
Motivation: 现有约束解码方法不适用于扩散大语言模型，而实际应用（如生成C++代码或JSON数据）需要语法正确性。

Method: 将约束解码问题转化为可加性填充问题，并通过高效算法判断目标语言与正则语言的交集是否为空。

Result: 实验表明，该方法在C++代码填充和JSON数据提取中几乎完美保证语法正确性，且计算开销可控。

Conclusion: 该方法为扩散大语言模型提供了一种高效的约束解码方案，适用于多种实际场景。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLMs在图任务中的潜力，验证其是否能在无专用图编码模型的情况下学习和泛化。

Method: 训练LLMs使用链式思维解决方案，评估其在不同图任务和结构上的表现。

Result: 小型LLMs能通过学习解决图任务，且具备泛化能力。

Conclusion: LLMs无需专用图编码器即可学习和泛化图任务，为相关研究提供了新方向。
...
Method: 提出CAD-RL框架，结合CoT冷启动和目标驱动的强化学习后训练，使用三个任务特定奖励，并引入三种优化策略。
...
TL;DR: 论文提出了一种名为Nested-ReFT的新框架，通过动态层跳过减少推理成本，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决标准ReFT框架在训练过程中因多步推理导致的高计算成本问题。

Method: 利用目标模型的部分层作为行为模型生成离策略补全，动态跳过层以减少推理成本。

Result: 在多个数学推理基准测试中提高了计算效率（tokens/sec），同时性能与基线ReFT相当。

Conclusion: Nested-ReFT通过降低推理成本，提供了一种高效且性能相当的ReFT替代方案。
...
TL;DR: 提出了一种基于反事实解释的OOD检测方法，通过计算输入到决策边界的距离，并结合可解释性策略，提高了检测效率和可解释性。
...
Method: 提出了一种后处理OOD检测方法，利用反事实解释计算输入到决策边界的距离，并通过在嵌入空间中直接计算反事实来提高可扩展性。
...
TL;DR: 论文研究了在模型过拟合情况下EM算法的行为，针对两分量混合线性回归（2MLR）问题，分析了回归参数和混合权重的收敛性，并探讨了有限样本下的统计精度。
...
Method: 通过理论分析，研究了EM算法在过拟合2MLR问题中的收敛性，包括初始混合权重不平衡和平衡两种情况。

Result: 在初始权重不平衡时，回归参数线性收敛；平衡时则次线性收敛。有限样本下，不平衡权重精度为O((d/n)^(1/2)，平衡时为O((d/n)^(1/4))。

Conclusion: 论文建立了EM算法在过拟合2MLR问题中的收敛理论，并展示了初始权重对收敛速度和统计精度的影响。
...
TL;DR: 论文探讨了推理语言模型的评估方法，指出性能提升可能源于基准测试驱动的学习而非算法改进或模型规模，并提出了基准测试作为训练课程的概念。


<details>
  <summary>Details</summary>
Motivation: 随着推理成为大型语言模型的下一个扩展维度，需要深入研究其在关键任务中的能力，以揭示性能提升的真正原因。

Method: 通过基准测试驱动的选择（benchmark-driven selection）方法，在DeepSeek-R1模型上验证了基准测试对模型学习的影响。

Result: 研究发现，性能提升可能源于基准测试作为学习课程的作用，而非单纯的算法或规模改进。

Conclusion: 基准测试可作为训练课程，而非仅用于评估，其新颖性对衡量推理模型的泛化能力至关重要。
...
TL;DR: 论文提出了一种基于可解释机器学习的方法，通过3轴加速度传感器和蓝牙物联网设备监测奶牛活动，帮助农民优化养殖管理。
...
Method: 使用3轴加速度传感器和蓝牙物联网设备收集数据，结合滑动窗口技术和统计特征提取，优化多种机器学习模型进行分类。
...
<details>
  <summary>Details</summary>
Motivation: 跨国象牙贸易导致非洲象数量持续下降，传统DNA取证成本高且有时无法获取，而手写标记虽易拍照但未被充分利用。

Method: 收集6,085张象牙照片，使用目标检测模型提取17,000多个标记，通过AI工具分析并识别184个重复标记，其中20个标记连接多个象牙批次。

Result: 发现20个重复标记连接多个象牙批次，为打击犯罪网络提供新证据。

Conclusion: AI手写分析填补了传统取证的空白，展示了AI在野生动物取证中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 探索D-Wave量子退火器在RBM采样中的潜力，并评估其与传统Gibbs采样的互补性。

Method: 采用局部谷（LV）中心方法，比较D-Wave和Gibbs采样在不同训练阶段的LV覆盖和能量表现。

Result: D-Wave采样覆盖更多LV，但与Gibbs采样重叠较多；后期训练阶段互补性降低，但发现了一些重要局部最小值。

Conclusion: D-Wave采样未显著提升质量，但结合经典-量子方法可能有改进空间。
...
TL;DR: 该研究开发了一种可解释的机器学习模型，用于早期预测肝硬化重症患者的急性肾损伤（AKI），LightGBM表现最佳。


<details>
  <summary>Details</summary>
Motivation: 肝硬化患者中AKI发生率高且预后差，现有预测工具准确性不足且难以与ICU工作流程结合。

Method: 回顾性分析MIMIC-IV数据库，使用多种算法（如LightGBM、XGBoost等）训练模型，评估指标包括AUROC、准确性等。

Result: LightGBM模型表现最佳（AUROC 0.808），关键预测因子包括凝血时间延长、pH值低等。

Conclusion: 该模型能准确预测AKI风险，高阴性预测值支持低风险患者的安全管理，未来需外部验证和电子病历系统整合。
...
TL;DR: 论文探讨了基于Transformer的语言模型在上下文学习（ICL）中的新应用，即学习密码函数（如单字母替换和Vigenère密码），以评估其归纳偏差和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究ICL在密码函数学习中的应用，以测试Transformer模型在结构化推理任务中的表现。

Method: 使用ICL范式，通过少量（密文，明文）对，让模型推断密码规则并解码新密文。

Result: 模型能够成功学习并推断密码函数的规则，展示了ICL在结构化任务中的潜力。

Conclusion: ICL在密码函数学习中的成功应用为Transformer模型的归纳能力提供了新的验证，并扩展了其应用领域。
...
Method: HPMI通过修剪最不重要头并注入预训练恶意头建立后门，无需重新训练或改变架构。

Result: 实验显示HPMI在攻击成功率（≥99.55%）、隐蔽性和防御绕过方面表现优异，且对干净数据准确性影响极小。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种新的指数神经网络算子，以改进函数的近似效果，并研究其收敛性质。

Method: 使用Max Min方法构建Kantorovich型指数神经网络算子，分析点态和均匀收敛性，利用对数连续性模估计收敛速率。

Result: 在Orlicz空间中验证了算子的收敛行为，并通过图形展示了近似误差。

Conclusion: Max Min Kantorovich型指数神经网络算子在函数近似中表现良好，具有潜在的应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 云原生数据库系统面临高资源动态性和调度复杂性的挑战，需要一种能够适应这些特性的资源编排方法。

Method: 采用多智能体强化学习，引入基于异构角色的智能体建模机制和奖励塑造机制，结合局部观察与全局反馈。

Result: 实验表明，该方法在资源利用率、调度延迟、策略收敛速度、系统稳定性和公平性等关键指标上优于传统方法。

Conclusion: 该方法在多种实验场景下表现出色，适用于高并发、高维状态空间和复杂依赖关系的实际大规模调度环境。
...
<details>
  <summary>Details</summary>
Motivation: 解决多租户云环境中的数据隐私泄露、资源行为异构和集中式建模的局限性。
...
Result: 在多个真实回归数据集上实验表明，方法优于基线，累计测试损失降低高达67.4%。

Conclusion: 通过理论推导和实验验证，方法充分利用偏移特性，实现了卓越的适应性能。
...
TL;DR: 论文提出了一种基于共形预测的框架，用于预测帕金森病患者的药物需求，并提供可靠的预测区间和统计保证，以优化药物治疗管理。


<details>
  <summary>Details</summary>
Motivation: 帕金森病药物治疗管理因疾病进展和治疗的异质性而复杂，当前方法依赖试错决策，缺乏系统预测方法，且现有机器学习方法因未考虑预测不确定性而临床实用性受限。

Method: 开发了一种两阶段共形预测框架，利用电子健康记录数据，先识别需要药物调整的患者，再预测左旋多巴等效日剂量调整需求。
...
TL;DR: SynBrain是一个生成框架，通过概率建模和语义约束，解决了视觉刺激到神经响应的变异性问题，并在fMRI合成和解码性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉刺激到神经响应的映射具有一对多的变异性，现有确定性方法难以同时建模生物变异性与功能一致性。

Method: SynBrain包含BrainVAE（概率建模）和语义到神经映射器，通过语义约束和概率分布模拟神经响应。

Result: SynBrain在视觉到fMRI编码性能上优于现有方法，适应新受试者能力强，并能提升fMRI到图像解码性能。

Conclusion: SynBrain不仅提升了性能，还揭示了跨试验和受试者的功能一致性，具有生物学解释性。
...
TL;DR: FedKEI是一种联邦学习框架，通过跨客户端和跨任务的知识转移，为新任务提供优化的初始化参数，提升医疗领域中的任务适应能力。


<details>
  <summary>Details</summary>
Motivation: 医疗环境中需要快速适应新任务或疾病，同时保护隐私，因此提出FedKEI框架。

Method: FedKEI通过全局聚类和双层优化方案，学习跨任务知识并个性化知识转移。

Result: 在皮肤病、胸部X光和视网膜OCT数据集上，FedKEI优于现有方法。

Conclusion: FedKEI有效提升了医疗联邦学习中新任务的适应能力。
...
Result: 在CIFAR-10和CIFAR-10-LT数据集上，CLIP-Fed平均降低了ASR（2.03%和1.35%），同时提高了MA（7.92%和0.48%）。
...
TL;DR: 论文提出了一种基于福利的公平聚类方法，通过距离和比例代表建模群体效用，并引入新算法优化两种目标（Rawlsian和Utilitarian），实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统公平聚类方法可能导致不理想的结果，因此需要一种福利中心的方法来更合理地建模群体效用。
...
TL;DR: 论文提出了一种多层次的IoMT入侵检测系统框架，能够高效检测零日攻击并区分已知和未知威胁，实验显示高准确率和F1分数。
...
TL;DR: 本文研究了语义通信中的分布学习问题，揭示了学习源统计量的基本条件，并分析了编码方案在即时性能与长期可学习性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 传统语义通信专注于单个意义的传输，而本文探索了在未知先验条件下学习源统计量的可能性，填补了语义通信中统计学习理论的空白。

Method: 通过理论分析，证明了有效传输矩阵满秩是学习性的必要条件，并量化了分布估计的收敛速度及语义失真。实验在CIFAR-10数据集上验证了理论框架。
...
TL;DR: eMamba是一个专为边缘设备设计的硬件加速框架，用于高效部署Mamba模型，通过轻量级替代和近似操作提升计算效率，并在多个数据集上验证了其性能和资源效率。


<details>
  <summary>Details</summary>
Motivation: 尽管Mamba模型在序列数据处理中表现出色且计算高效，但缺乏针对边缘设备的硬件加速框架，限制了其实际应用。

Method: eMamba通过替换复杂归一化层、近似昂贵操作（如SiLU激活和指数运算），并结合近似感知的神经架构搜索（NAS）优化参数。

Result: 在多个数据集上，eMamba以更少的参数实现与现有技术相当的精度，并在FPGA和ASIC上显著降低了延迟、功耗和面积。

Conclusion: eMamba为边缘设备提供了一个高效、低功耗的Mamba模型部署解决方案，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有ITL方法缺乏评估其是否真正捕捉标注者行为模式并提供有意义解释的框架。

Method: 提出DIC和BAE两个指标，分别量化模型捕捉标注者倾向的能力和解释行为与决策的相关性。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM推理中内存占用高和带宽需求大的问题，利用计算能力提升的趋势。

Method: 量化并缓存层输入激活X，动态重计算Keys和Values，提出XQuant-CL利用跨层相似性进一步压缩。

Result: XQuant实现7.7倍内存节省，XQuant-CL实现10倍内存节省，精度损失极小。

Conclusion: XQuant通过计算换内存的策略，显著优化LLM推理效率，超越现有KV缓存量化方法。
...
TL;DR: SC2Arena和StarEvolve填补了现有基准测试的不足，支持完整的游戏复杂性和战略规划。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法全面评估大型语言模型在复杂决策任务（如《星际争霸II》）中的表现。

Method: 提出SC2Arena基准测试和StarEvolve框架，结合战略规划和战术执行，并通过自我修正和高质量数据优化。

Result: StarEvolve在战略规划中表现优异，SC2Arena提供了开发通用智能体的新见解。

Conclusion: SC2Arena和StarEvolve为复杂决策任务提供了更全面的评估和优化方法。
...
TL;DR: SAM的隐式正则化在张量化或尺度不变模型中的行为尚未充分研究。本文通过尺度不变性分析SAM的范数动态，提出Norm Deviation作为核心范数不平衡的全局度量，并基于梯度流分析其演化。研究发现SAM对Norm Deviation的隐式控制由核心范数与梯度幅度的协方差决定，进而提出Deviation-Aware Scaling (DAS)方法，实验证明其性能优于SAM且计算开销更低。
...
Method: 利用尺度不变性分析SAM的范数动态，提出Norm Deviation概念，并通过梯度流分析其演化。基于研究发现，提出DAS方法，显式模拟SAM的隐式正则化行为。

Result: 实验表明，DAS在张量补全、噪声训练、模型压缩和参数高效微调等任务中表现优于SAM，且计算开销更低。

Conclusion: DAS通过显式模拟SAM的隐式正则化行为，在多个任务中实现了更优的性能和效率。
...
TL;DR: RealAC框架生成现实且可操作的反事实解释，自动保留特征间复杂依赖关系，并支持用户冻结特定属性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工约束或领域知识，难以捕捉复杂非线性关系，且缺乏用户定制能力。

Method: RealAC通过对齐事实与反事实实例的特征联合分布，自动保留特征依赖，并允许用户冻结属性。
...
<details>
  <summary>Details</summary>
Motivation: GNN在医学图像分类等任务中表现出色，但其决策过程不透明，限制了在高风险临床应用中的可信度。现有解释技术多为事后和全局性，无法提供局部推理的详细解释。

Method: X-Node为每个节点构建结构化上下文向量，通过轻量级Reasoner模块生成解释向量，用于重建节点嵌入、生成自然语言解释并反馈到消息传递过程中。

Result: 在MedMNIST和MorphoMNIST数据集上，X-Node在保持分类准确性的同时，生成了忠实于节点的解释。

Conclusion: X-Node为GNN提供了一种高效的自解释框架，适用于需要高可解释性的应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 联邦图学习面临数据分布不均和类别不平衡问题，导致模型偏向多数类，难以识别少数类关键事件。

Method: 提出GraphFedMIG框架，利用分层生成对抗网络，通过互信息引导机制优化生成器参数，生成高价值少数类特征。
...
TL;DR: 论文探讨了推荐系统中因忽略观察特征而导致的混淆问题，并提出了解决方法。


<details>
  <summary>Details</summary>
Motivation: 未观察到的混淆会导致因果效应估计偏差，影响观察性研究。推荐系统虽依赖完全观察数据，但实践中常忽略特征，引发类似问题。

Method: 通过模拟研究展示常见实践（如特征工程、A/B测试、模块化）如何引入混淆，并提供减少混淆的实际建议。

Result: 研究发现推荐系统中常见实践会引入混淆，影响性能，模拟研究支持这一结论。

Conclusion: 论文建议实践者采取措施减少或避免混淆，以提升推荐系统性能。
...
TL;DR: 提出了一种确保神经网络满足凸约束的输出层方法Πnet，结合算子分裂和隐函数定理，实现了快速可靠的优化代理。
...
Method: 利用算子分裂进行快速投影（前向传播），隐函数定理实现反向传播，设计为GPU友好的JAX实现。

Result: 在单问题和批量问题中均显著快于传统求解器，训练时间、解质量和超参数鲁棒性优于现有学习方法。

Conclusion: Πnet在保持推理时间的同时，提供了高效、可靠的优化代理，适用于非凸轨迹规划等复杂问题。
...
TL;DR: 本文提出了一种利用连续时间动态系统和神经ODE从任意观测数据构建世界模型的新方法，通过收缩嵌入和Lipschitz常数约束构建有序潜在状态空间。


<details>
  <summary>Details</summary>
Motivation: 联合嵌入预测架构（JEPA）比基于重构的方法更具潜力，但如何利用其构建更通用的世界模型仍待探索。

Method: 结合序列嵌入和神经ODE，采用约束收缩嵌入和Lipschitz常数的损失函数，构建有序潜在状态空间。
...
TL;DR: 本文提出了一种统一的光谱框架，用于分析和量化解释的平滑性与忠实性之间的权衡，并揭示了现有方法在解释中的失真问题。


<details>
  <summary>Details</summary>
Motivation: ReLU网络在视觉数据中广泛应用，但其梯度解释存在噪声和难以解释的问题，现有方法（如GradCAM）通过牺牲忠实性来平滑解释。

Method: 引入一个光谱框架，系统分析平滑性与忠实性的权衡，并通过正则化ReLU网络对高频信息的贡献来优化解释。

Result: 理论分析揭示了基于代理的平滑方法如何扭曲解释，并提出了“解释差距”的正式定义和测量方法。

Conclusion: 通过实验验证了理论发现，为解释方法的优化提供了原则性指导。
...
TL;DR: 论文提出了一种基于对比学习的自动代码本学习方法，用于多类分类，相比传统方法更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统ECOC方法依赖手动或随机生成的代码本，效率低且效果不佳，需要一种自适应学习的方法。

Method: 引入三种基于对比学习的模型，直接从数据中自适应学习代码本。

Result: 在四个数据集上，提出的模型对对抗攻击表现出更强的鲁棒性。

Conclusion: 自动代码本学习方法在多类分类中优于传统方法，具有实际应用潜力。
...
TL;DR: MAGUS是一个模块化框架，通过解耦的认知和决策阶段统一多模态理解和生成，支持灵活扩展和高效跨模态转换。


<details>
  <summary>Details</summary>
Motivation: 现实多模态应用需要任意模态间的理解和生成能力，但现有方法缺乏灵活性和可扩展性。

Method: MAGUS采用多智能体协作，分为认知阶段（感知、规划、反思）和决策阶段（基于搜索的推理与生成协同）。

Result: 实验表明MAGUS在多个基准测试中优于基线方法和先进系统，甚至在MME基准上超越GPT-4o。

Conclusion: MAGUS通过模块化设计实现了高效的多模态任务处理，无需联合训练即可支持语义对齐和扩展性。
...
TL;DR: 论文提出了一种基于深度强化学习的非平衡非局部蒙特卡洛（NMC）算法，用于优化组合优化问题的复杂成本函数。该方法通过训练非局部转移策略，显著提升了在硬基准问题上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统MCMC算法在解决硬基准问题时效率低下，尤其是在计算相变附近。NMC算法通过非均匀温度分布加速探索，但之前的策略设计较为经验化。
...
Conclusion: 深度强化学习可以有效地训练NMC策略，显著提升组合优化问题的求解性能。
...
TL;DR: 提出了一种名为PCD的新方法，用于在测试时生成联合相关的样本，同时满足任务特定约束，无需重新训练扩散模型。
...
Result: 在图像对生成、物体操纵和多机器人运动规划中表现出色，提高了耦合效果并确保约束满足。

Conclusion: PCD是一种高效且无需重新训练的联合生成框架。
...
TL;DR: Applm是一种基于xTrimoPGLM蛋白质语言模型的过敏原预测框架，性能优于现有方法，适用于多种复杂场景。


<details>
  <summary>Details</summary>
Motivation: 过敏原蛋白对公共健康构成挑战，需要更准确的方法进行识别。

Method: 利用1000亿参数的xTrimoPGLM模型开发Applm框架，支持多种复杂任务。

Result: Applm在多种任务中表现优于七种现有方法，尤其在识别新过敏原和高相似度同源蛋白区分上。

Conclusion: xTrimoPGLM模型对Applm性能至关重要，开源框架和数据集将推动未来研究。
...
TL;DR: 论文提出了一种针对软件工程任务的强化学习框架（SWE-oriented RL Framework）和一种新的奖励累积方法（G-RA），解决了长期任务中奖励稀疏和奖励对齐问题。


<details>
  <summary>Details</summary>
Motivation: 长期强化学习任务中奖励稀疏和奖励对齐问题导致策略优化困难，现有方法难以定义无偏的即时奖励或避免奖励滥用。

Method: 提出了SWE-oriented RL Framework支持多轮交互和自定义奖励函数，并设计了Gated Reward Accumulation（G-RA）方法，仅在长期奖励达标时累积即时奖励。

Result: 实验显示G-RA显著提高了任务完成率（47.6%→93.8%）和修改率（19.6%→23.8%），避免了策略退化。

Conclusion: G-RA方法在长期任务中实现了稳定的强化学习优化，为奖励对齐问题提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 观测数据中的治疗效果估计复杂且需要专业知识，限制了先进方法的广泛应用。

Method: CATE-B通过因果发现、LLM辅助的边定向、最小不确定性调整集准则和回归方法选择，提供交互式指导。

Result: 系统降低了因果分析的难度，并发布了多领域基准任务以促进评估。

Conclusion: CATE-B为自动化治疗效果估计奠定了基础，推动了因果推理的普及。
...
TL;DR: 论文提出了一种统一学习范式，通过图表示将不同架构的模型统一到一个共享的图学习空间中，利用统一图神经网络（uGNN）优化模型，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在医学影像中因数据分布差异（如成像技术、设备等）导致的泛化能力不足问题。

Method: 将不同模型编码为图表示，通过uGNN进行统一优化，支持参数共享和知识迁移。

Result: 在MorphoMNIST和MedMNIST基准测试中，统一学习方法显著提升了模型在分布变化数据上的性能。

Conclusion: 统一学习通过图表示和uGNN优化，有效提升了模型在异构数据和分布变化下的泛化能力。
...
TL;DR: 提出了一种基于生成对抗变换器（GATs）的新方法，用于解决能源系统模型中的时间序列上采样问题，无需真实高分辨率数据即可训练，显著降低了误差并提高了应用场景的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统上采样方法存在信息丢失或噪声增加的问题，而现有高级模型（如生成模型、超分辨率模型和插补模型）因依赖监督学习和数据稀疏性而面临挑战。

Method: 利用生成对抗变换器（GATs）进行无监督训练，无需真实高分辨率数据，生成统计特性相似的高分辨率时间序列。

Result: 与传统插值方法相比，该方法将上采样任务的均方根误差（RMSE）降低了9%，并在模型预测控制（MPC）应用场景中提高了13%的准确性。

Conclusion: GATs方法有效解决了上采样问题，无需依赖真实高分辨率数据，显著提升了性能和应用效果。
...
TL;DR: 论文通过概率和谱视角分析深度网络的解释方法，揭示了梯度使用中的谱偏差，并提出两种改进方法：标准化扰动尺度和SpectralLens聚合方法。
...
Method: 采用概率和谱视角分析解释方法，揭示梯度使用中的谱偏差，并提出标准化扰动尺度和SpectralLens聚合方法。

Result: 研究发现梯度使用中存在谱偏差，并通过定量评估验证了提出的改进方法的有效性。

Conclusion: 通过形式化分析，论文为解释方法的设计提供了理论基础，并提出了实际改进方案。
...
TL;DR: 本文系统综述了分割学习（SL）中的安全攻击，分类分析了攻击类型、隐私风险及防御方法，并指出未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 分割学习虽提升隐私保护，但分布式特性带来新的安全挑战，需全面探索潜在攻击。

Method: 通过分类攻击（攻击者角色、隐私风险类型、数据泄露时机、漏洞位置）并分析现有防御方法（加密、数据修改、分布式技术、混合方案）。

Result: 揭示了安全漏洞，评估了现有防御的有效性和局限性。

Conclusion: 为改进分割学习的隐私问题及未来研究提供了有价值的信息和方向。
...
TL;DR: 本文提出了一种通过方差缩减技术提高多目标强化学习样本效率的方法，同时保持通用假设。


<details>
  <summary>Details</summary>
Motivation: 多目标强化学习（MORL）需要同时优化多个冲突目标，但现有策略梯度方法样本效率低，且依赖严格假设。

Method: 采用方差缩减技术，降低策略梯度的样本复杂度，同时避免严格假设。

Result: 提高了样本效率，同时保持了策略梯度方法在大状态-动作空间中的可扩展性。

Conclusion: 方差缩减技术有效解决了MORL中样本效率低的问题，且无需牺牲通用性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的数据划分方法忽略了每个实例的内在质量，而IRT可以揭示数据的异质性并帮助优化模型验证。

Method: 利用IRT参数对数据集进行划分，并评估其对多种机器学习模型性能的影响。

Result: IRT揭示了数据的异质性，并帮助创建平衡的分区，优化了模型的偏差-方差权衡。高猜测参数实例会显著降低模型性能。

Conclusion: IRT指导的数据划分能更好地理解模型性能，高猜测参数实例对模型有负面影响。
...
TL;DR: 提出了一种新的预测结合自由能变化（ΔΔG）的方法，通过分解为序列和结构两部分，结合能量模型和统计力学，优于现有深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 预测ΔΔG对蛋白质工程和药物发现至关重要，但传统方法难以估计蛋白质复合物的全构象分布。
...
Method: 构建了MUStARD++$^{R}$数据集，去除捷径信号，并提出了多模态条件信息瓶颈模型（MCIB）以实现高效模态融合。

Result: MCIB在不依赖捷径学习的情况下取得了最佳性能。

Conclusion: 改进模态融合策略和消除捷径信号是提升多模态讽刺检测性能的关键。
...
TL;DR: SPHENIC方法通过结合空间拓扑信息和邻居优化模块，提升了空间转录组聚类性能。
...
Method: 提出SPHENIC方法，结合持久同源性和空间约束优化模块，提升聚类稳定性。

Result: 在14个基准数据集上，SPHENIC性能优于现有方法3.31%-6.54%。

Conclusion: SPHENIC通过拓扑特征和空间优化，显著提升了空间转录组聚类的效果。
...
<details>
  <summary>Details</summary>
Motivation: 土地覆盖变化对地球系统过程有重要影响，但现有模型在预测土地覆盖变化方面能力不足。因此，研究提出利用生成式AI改进预测能力。

Method: 将土地覆盖变化预测视为数据合成问题，使用扩散模型进行十年尺度的不透水表面预测，并与基线模型（假设无变化）进行比较。

Result: 在12个大都市区的实验中，模型在分辨率≥0.7×0.7km²时，平均绝对误差（MAE）低于基线模型，表明生成模型能有效捕捉历史数据中的时空模式。
...
<details>
  <summary>Details</summary>
Motivation: 探索一种更高效且表达能力强的图分类方法，同时保持与现有方法相当的准确性。

Method: 通过修改Weisfeiler-Leman算法的逻辑框架生成变体，将其用于图数据表格化，再应用表格数据分类方法。

Result: 在12个基准数据集上，该方法在准确性上与最先进的图神经网络和图核方法相当，同时在时间或内存效率上更优。
...
TL;DR: 论文提出了一种名为MDNS的新框架，用于训练离散神经采样器，以从高维多模态分布中生成样本。


<details>
  <summary>Details</summary>
Motivation: 解决在高维离散状态空间中生成样本的挑战性问题，尤其是在分布多模态且基数大的情况下。

Method: 通过将两个路径度量对齐，基于连续时间马尔可夫链的随机最优控制理论，提出MDNS框架。

Result: MDNS在多种统计特性的分布上表现出高效性和可扩展性，显著优于其他基于学习的方法。

Conclusion: MDNS框架在解决高维离散采样问题上具有显著优势，并通过实验验证了其有效性和潜力。
...
TL;DR: REFN利用强化学习训练LLM自动生成网络过滤器，有效预防1天或N天漏洞攻击，具有高准确性、效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有防御措施（如主机补丁和网络过滤）因可扩展性、兼容性和部署问题不足，无法应对大规模漏洞威胁。

Method: REFN通过强化学习驱动LLM生成网络过滤器，结合在线网络奖励、边缘安全网关部署和实时验证。

Result: 在22类漏洞测试中，REFN准确性提高21.1%，平均补丁时间降至3.65小时，可扩展至1万台设备。

Conclusion: REFN为LLM快速预防大规模漏洞攻击提供了初步解决方案。
...
TL;DR: 提出了一种基于GPU的电磁仿真框架，用于天线设计与优化的机器学习应用，并与商业软件结果对比。
...
Conclusion: GPU和开源软件为天线设计的机器学习提供了高效解决方案。
...
Method: 使用冻结基础模型提取特征，开发双流分析模型（共享主流的全局泛化和专用细化的本地个性化）。

Result: APFL在多个数据集上表现优于现有方法，准确率提升1.10%-15.45%。

Conclusion: APFL通过双流设计实现了异构不变性，显著提升了非独立同分布数据下的个性化联邦学习效果。
...
TL;DR: 论文探讨了在强化学习中使用Pass@k作为奖励的潜力，发现其能提升模型的探索能力，并提出了一种高效的分析方法，表明探索与利用可以相互促进。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习中探索与利用的平衡问题，避免策略陷入局部最优。

Method: 使用Pass@k作为奖励训练策略模型，并通过分析推导其优势函数。

Result: Pass@k训练提升了探索能力，探索与利用可以相互促进，优势函数设计显示出潜力。

Conclusion: Pass@k训练为强化学习提供了一种新方向，优势函数设计是未来研究的重点。
...
TL;DR: 结合Erwin架构与Native Sparse Attention机制，提升Transformer模型在大规模物理系统中的效率和感受野，解决二次注意力复杂度问题。
...
Conclusion: Erwin NSA模型有效解决了二次注意力复杂度问题，适用于大规模物理系统。
...
TL;DR: IBEX提出了一种基于信息瓶颈理论的粗到细流程，解决了蛋白质-配体复合物数据稀缺的问题，显著提升了药物设计的性能。
...
Method: IBEX利用PAC-Bayesian信息瓶颈理论量化样本信息密度，采用Scaffold Hopping任务提升模型能力，并结合L-BFGS优化步骤精细调整构象。

Result: IBEX将零样本对接成功率从53%提升至64%，Vina评分从-7.41 kcal/mol提升至-8.07 kcal/mol，并在100个口袋中57个取得最佳Vina能量。
...
<details>
  <summary>Details</summary>
Motivation: 现有GNN-based GAD模型可能放大训练数据中的偏见，导致不公平结果，而现有公平性研究多针对节点分类任务，而非基于自动编码器的GAD模型。

Method: 提出DECAF-GAD，使用结构因果模型（SCM）解耦敏感属性，设计专用自动编码器架构和公平性损失函数。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用（如医疗和推荐系统）中的多臂老虎机问题通常是非平稳的，传统算法难以应对。
...
TL;DR: 本文比较了多种在线高斯过程（GP）算法的数据点缩减标准，分析了其计算复杂度和缩减行为，并提出了进一步过滤冗余数据点的接受标准。


<details>
  <summary>Details</summary>
Motivation: 高斯过程在回归和系统识别中广泛应用，但其计算复杂度限制了其在小数据集上的适用性，尤其是在流式数据场景下，数据点不断累积，即使是稀疏高斯过程也难以处理。

Method: 研究比较了多种数据点缩减标准，并在基准函数和真实数据集（包括动态系统识别任务）上进行了评估。同时提出了接受标准以进一步过滤冗余数据点。

Result: 研究结果为选择适合在线高斯过程算法的缩减标准提供了实用指南。

Conclusion: 本文为在线高斯过程算法的数据点缩减提供了统一比较和实用建议，有助于提升其在大规模流式数据中的应用效率。
...
TL;DR: 该综述提出了一种统一框架，将神经科学原理与记忆增强Transformer的工程进展结合，分析了功能目标、记忆表示和集成机制，并探讨了核心记忆操作的转变。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在长范围上下文保留、持续学习和知识整合方面的关键限制，结合神经科学原理提升其能力。

Method: 通过三个分类维度（功能目标、记忆表示、集成机制）组织进展，分析核心记忆操作，并提出解决方案。
...
TL;DR: 该论文提出了一个数据最小化在机器学习（DMML）的框架，旨在统一相关研究并帮助实践者应用数据最小化原则。


<details>
  <summary>Details</summary>
Motivation: 数据最小化（DM）是数据保护法规的核心原则，但在机器学习中缺乏统一的研究框架，导致实践者难以理解和应用。

Method: 引入了一个全面的DMML框架，包括统一的数据管道、对手模型和最小化点，并系统回顾了相关文献。

Result: 框架首次提供了结构化的概述，帮助实践者和研究者有效应用DM原则。
...
TL;DR: 提出了一种交互式验证范式，解决数据归属模型的计算成本高和信任问题，确保验证者能以较低计算成本验证数据归属的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前数据归属模型的计算成本高，导致资源有限的用户难以验证其准确性，从而引发信任问题。

Method: 通过交互式验证协议，由计算能力强的证明者与资源有限的验证者交互，确保数据归属的准确性和效率。

Result: 协议提供了形式化的完备性、可靠性和效率保证，验证者的计算成本与数据集大小无关。

Conclusion: 该方法为数据归属任务提供了一种高效且可信的验证机制，适用于多种线性函数验证。
...
TL;DR: 论文介绍了一个名为\ourdataset的数据集，用于解决AI驱动药物设计中缺乏实验先验知识的问题，通过LLM管道从文献中提取先验信息，并在多个任务中展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: AI驱动的药物设计虽然能提高效率，但缺乏实验先验知识可能导致设计违反隐含约束，例如产生高突变风险的分子。

Method: 构建了一个包含3230万对自然语言事实和实体表示的数据集\ourdataset，使用LLM管道从文献中提取信息，并训练了LLM、CLIP和LLava架构进行联合推理。

Result: 在TDC任务中，使用\ourdataset预训练的模型（仅1500万参数）表现优于更大的2B TxGemma模型，并与9B模型相当。在GuacaMol优化中，使用该数据集的模型能生成更安全且有效的分子。

Conclusion: \ourdataset为药物设计提供了有效的先验知识，显著提升了模型的性能和安全约束能力，未来将随文献增长扩展数据集。
